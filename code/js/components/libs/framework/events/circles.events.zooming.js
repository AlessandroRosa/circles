...
function circles_lib_coords_pickupyours_close_proc( _redraw, _question, _silent )
{
   ...
   if ( _redraw == YES )
   {
     ...
     _ret_chunk = circles_lib_canvas_process_ask(_question,_silent,W_PLANE,YES,YES,CHECK);

     var _ret_id = _ret_chunk != null ? safe_int( _ret_chunk[0], 0 ) : 0 ;
     var _ret_msg = _ret_chunk != null ? _ret_chunk[1] : _ERR_00_00 ;
   }
   else
   {
     ...
     {
       _ret_chunk = circles_lib_canvas_render_wplane( null, wplane_sm, null, YES, YES, YES, _question, _silent, OUTPUT_SCREEN );
       var _ret_id = is_array( _ret_chunk ) ? _ret_chunk[0] : RET_ERROR ;
			 var _ret_msg = is_array( _ret_chunk ) ? _ret_chunk[1] : "Unknown error" ;
			 if ( _ret_id == RET_ERROR ) circles_lib_log_add_entry( _ret_msg, LOG_ERROR );
		 }
...
{
    _plane_type = circles_lib_return_plane_type( _plane_type ) ;
    _canvas_role = safe_int( _canvas_role, 0 ), _question = safe_int( _question, YES ), _silent = safe_int( _silent, NO );
