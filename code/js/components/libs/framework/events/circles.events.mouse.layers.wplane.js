function W_PLANE_work_canvas_onmouseover( obj, event )
{
    ...
    if ( _glob_wplaneMOUSEprocSWITCH == MOUSE_ZOOM_PROC_ID )
		$( "#"+_glob_wplane_work_layer_pointer.id ).css('cursor','zoom-in');
    else if ( _glob_wplaneMOUSEprocSWITCH == MOUSE_PICK_LASTPT_PROC_ID )
		$( "#" + _glob_wplane_work_layer_pointer.id ).css('cursor', "url("+_glob_path_to_img+"icons/picker/picker.icon.01.20x20.png), auto" );
    else $( "#" + _glob_wplane_work_layer_pointer.id ).css('cursor','default');
}
...
function W_PLANE_work_canvas_onmouseup( obj, event )
{
    ...
    if ( _glob_wplaneMOUSEprocSWITCH == MOUSE_ZOOM_PROC_ID )
    {
       if ( is_rect( _glob_zoom_rect ) )
       {
           ...
           circles_lib_canvas_blowup( _glob_wplane_rendering_layer_pointer,
