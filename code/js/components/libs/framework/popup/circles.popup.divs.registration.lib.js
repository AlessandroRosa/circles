						                             ...
						                             _normalize_fns, _minimize_fns, _maximize_fns )
{
    ...
    HTMLcode += " ONDBLCLICK=\"javascript:circles_lib_popup_normalize('"+_div_id+"', '"+( is_array( _normalize_fns ) ? _normalize_fns.join("|") : "" )+"');\"" ;
    ...
    HTMLcode += "<td CLASS=\"popup_caption_btn\" ONCLICK=\"javascript:circles_lib_popup_maximize('"+_div_id+"', '"+( is_array( _maximize_fns ) ? _maximize_fns.join("|") : "" )+"');\"><IMG TITLE=\"Maximize Window\" SRC=\"%imgpath%icons/wnd/maximize.icon.01.12x12.png\"></td>" ;
		HTMLcode += "<td CLASS=\"popup_caption_btn\" ONCLICK=\"javascript:circles_lib_popup_minimize('"+_div_id+"', '"+( is_array( _minimize_fns ) ? _minimize_fns.join("|") : "" )+"');\"><IMG TITLE=\"Minimize Window\" SRC=\"%imgpath%icons/wnd/minimize.icon.01.12x12.png\"></td>" ;
	  HTMLcode += "<td CLASS=\"popup_caption_btn\" ONCLICK=\"javascript:circles_lib_popup_normalize('"+_div_id+"', '"+( is_array( _normalize_fns ) ? _normalize_fns.join("|") : "" )+"');\"><IMG TITLE=\"Normalize Window\" SRC=\"%imgpath%icons/wnd/normalize.icon.01.12x12.png\"></td>" ;

    if ( _arrows )
    {
       HTMLcode += "<td WIDTH=\"2\"></td>" ;
       HTMLcode += "<td CLASS=\"popup_caption_btn\" ONCLICK=\"javascript:circles_lib_popup_move_wnd( '"+_div_id+"', '', 'TOP' );\"><IMG TITLE=\""+_CAPTION_BTN_03+"\" SRC=\"%imgpath%icons/bullets/bullet.up.12x12.png\"></td>" ;
       HTMLcode += "<td CLASS=\"popup_caption_btn\" ONCLICK=\"javascript:circles_lib_popup_move_wnd( '"+_div_id+"', '', 'BOTTOM' );\"><IMG TITLE=\""+_CAPTION_BTN_04+"\" SRC=\"%imgpath%icons/bullets/bullet.down.12x12.png\"></td>" ;
       HTMLcode += "<td CLASS=\"popup_caption_btn\" ONCLICK=\"javascript:circles_lib_popup_move_wnd( '"+_div_id+"', 'LEFT', '' );\"><IMG TITLE=\""+_CAPTION_BTN_01+"\" SRC=\"%imgpath%icons/bullets/bullet.left.12x12.png\"></td>" ;
       HTMLcode += "<td CLASS=\"popup_caption_btn\" ONCLICK=\"javascript:circles_lib_popup_move_wnd( '"+_div_id+"', 'RIGHT', '' );\"><IMG TITLE=\""+_CAPTION_BTN_02+"\" SRC=\"%imgpath%icons/bullets/bullet.right.12x12.png\"></td>" ;
       HTMLcode += "<td WIDTH=\"2\"></td>" ;
    }
...
function circles_lib_popup_create( _base_id, _div_id, _subset, WIDTH, HEIGHT, contents, _class, _zIndex, _bind_events, _top )
{
                                 ...
                                 function(e)
                                 {
                                    if ( e != null )
                                    {
                                       if ( e.originalEvent != null )
                                       {
                                          ...
                                          circles_lib_popup_focus( _div_id, _subset, YES, e );
                                       }
                                    }
                                 }
                               );
...
function circles_lib_popup_destroy_wnd( _id )
{
    var _ret_index = circles_lib_popup_exists( _id, POPUP_SEARCH_BY_DIV_ID | POPUP_SEARCH_BY_BASE_ID | POPUP_SEARCH_BY_UNIQUE_ID );
    if ( _ret_index != UNFOUND )
    {
       var _popup_chunk = circles_lib_popup_find_wnd( _id, POPUP_SEARCH_BY_DIV_ID | POPUP_SEARCH_BY_BASE_ID | POPUP_SEARCH_BY_UNIQUE_ID );
       ...
       else
       {
         ...
         if ( $( _div_id ).get(0) != null )
         {
            ...
            circles_lib_popup_delete_from_archive( _ret_index );
...
function circles_lib_popup_register( _calling_params, _div_id, _caption, _status, _visible, _caption_class, _subset, _base_id, _allow_multiple_instances )
{
    ...
    if ( circles_lib_popup_exists( _unique_id, POPUP_SEARCH_BY_UNIQUE_ID ) == UNFOUND || _allow_multiple_instances )
					                           ...
					                           _normalize_fns, _minimize_fns, _maximize_fns, _caption_class )
{
    ...
    var _fn = function() { circles_lib_popup_destroy_wnd( _div_id ) } ;
    _b_open ? $( "#" + _div_id ).slideDown("slow") : $( "#" + _div_id ).slideUp("fast", _fn );

    if ( !_b_open ) hideCOLORTABLE();
    else _glob_popup_sel_unique_id = _div_id ;

    var _index = circles_lib_popup_exists( _div_id, POPUP_SEARCH_BY_DIV_ID | POPUP_SEARCH_BY_BASE_ID | POPUP_SEARCH_BY_UNIQUE_ID, _caption );
    var _popup_obj = null ;
    if ( _b_open )
    {
        if ( _index == UNFOUND || _allow_multiple_instances )
        {
           circles_lib_popup_set_property_to_all_entries( 0, POPUP_SEARCH_BY_STATUS );
           _popup_obj = circles_lib_popup_register( _calling_params, _div_id, _caption, OPEN, SHOW, _caption_class, _subset, _base_id, _allow_multiple_instances );
        }
        else
        {
    ...
    else if ( _index != UNFOUND )
    {
        ...
        circles_lib_popup_delete_from_archive( _index );
    }
    ...
    if ( _b_open ) circles_lib_popup_list_selection_render( YES, YES );

    $( "#" + _div_id ).draggable(
    {
        start: function() { circles_lib_popup_dragstart_override_fn(); },
        drag: function() { circles_lib_popup_drag_override_fn(); },
        stop: function()
              {
                 ...
                 _index = circles_lib_popup_exists( _div_id, POPUP_SEARCH_BY_DIV_ID | POPUP_SEARCH_BY_BASE_ID | POPUP_SEARCH_BY_UNIQUE_ID, _caption );
                 _glob_popups_array[_index][14].width_height_constructor( _left, _top, _width, _height, _RECT_ORIENTATION_SCREEN );
                 circles_lib_popup_dragstop_override_fn();
              }
    } );
    ...
    if ( !_b_open && $( "#" + _div_id + "_menu_div" ).get(0) != null ) circles_lib_popup_destroy_wnd( _div_id + "_menu_div" );
    var _n = circles_lib_popup_match_count( _base_id, POPUP_SEARCH_BY_BASE_ID ) ;
