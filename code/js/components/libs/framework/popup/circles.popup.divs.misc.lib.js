...
function circles_lib_popup_clean_baseid( _id ) { return safe_string( _id, "" ).replaceAll( [ "_", ".", " " ], "" ).toUpperCase(); }
function circles_lib_popup_build_divid( _subset, _base_id ) { return "CIRCLES"+( safe_string( _subset, "forms" ) )+( safe_string( _base_id, "" ).replaceAll( [ "_", ".", " " ], "" ).toUpperCase() )+"popupWND" ; }
function circles_lib_popup_focus_wnd_from_inner_ctrl_id( _ctrl_id, _subset )
{
    if ( $("#"+circles_lib_popup_get_wnd_from_inner_ctrl_id( _ctrl_id ) ).length > 0 )
    circles_lib_popup_focus( _popup_id, subset );
}

function circles_lib_popup_get_wnd_from_inner_ctrl_id( _ctrl_id )
{
        ...
        while( _ctrl != null )
        {
            ...
            if ( _parent_node != null )
            {
                ...
                if ( _parent_id.length > 0 && _parent_tag.length > 0 &&
                     circles_lib_popup_get_datamask_from_property( _parent_id ) == POPUP_SEARCH_BY_DIV_ID )
                {
                   _b_found = YES ;
                   _popup_obj = circles_lib_popup_find_wnd( _parent_id, POPUP_SEARCH_BY_DIV_ID, NO ) ;
                   break ;
                }
...
function circles_lib_popup_get_max_zindex()
{
    var zI = 0 ;
...
function circles_lib_popup_get_topmost_wnd()
{
    var _div_id = "", zI = 0 ;
...
function circles_lib_popup_load()
{
...
// PROPERTIES
function circles_lib_popup_set_property( _unique_id, _obj, _datatype_mask, _offset )
{
...
function circles_lib_popup_get_datamask_from_property( _prop_value, _offset )
{
    var _array = _glob_popups_array, _len = safe_size( _array, 0 ), _ret = UNFOUND ;
    _offset = Math.min( Math.max( safe_int( _offset, 0 ), 0 ), _len - 1 ) ;
    for( var _i = _offset ; _i < _len ; _i++ )
...
function circles_lib_popup_check_property( _unique_id, _val, _datatype_mask, _offset )
{
...
function circles_lib_popup_set_property_to_all_entries( _obj, _datatype_mask /* 1: unique id, 2: description, 3: status, 4: visible, 5: height */, _offset )
{
    var _array = _glob_popups_array, _len = safe_size( _array, 0 ), _ret = UNFOUND ;
