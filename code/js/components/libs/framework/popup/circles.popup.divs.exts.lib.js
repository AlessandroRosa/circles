...
function circles_lib_popup_dragstart_override_fn() {}
function circles_lib_popup_drag_override_fn() {}
function circles_lib_popup_dragstop_override_fn() {}
function circles_lib_popup_get_active_wnd()
{
...
function circles_lib_popup_get_wnd_from_pos( _x, _y )
{
...
function circles_lib_popup_is_visible( _form_title )
{
...
function circles_lib_popup_reload_basic_forms( _param_01 )
{
    ...
    if ( circles_lib_popup_exists( _base_ids[_p], POPUP_SEARCH_BY_BASE_ID ) != UNFOUND )
		circles_lib_popup_load('forms',_base_ids[_p], NO, _param_01 );
}

function circles_lib_popup_hide_all()
{
...
function circles_lib_popup_close_all()
{
    ...
    if ( _len > 0 )
    {
        ...
        for( var _i = 0 ; _i < _len ; _i++ )
        {
            if ( is_array( _array[_i] ) )
            {
               ...
               if ( function_exists( _candidate_close_fn ) ) circles_lib_popup_activate( _allow_multiple_instances, _base_id, "", "", _subset, CLOSE, _array[_i], '', _candidate_close_fn );
               else if ( _subset.stricmp( "embeddings" ) ) circles_lib_popup_activate( _allow_multiple_instances, _base_id, "", "", _subset, CLOSE, GLOB_PLUGIN_DIV_ID, '', 'PLUGINSpopupCLOSE' );
            }
        }
    }
}

function circles_lib_popup_deactivate_all( _close )
{
    ...
    for( var _i = 0 ; _i < _len ; _i++ )
    {
        if ( is_array( _array[_i] ) )
        {
           ...
           */
           ...
           startINDEX++ ;
           _allow_multiple_instances = _array[_i][startINDEX] ;
           if ( _close ) circles_lib_popup_activate( _allow_multiple_instances, _base_id, "", "", _subset, CLOSE, _div_id, _caption );
           _glob_popups_array[_i][startINDEX] = 0 ; // status inactive
        }
    }
}

function circles_lib_popup_get_list( _fn_str, _html )
{
    ...
    if ( _len > 0 )
    {
       ...
       _list += "<td ALIGN=\"right\" TITLE=\"Close\" CLASS=\"link\" STYLE=\"padding-right:2px;\" ONCLICK=\"javascript:circles_lib_popup_show_list(HIDE);\" WIDTH=\"35\"><IMG SRC=\"%imgpath%icons/close/close.icon.01.12x12.png\"></td>" ;
       _list += "</tr>" ;
       _list += "</table>" ;
       _list += "</td>" ;
       _list += "</tr>" ;
       ...
       for( var _i = 0 ; _i < _len ; _i++ )
       {
           ...
           if ( _chunk != null )
           {
              ...
              _list += "ONCLICK=\"javascript:circles_lib_popup_focus('"+_div_id+"','"+_subset+"',YES,event);\"" ;
              _list += ">"+_caption+"</td>" ;
              ...
              _list += "<td WIDTH=\"20\" CLASS=\"link\" ONCLICK=\"javascript:circles_lib_popup_activate( "+_allow_multiple_instances+", '"+_base_id+"', '', '', '"+_subset+"', CLOSE, '"+_div_id+"','');\"><IMG SRC=\"%imgpath%icons/delete/delete.icon.16x16.png\"></td>" ;
...
function circles_lib_popup_show_list( bSHOW )
{
    var _statusbar_popup_id = "STATUSBARpopuplist" ;
    var _status_box = $("#" + _statusbar_popup_id ).get(0);
    if ( _status_box != null )
    {
       var _ret_chunk = circles_lib_popup_get_list( "circles_lib_popup_list_selection_render( YES, YES )" );
       ...
       if ( bSHOW && _ret_popups_n > 0 )
       {
           ...
           if ( _div != null )
           {
               ...
               _div.style.zIndex = circles_lib_popup_get_max_zindex() + 1 ;
...
function circles_lib_popup_normalize( _div_id, _resize_fns )
{
...
function circles_lib_popup_minimize( _div_id, _resize_fns )
{
...
function circles_lib_popup_maximize( _div_id, _resize_fns )
{
    _div_id = safe_string( _div_id, "" );
...
function circles_lib_popup_add_contextmenu_entry( _base_id, _menu_label_entry, _fn_name, _fn_args_array )
{
    ...
    if ( !is_array( _fn_args_array ) ) _fn_args_array = [] ;
		var _entry_array_ref = circles_lib_popup_find_wnd( _base_id, POPUP_SEARCH_BY_DIV_ID | POPUP_SEARCH_BY_BASE_ID | POPUP_SEARCH_BY_UNIQUE_ID, YES ) ;
