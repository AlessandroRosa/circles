function circles_lib_items_copy_to_storage_space( _item_type, _question, _silent, _out_channel )
{
    ...
    if ( _items_n > 0 )
    {
       ...
       if ( _b_go )
       {
           ...
           if ( !_silent && _out_channel == OUTPUT_SCREEN ) circles_lib_output( OUTPUT_SCREEN, _n_copied == 0 ? DISPATCH_WARNING : DISPATCH_SUCCESS, _msg, _glob_app );
...
function circles_lib_items_unselect( _question, _silent, _out_channel )
{
    ...
    if ( _glob_disk_sel_index != UNDET )
    {
		   ...
		   {
		      if ( _out_channel == OUTPUT_SCREEN && !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_ERROR, "Can't unselect: " + _ERR_33_01, _glob_app );
...
function circles_lib_items_switch_to( _switch_to_val, _silent, _out_channel )
{
    ...
    if ( _out_channel == OUTPUT_SCREEN && !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, _ret_msg, _glob_app + " - Items init" );
...
function circles_lib_items_init_wrapper_fn( _index, _question, _silent, _init_mask, _out_channel )
{
    ...
    {
       if ( _out_channel == OUTPUT_SCREEN && !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, _ERR_24_03, _glob_app );
    ...
    else if ( _items_n > 0 )
    {
       if ( !_question ? YES : confirm( _QUESTION_26_01 ) )
       {
          ...
          {
              if ( _out_channel == OUTPUT_SCREEN && !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_ERROR, _ret_chunk[0], _glob_app );
              return _ret_chunk ;
          }
          ...
          else if ( _items_error != ITEM_ERR_NONE )
          {
             ...
             if ( _out_channel == OUTPUT_SCREEN && !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, errMSG, _glob_app );
             return [ RET_ERROR, errMSG ];
          }
...
function circles_lib_items_create_from_disk( _index, complex_circle, screen_circle )
{
    ...
    return new item_obj( _mm, complex_circle, screen_circle, _symbol,
                         ...
                         _check ? complex_circle.lineiwdth : _items_array[_index].complex_circle.linewidth,
    ...
    else if ( _items_n > 0 )
    {
			 ...
			 if ( _check )
       {
           ...
           for( var _i = 0 ; _i < _items_n ; _i++ )
           {
               ...
               if ( _failure_mask ) return [ RET_ERROR, "Inconsistent item symbol at index #"+_i+"\n\nSet up the symbols manually or click <SPAN STYLE=\"cursor:pointer;font-weight:bold;color:blue;\" ONCLICK=\"javascript:alertCLOSE();circles_lib_alphabet_autoconfig_all_symbols(!_glob_terminal_silent,_glob_terminal_silent,NO,OUTPUT_SCREEN);\">here</SPAN> for &lsquo;auto-config&rsquo;" ] ;
       ...
       {
           if ( _out_channel == OUTPUT_SCREEN && !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, _ERR_33_06, _glob_app );
           return [ RET_ERROR, _ERR_33_06 ] ;
       }
    }
    else
    {
       if ( _out_channel == OUTPUT_SCREEN && !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, _ERR_33_01, _glob_app );
...
function circles_lib_items_init_group_from_maps( _silent, _init_mask, _report, _force, _out_channel )
{
    ...
    else if ( _items_n > 0 )
    {
       if ( _items_n % 2 == 0 )
       {
        	 ...
        	 var _draw, _fill, _drawcolor, _fillcolor, _linewidth, _inv_symbol, coords ;
           if ( _report )
           {
           ...
           for( var _i = 0 ; _i < _items_n ; _i++ )
           {
               ...
               if ( _symbol.length > 0 && is_mobius_map( ITEM.map ) )
               {
                   ...
                   _linewidth = _items_array[_i].complex_circle.linewidth ;
                   if ( !_locked )
                   {
                       ...
                       _items_array[_i].complex_circle.linewidth = _linewidth ;
					             _items_array[_i].screen_circle = circles_lib_get_screendisk_from_complexdisk( zplane_sm, _items_array[_i].complex_circle );
                       ...
                       if ( _glob_method.is_one_of( METHOD_ALGEBRAIC ) )
                       {
                           ...
                           if ( _index_inverse == UNFOUND && _init_mask & INIT_PAIRED_ITEMS )
                           {
                               ...
                               _inverse_mm_obj = new item_obj( _inverse_mm, _complex_circle, screen_circle, _inv_symbol, 0,
                                                               ...
                                                               _symbol, _linewidth, ITEM_TYPE_CIRCLE, "", _inv_symbol );
                               ...
                               if ( _index_inverse != UNFOUND )
                               {
                                  if ( _items_array[_index_inverse].symbol.strcmp( _inv_symbol ) )
                                  {
                                     ...
                                     _items_array[_index_inverse].complex_circle.linewidth = _linewidth ;
               ...
               else
               {
                   ...
                   if ( _out_channel == OUTPUT_SCREEN && !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_ERROR, _msg.strip_tags(), _glob_app );
                   return [ RET_ERROR, _msg ];
               }
           }
       ...
       {
          if ( _out_channel == OUTPUT_SCREEN && !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_ERROR, "Failure: " + _ERR_33_06, _glob_app );
          return [ RET_ERROR, "Failure: " + _ERR_33_06 ];
       }
    }
    else
    {
       if ( _out_channel == OUTPUT_SCREEN && !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_ERROR, _ERR_00_10, _glob_app );
...
function circles_lib_items_group_test( _silent, _out_channel ) // main function
{
    ...
    if ( !_silent && _out_channel == OUTPUT_SCREEN ) circles_lib_output( OUTPUT_SCREEN, _ret_id >= 0 ? DISPATCH_SUCCESS : DISPATCH_ERROR, _ret_msg, _glob_app );
