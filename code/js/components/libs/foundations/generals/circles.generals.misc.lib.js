//LOG
function circles_lib_log_add_entry( _log_str = "", _log_type = LOG_NONE ) { if ( _log_str.length > 0 ) { _glob_app_log.push( [ today_date() + " - "+ current_time(), _log_str, _log_type ] ) ; circles_lib_log_update_statusbar() ; } }
function circles_lib_log_update_statusbar()
{
	circles_lib_statusbar_log_icon_show( safe_size( _glob_app_log, 0 ) > 0 ? YES : NO ) ;
	if ( circles_lib_plugin_find_index( { subset : "forms", base_id : "log" }, POPUP_SEARCH_BY_BASE_ID | POPUP_SEARCH_BY_SUBSET ) != UNFOUND )
	{
...
function circles_lib_count_items( _items_array = _glob_seeds_array, _safe_count )
{
...
// OUTPUT SELECTOR FNS
function circles_lib_samples_load( i = 0, _silent = YES )
{
...
// CONFIG FNS
function circles_lib_drawentity_set( _drawentity = DRAWENTITY_NONE ) { _glob_drawentity = safe_int( _drawentity, DRAWENTITY_NONE ); }
function circles_lib_drawentity_get() { return _glob_drawentity ; }
function circles_lib_depth_get() { return safe_int( _glob_depth, 0 ); }
function circles_lib_depth_set( _d = UNDET, _force_dict = NO )
{
...
function circles_lib_masterdiv_display( _b_show = 1 ) { _b_show ? $("#MASTERdiv").show() : $("#MASTERdiv").hide(); }
function circles_lib_construction_mode_get() { return _glob_construction_mode ; }
function circles_lib_construction_mode_set( _cm = CONSTRUCTION_TILING )
{
