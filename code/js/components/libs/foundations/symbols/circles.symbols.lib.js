...
{
	_items_array = circles_lib_items_set( _items_array ) ;
    _canvas = !is_html_canvas( _canvas ) ? _glob_zplane_work_layer_pointer : _canvas ;
    ...
    {
        var ITEM, _circle_obj, _center_pt, _radius, _symbol, _bordersize, _check_sc ;
        for( var i = 0 ; i < _items_n ; i++ )
        {
            ITEM = _items_array[i] ;
            ...
            _bordersize = _check_sc ? safe_int( ITEM.screen_circle.bordersize, 0 ) : 0 ;
