...
function circles_lib_alphabet_autoconfig_all_symbols( _question, _silent, _force, _init_maps, _output_channel )
{
		_question = safe_int( _question, YES ), _silent = safe_int( _silent, NO ), _force = safe_int( _force, NO );
    _init_maps = safe_int( _init_maps, YES ), _output_channel = safe_int( _output_channel, OUTPUT_SCREEN );
    var _items_array = _glob_items_switch == ITEMS_SWITCH_GENS ? _glob_gens_array : _glob_seeds_array ;
    var _items_n = circles_lib_count_items(_items_array);
    if ( _glob_method == METHOD_NONE )
    {
        if ( _output_channel == OUTPUT_SCREEN && !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, _ERR_24_02, _glob_app_title );
    ...
    {
        var _b_go = ( _output_channel == OUTPUT_SCREEN && _question ) ? confirm( _QUESTION_24 ) : YES ;
        if ( _b_go )
        {
           if ( _items_n > 0 )
           {
