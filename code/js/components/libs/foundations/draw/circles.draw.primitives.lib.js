function circles_lib_draw_arrow( _context, _from_x, _from_y, _to_x, _to_y, _head_len, _linesize, _bordercolor, _display_shaft )
{
    ...
    _bordercolor = safe_string( _bordercolor, "transparent" );
    _display_shaft = safe_int( _display_shaft, YES );
    _glob_persistent_vars['angle'] = Math.atan2( _to_y - _from_y , _to_x - _from_x );
    _context.beginPath();
    _context.strokeStyle = _bordercolor, _context.lineWidth = _linesize ;
    if ( _display_shaft )
    {
...
function circles_lib_draw_screen_disk( _context, _word, _scr_circle, _border, _bordercolor, _fill, _fillcolor, _bordersize, _opacity, _properties_mask, _sector_rad_start, _sector_rad_end )
{
    ...
    else if ( !_border )
    {
        ...
        else
        {
           ...
           if ( _glob_apply_dashed_border && _bordersize > 0 ) _context.stroke();
    ...
    if ( _glob_symbols_display_wplane )
    {
        ...
        {
            case EXPORT_SVG:
            _svg_text( _glob_export_code_array, _glob_persistent_vars['x'], _glob_persistent_vars['y'], _word, _bordercolor, DEFAULT_FONT_FAMILY, DEFAULT_FONT_SIZE );
            break ;
            case EXPORT_PS:
            _glob_js_e_ps_obj.text( DEFAULT_FONT_FAMILY, DEFAULT_FONT_SIZE, _bordercolor, _glob_persistent_vars['x'], _glob_persistent_vars['y'], _word, "" );
            break ;
            case EXPORT_EPS:
            _glob_js_e_ps_obj.text( DEFAULT_FONT_FAMILY, DEFAULT_FONT_SIZE, _bordercolor, _glob_persistent_vars['x'], _glob_persistent_vars['y'], _word, "" );
            break ;
            case EXPORT_LATEX:
            _glob_js_latex_obj.text( _glob_persistent_vars['x'], _glob_persistent_vars['y'], _word, _bordercolor );
            break ;
            case EXPORT_NONE:
            default: break ;
        }
...
function circles_lib_draw_complex_disk( _context, _mapper, _complex_x, _complex_y, _complex_radius, _border, _bordercolor, _fill, _fillcolor, _bordersize, _opacity, _sector_rad_start, _sector_rad_end, _word, _properties_mask )
{
   ...
   if ( _glob_symbols_display_wplane )
   {
       ...
       switch( _glob_export_format )
       {
           ...
           break ;
           case EXPORT_PS:
           _glob_js_e_ps_obj.text( DEFAULT_FONT_FAMILY, DEFAULT_FONT_SIZE, _bordercolor, _glob_persistent_vars['x'], _context.canvas.clientHeight - _glob_persistent_vars['y'], _word, "" );
           break ;
           case EXPORT_EPS:
           _glob_js_e_ps_obj.text( DEFAULT_FONT_FAMILY, DEFAULT_FONT_SIZE, _bordercolor, _glob_persistent_vars['x'], _context.canvas.clientHeight - _glob_persistent_vars['y'], _word, "" );
           break ;
           case EXPORT_LATEX:
           _glob_js_latex_obj.text( _glob_persistent_vars['x'], _context.canvas.clientHeight - _glob_persistent_vars['y'], _word, _bordercolor );
           break ;
...
function circles_lib_draw_segment( _context, _mapper, _from_x, _from_y, _to_x, _to_y, _bordercolor, _bordersize, _opacity, _properties_mask, _map_it )
{
   ...
   _properties_mask = safe_int( _properties_mask, 0 );
   _bordercolor = safe_string( _bordercolor, _glob_draw_seed_color );
   if ( _map_it )
   {
   ...
   switch( _glob_export_format )
   {
       ...
       if ( _border && is_point( _scr_start_pt ) && is_point( _scr_end_pt ) )
       {
           ...
           _glob_js_e_ps_obj.line( _scr_start_pt.x, _context.canvas.clientHeight - _scr_start_pt.y,
                                   ...
                                   _bordersize, _bordercolor, "", UNDET, "" );
       }
       break ;
       case EXPORT_EPS:
       if ( _border && is_point( _scr_start_pt ) && is_point( _scr_end_pt ) )
       ...
       if ( _border && is_point( _scr_start_pt ) && is_point( _scr_end_pt ) )
       {
           ...
           _glob_js_latex_obj.line( _scr_start_pt.x, _context.canvas.clientHeight - _scr_start_pt.y,
                                    _scr_end_pt.x, _context.canvas.clientHeight - _scr_end_pt.y, _bordercolor );
       }
       break ;
       case EXPORT_NONE: default: break ;
   }
...
function circles_lib_draw_polyline( _context, _mapper, _pts_array = [], _bordercolor, _fillcolor, _bordersize, _close, _opacity, _tolerance, _properties_mask, _map_it )
{
    ...
    if ( _glob_persistent_vars['p_len'] > 1 )
    {
       ...
       if ( _bordercolor.length > 0 ) _context.stroke();
    ...
    switch( _glob_export_format )
    {
        ...
        _glob_js_e_ps_obj.broken_line( _glob_persistent_vars['out_pts_array'], _bordersize, _close, _bordercolor, _fillcolor, "", "" );
        break ;
        case EXPORT_EPS:
        _glob_persistent_vars['pt_l'] = safe_size( _glob_persistent_vars['scr_pts_array'], 0 );
        _glob_persistent_vars['out_pts_array'] = [];
        ...
        _glob_persistent_vars['out_pts_array'] = [];
        for( var _i = 0 ; _i < _glob_persistent_vars['pt_l'] ; _i++ ) _glob_persistent_vars['out_pts_array'].push( [ _glob_persistent_vars['scr_pts_array'][_i].x, _context.canvas.clientHeight - _glob_persistent_vars['scr_pts_array'][_i].y ] );
        _glob_js_latex_obj.broken_line( _glob_persistent_vars['out_pts_array'], _bordercolor, _close );
        break ;
        case EXPORT_NONE: default: break ;
    }
    return YES ;
...
function circles_lib_draw_rounded_rect( _context, _mapper, _rect_obj, _border, _bordercolor, _fill, _fillcolor, _bordersize, _radius, _map_it, _opacity, _properties_mask )
{
    ...
    _glob_persistent_vars['width'] = _bordercolor.length == 0 ? _rect_obj.width() + 1 : _rect_obj.width();
...
function circles_lib_draw_pixel( _context, _mapper, _pt_x, _pt_y, _bordercolor, _size, _opacity, _properties_mask, _map_it )
{
   if ( !is_html_context( _context ) || !is_screen_mapper( _mapper ) ) return null ;
    ...
    {
       case EXPORT_SVG:
       _svg_pixel( _glob_export_code_array, _glob_persistent_vars['scr_pt'], 1, YES, _bordercolor );
       break ;
       case EXPORT_PS:
       _glob_js_e_ps_obj.pixel( _glob_persistent_vars['scr_pt'].x, _context.canvas.clientHeight - _glob_persistent_vars['scr_pt'].y, _bordercolor, "" );
       break ;
       case EXPORT_EPS:
       _glob_js_e_ps_obj.pixel( _glob_persistent_vars['scr_pt'].x, _context.canvas.clientHeight - _glob_persistent_vars['scr_pt'].y, _bordercolor, "" );
       break ;
       case EXPORT_LATEX:
       _glob_js_latex_obj.pixel( _glob_persistent_vars['scr_pt'].x, _context.canvas.clientHeight - _glob_persistent_vars['scr_pt'].y, _bordercolor, "" );
       break ;
       case EXPORT_NONE: default: break ;
    }
