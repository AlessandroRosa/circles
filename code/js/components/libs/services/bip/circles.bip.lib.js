...
function circles_lib_bip_apply_settings( _out_channel = OUTPUT_SCREEN, _question = YES, _silent = NO, _update = NO,
			                             _center, _x_extent = 1, _y_extent = 1,
			                             _shorterside = 0, _coords_diagram, _data_diagram, _bk = "" )
{
    ...
    var _b_go = ( _out_channel == OUTPUT_SCREEN && _question ) ? ( confirm( "Do you confirm to apply these settings ?" ) ? YES : NO ) : YES ;
    ...
    if ( _b_go )
    {
        ...
        else
        {
            ...
            if ( _shorterside > 0 ) _glob_bip_shorterside_pixels = safe_int( _shorterside, 1 );
        ...
        if ( _b_fail == 0 && _glob_bip_halt == NO )
        {
            ...
            {
				var _ratio = _glob_bip_y_extent > 0 ? ( _glob_bip_x_extent / _glob_bip_y_extent ) : 1 ;
                // resize canvas width/height
                if ( _glob_bip_x_extent <= _glob_bip_y_extent )
                {
                    _canvas_width = _glob_bip_shorterside_pixels ;
                    _canvas_height = _canvas_width / _ratio ;
                    _canvas_height = safe_int( _canvas_height, DEFAULT_BIP_SIZE );
                }
                else
                {
                    _canvas_height = _glob_bip_shorterside_pixels ;
                    _canvas_width = _canvas_height * _ratio ;
            ...
            if ( is_html_canvas( _glob_bipbox_canvas ) )
            {
                ...
                circles_lib_canvas_clean( _glob_bipbox_canvas, _glob_bip_bk, _out_channel );
...
function circles_lib_bip_render( _silent, _out_channel )
{
    ...
    else if ( is_html_canvas( _glob_bipbox_canvas ) )
    {
        ...
        switch( _glob_bip_original_plane_data )
        {
            ...
            circles_lib_canvas_clean( _glob_bipbox_canvas, "", _out_channel );
