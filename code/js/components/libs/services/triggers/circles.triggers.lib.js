function circles_lib_triggers_open( _trigger_id, _silent, _output_channel )
{
    ...
    if ( _trigger_chunk != null )
    {
       ...
       {
          var _ret ;
          $.getScript( _trigger_fullpath,
                       function( response, status )
                       {
                          if ( status.toLowerCase().stricmp( "success" ) )
                          {
                             if ( _trigger_fn.omits( "(" ) ) _trigger_fn += "( _silent, _output_channel )" ;
                             else if ( _trigger_fn.end_with( "()" ) ) _trigger_fn = _trigger_fn.replaceAll( "()", "( _silent, _output_channel )" );
                             eval( _trigger_fn );
                          ...
                          else
                          {
                             ...
                             if ( !_silent && _output_channel == OUTPUT_SCREEN ) circles_lib_output( OUTPUT_SPECIAL_FX, DISPATCH_WARNING, _msg, "circles_lib_triggers_outbox" ) ;
                          }
                       }
                     );
    ...
    else
    {
       ...
       if ( !_silent && _output_channel == OUTPUT_SCREEN ) circles_lib_output( OUTPUT_SPECIAL_FX, DISPATCH_ERROR, _msg, "circles_lib_triggers_outbox" ) ;
       return [ RET_ERROR, _msg ] ;
    }
}
...
function circles_lib_triggers_open_all_automated_entries( _silent, _output_channel )
{
    ...
    $.each( _triggers_ids,
            function( _i, _trig_id )
            {
               _auto = safe_int( _glob_triggers_table[''+_trig_id][4], 0 ) ;
               _ret_chunk = _auto ? circles_lib_triggers_open( _trig_id, YES, _output_channel ) : null ;
...
function circles_lib_triggers_open_all( _silent, _output_channel )
{
    _silent = safe_int( _silent, NO ), _output_channel = safe_int( _output_channel, OUTPUT_SCREEN );
    ...
    $.each( _triggers_ids,
            function( _i, _trig_id )
            {
               _ret_chunk = circles_lib_triggers_open( _trig_id, YES, _output_channel );
               _ret &= _ret_chunk[0] ;
...
function circles_lib_triggers_set_all_to_automated( _on, _silent, _output_channel )
{
    ...
    if ( !_silent && _output_channel == OUTPUT_SCREEN ) circles_lib_output( OUTPUT_SPECIAL_FX, DISPATCH_SUCCESS, _msg, "circles_lib_triggers_outbox" ) ;
    return [ RET_OK, _msg ] ;
}