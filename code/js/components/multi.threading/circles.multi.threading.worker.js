	...
	else if ( _workarea == MULTITHREADING_OBJ_WORKAREA_RENDERING ) // rendering
    {
       ...
       var _canvas_w = 0, _canvas_h = 0 ;
  	   ...
  	   var _working_plane = safe_int( _options_chunk[ _startINDEX ], _glob_target_plane );
           _startINDEX++ ;
  	   var _canvas = _options_chunk[ _startINDEX ]; if ( !is_html_canvas( _canvas ) ) _canvas = _glob_wplane_rendering_layer_pointer ;
           _startINDEX++ ;
       ...
       for( var _key in _glob_symbols_index_array )
       {
         ...
         _tmp_array.push( _key + "|" + _glob_symbols_index_array[_key] );
			 ...
			 {
				 case Z_PLANE:
				 _glob_opacity = $( "#"+_glob_zplane_rendering_layer_pointer.get_iddiv() ).get(0).style.opacity ;
         _canvas_w = is_html_canvas( _glob_zplane_rendering_layer_pointer ) ? _glob_zplane_rendering_layer_pointer.get_width() : 0 ;
         _canvas_h = is_html_canvas( _glob_zplane_rendering_layer_pointer) ? _glob_zplane_rendering_layer_pointer.get_height() : 0 ;
         _glob_multithread_canvas = _glob_zplane_rendering_layer_pointer ;
         _glob_multithread_mapper = zplane_sm.copy() ;
         _glob_multithread_pixelsize = 1 ;
				 break ;
				 case W_PLANE:
				 _glob_opacity = $( "#"+_glob_wplane_rendering_layer_pointer.get_iddiv() ).get(0).style.opacity ;
         _canvas_w = is_html_canvas( _glob_wplane_rendering_layer_pointer ) ? _glob_wplane_rendering_layer_pointer.get_width() : 0 ;
         _canvas_h = is_html_canvas( _glob_wplane_rendering_layer_pointer ) ? _glob_wplane_rendering_layer_pointer.get_height() : 0 ;
         _glob_multithread_canvas = _glob_wplane_rendering_layer_pointer ;
