function circles_lib_terminal_interpreter( _cmd, _terminal, _out_channel )
{
    ...
    if ( safe_size( _cmd, 0 ) > 0 && _terminal != null )
    {
       ...
       var _ret_cmd = circles_lib_terminal_exec( _cmd_tag, _params_str, _terminal, _out_channel );
       ...
       if ( _out_channel == OUTPUT_FUNCTION ) return _ret_cmd ;
    }
    else return null ;
}
...
function circles_lib_terminal_exec( _input_str_01, _input_str_02, _terminal, _out_channel )
{
    ...
    else
    {
       var _cmd_tag = _input_str_01, _params_str = _input_str_02 ;
	     ...
	     {
          var _ret_chunk = circles_lib_terminal_load_cmd( _cmd_tag, _params_str, _out_channel, TERMINAL_CMD_MODE_ACTIVE );
          ...
          if ( _out_channel == OUTPUT_FUNCTION ) return _ret_chunk ;
			 }
			 else
       {
          var _ret_cmd = circles_lib_process_cmd( _cmd_tag, _params_str, _out_channel, TERMINAL_CMD_MODE_ACTIVE );
          if ( _out_channel == OUTPUT_FUNCTION ) return _ret_cmd ;
       }
    }
}
...
function circles_lib_terminal_load_cmd( _expression, _params_str, _out_channel, _cmd_mode, _caller_id )
{
    ...
    if ( _glob_code_run_cmds_array.includes( _cmd_tag ) )
	  return circles_lib_process_cmd( _cmd_tag, _params_str, _out_channel, _cmd_mode, _caller_id );
...
function circles_lib_process_cmd( _cmd_tag, _params_str, _out_channel, _cmd_mode, _caller_id )
{
    ...
    {
    	 var _full_cmd = _sub_fn + "( '"+_params_str+"', "+_out_channel+", '', "+_cmd_mode+", '"+_caller_id+"' )" ;
       var _out = "" ;
