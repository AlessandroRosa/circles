...
function circles_terminal_cmd_auto()
{
     ...
     if ( _glob_verbose ) circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<slategray>cmd '"+_cmd_tag+"' running in "+( _cmd_mode == TERMINAL_CMD_MODE_ACTIVE ? "active" : "passive" )+" mode</slategray>", _par_1, _cmd_tag );
     ...
     else if ( _params.length > 0 )
     {
         ...
         // pre-scan for levenshtein correction
    		 var _local_cmds_params_array = [];
    				 _local_cmds_params_array.push( "all", "init", "wplane", "off", "on", "refresh", "zplane", "help", "html" );
         ...
         else if ( !_b_fail )
         {
             ...
             if ( _action.stricmp( "refresh" ) || _all )
             {
                 ...
                 if ( _glob_target_plane == NO_PLANE && _mode == 1 )
                 {
                      ...
                      circles_lib_output( _out_channel, DISPATCH_WARNING, "At least one plane shall be chosen before setting autorefresh on", _par_1, _cmd_tag );
                 }
             ...
             if ( _all || _action.stricmp( "refresh" ) )
             {
                 ...
                 if ( _mode )
                 {
                     ...
                     circles_lib_terminal_interpreter( "refresh "+_tmp_array.join( " " ), _glob_terminal, _out_channel );
                 }
             }
         }
     }
