            ...
            else
            {
                switch( _action )
                {
                   ...
                   if ( _coords_rect != null && _display_rect != null )
                   {
                      ...
                      circles_lib_output( _out_channel, DISPATCH_SUCCESS, "Coords acquired from " + circles_lib_plane_get_def( _glob_bip_original_plane_coords ) + " with success", _par_1, _cmd_tag );

                      var _ret_chunk = circles_lib_terminal_bip_apply( _out_channel, _tmp_applied_settings_mask, _par_1, _cmd_tag );
                   ...
                   if ( is_html_canvas( _glob_bip_canvas ) )
                   {
                      ...
                      var _ret_chunk = circles_lib_canvas_clean( _glob_bip_canvas, _glob_bip_bk, _out_channel );
                   ...
                   var _ret_msg = is_array( _ret_chunk ) ? new String( _ret_chunk[1] ) : "Rendering: memory failure" ;
                       _ret_msg = _ret_msg.trim();
                   circles_lib_output( _out_channel, _ret_id ? DISPATCH_SUCCESS : DISPATCH_ERROR, _ret_msg, _par_1, _cmd_tag );
                   if ( _ret_id ) circles_lib_output( _out_channel, DISPATCH_INFO, "Bip session has been closed after rendering", _par_1, _cmd_tag );
                   break ;
                   case "save":
         ...
         else if ( _params_assoc_array['toggle'] != null )
         {
            ...
            switch( _toggle )
            {
                ...
                else
                {
                    ...
                    circles_lib_output( _out_channel, _already ? DISPATCH_INFO : DISPATCH_SUCCESS, "Bip session is "+( _already ? "already " : "" )+( _glob_bip_use ? "open" : "closed" )+"", _par_1, _cmd_tag );
...
function circles_lib_terminal_bip_apply( _out_channel, _par_1 )
{
    var _tmp_applied_settings_mask = 0 ;
