function circles_terminal_cmd_bk()
{
     ...
     else if ( _params.length > 0 )
     {
         ...
         for( var _i = 0 ; _i < _params_array.length ; _i++ )
         {
            ...
            else if ( _p.is_one_of_i( "default", "reset" ) ) _params_assoc_array['bkcolor'] = DEFAULT_BK_COLOR ;
         ...
         else if ( _params_assoc_array['help'] )
         {
              jQuery.get( _glob_terminal_help_path + _cmd_tag + ".cmd.hlp",
                          function( _help_text )
                          {
                             ...
                             _params_assoc_array['html'] ? circles_lib_terminal_color_decode_htmltext( _help_text, _cmd_tag ) : circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, _help_text, _par_1, _cmd_tag );
                          },
                          'html');
         }
         else if ( !_b_fail )
         {
             ...
             switch( _action )
             {
                ...
                break ;
                default:
                $("#terminal_div").css( "background-color", _params_assoc_array['bkcolor'] );
                $("#terminal_div").css( "opacity", DEFAULT_OPACITY );
                $("#terminal_div").css( "-moz-opacity", DEFAULT_OPACITY );
                $("#terminal_div").css( "filter", "progid:DXImageTransform.Microsoft.Alpha(Opacity="+( DEFAULT_OPACITY * 100 )+")" );
                $("#terminal_div").css( "-ms-filter", "alpha(opacity="+( DEFAULT_OPACITY * 100 )+")" );
                break ;
             }
         }
     }
