function circles_terminal_cmd_benchmark()
{
     ...
     else if ( _params.length > 0 )
     {
         ...
         _params_assoc_array['status'] = OFF ;
         ...
         for( var _i = 0 ; _i < _up_to_index ; _i++ )
         {
            ...
            else if ( _p.stricmp( "on" ) ) _params_assoc_array['status'] = ON ;
            else if ( _p.stricmp( "off" ) ) _params_assoc_array['status'] = OFF ;
            else
            {
     ...
     else if ( !_b_fail )
     {
         ...
         switch( _action )
         {
             ...
             var _msg = "Process duration : " + _diff + " second" + ( _diff == 1 ? "" : "s" );
             circles_lib_output( _output_channel, DISPATCH_INFO, _msg, _par_1, _cmd_tag );     
             if ( _params_assoc_array['dump'] )
             {
    					  _params_assoc_array['dump_array'] = is_array( _params_assoc_array['dump_array'] ) ? _params_assoc_array['dump_array'][0] : "circles.benchmark.txt" ;
