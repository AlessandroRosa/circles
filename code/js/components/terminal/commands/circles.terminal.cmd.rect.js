function circles_terminal_cmd_rect()
{
     ...
     if ( _params.length > 0 )
     {
        ...
        _params_array.clean_from( " " );       _params_array.clean_from( "" );
         ...
         {
    				 for( var _d = _dump_operator_index + 1 ; _d < _params_array.length ; _d++ )
    				 if ( _params_array[_d].trim().length > 0 ) _params_assoc_array['dump_array'].push( _params_array[_d] );
         }
     ...
     // elaborating the params
          ...
          if ( _params_assoc_array['settings']['width'] != null && _params_assoc_array['settings']['height'] == null )
          _params_assoc_array['settings']['height'] = _params_assoc_array['settings']['width'] ;
          
          var _rect_obj = null ;
          // switch to different constructors, according to input data
          if ( _params_assoc_array['settings']['start_pt'] != null && _params_assoc_array['settings']['end_pt'] != null )
          {
              _rect_obj = new rect( _params_assoc_array['settings']['start_pt'].x, _params_assoc_array['settings']['start_pt'].y,
                                    _params_assoc_array['settings']['end_pt'].x, _params_assoc_array['settings']['end_pt'].y, _RECT_ORIENTATION_CARTESIAN );
              _rect_obj.correct( _RECT_ORIENTATION_CARTESIAN );
          }
          else if ( _params_assoc_array['settings']['start_pt'] != null &&
                    _params_assoc_array['settings']['end_pt'] == null &&
                    _params_assoc_array['settings']['width'] != null &&
                    _params_assoc_array['settings']['height'] != null )
          {
              _rect_obj = new rect();
              _rect_obj.width_height_constructor( _params_assoc_array['settings']['start_pt'].x, _params_assoc_array['settings']['start_pt'].y, _params_assoc_array['settings']['width'], _params_assoc_array['settings']['height'], _RECT_ORIENTATION_CARTESIAN );
              _rect_obj.correct( _RECT_ORIENTATION_CARTESIAN );
          }
          ...
          if ( is_rect( _rect_obj ) )
          {
              ...
              circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<snow>(" + circles_lib_plane_get_def( _params_assoc_array['settings']['plane'] ) + ")</snow> <green>Rect processed with success</green>", _par_1, _cmd_tag );
              if ( _params_assoc_array['settings']['rec'] == YES || _storage_queue_request )
              {
                   ...
                   if ( _storage_queue_request )
                   {
                       ...
                       if ( is_array( _glob_storage[_subset] ) )
                       {
                           ...
                           var _msg = "<green>Rect "+( _rec_chunk['label'].length > 0 ? "'"+_rec_chunk['label']+"'" : "" )+" has been copied into data storage space</green>" ;
                           circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, _msg, _par_1, _cmd_tag );
                       }
                   ...
                   circles_lib_output( _out_channel, DISPATCH_INFO, "Rect recorded", _par_1, _cmd_tag );
              }
          }
          else
          {
