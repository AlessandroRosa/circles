_glob_terminal_cmd_files_include[ "canvas" ] = [ "keepcmd" ] ;

function circles_terminal_cmd_canvas()
{
     ...
     else if ( _params.length > 0 )
     {
         ...
         _params_assoc_array['plane'] = _glob_target_plane != NO_PLANE ? _glob_target_plane : Z_PLANE ;
         ...
         for( var _i = 0 ; _i < _up_to_index ; _i++ )
         {
              ...
              else if ( _p.testME( _glob_positive_integer_regex_pattern ) )
              {
                   if ( _params_assoc_array['w'] == null ) _params_assoc_array['w'] = safe_int( _p, 0 );
                                        ...
                                        "redirect", "release", "resize", "split", "thumbnail", "update" ) )
              {
              ...
              {
                  if ( _params_assoc_array['fromplane'] == null )
                  {
                      if ( _p.stricmp( "zplane" ) ) _params_assoc_array['fromplane'] = Z_PLANE ;
                      else if ( _p.stricmp( "wplane" ) ) _params_assoc_array['fromplane'] = W_PLANE ;
                      _index_associations['fromplane'] = _i ;
                  }
                  else if ( _i == _index_associations[ "fromplane" ] + 1 )
                  {
                      _params_assoc_array['fromactivity'] = _p ;
                      _index_associations['fromactivity'] = _i ;
                  }
                  else if ( _i == _index_associations[ "to" ] + 1 )
                  {
                      if ( _p.stricmp( "zplane" ) ) _params_assoc_array['toplane'] = Z_PLANE ;
                      else if ( _p.stricmp( "wplane" ) ) _params_assoc_array['toplane'] = W_PLANE ;
                      else if ( _p.stricmp( "bip" ) ) _params_assoc_array['toplane'] = BIP_BOX ;
                      _index_associations['toplane'] = _i ;
                  }
                  else if ( _i == _index_associations[ "toplane" ] + 1 )
                  {
                      _params_assoc_array['tolayer'] = _p ;
                      _index_associations['tolayer'] = _i ;
                  }
              }
     ...
     else if ( !_b_fail )
     {
           ...
           switch( _action )
           {
                ...
                var _default_pile = circles_lib_canvas_layer_pile_get_default( _plane_type );
                ...
                {
                     _b_fail = YES ;
                     _error_str = "Invalid source / destination copy specifications" ;
                }
                _glob_storage[_cmd_tag]['open'] = PENDING ;
                break ;
                case "create":
                ...
                else if ( _action.strcmp( "update" ) ) _required_n_params = 4 ; // 2 canvas ref + 2 params values
                ...
                if ( _params.length == _required_n_params )
                {
                     ...
                     else if ( _action.strcmp( "create" ) )
                     {
                          ...
                          {
                              _b_fail = YES ;
                              _error_str = "The layer '"+_action_params_array['roledef']+"' already exists in the "+_action_params_array['plane']+" list" ;
                          }
                     }
                     else if ( _action.strcmp( "delete" ) )
                     {
                          ...
                          else
                          {
                               ...
                               var _delete_layer = function()
                               {
                                   ...
                                   else if ( _ret == RET_UNAUTHORIZED )
                                   {
                                       _b_fail = YES ;
                                       _error_str = "Deletion aborted: layer '"+_action_params_array['roledef']+"' is a default item and cannot be deleted" ;
                                   }
                                   else
                                   {
                     ...
                     else if ( _action.strcmp( "update" ) )
                     {
                          ...
                          {
                              _b_fail = YES ;
                              _error_str = "Update aborted: incorrect plane reference" ;
                          }
                          else if ( !_b_fail )
                          {
                              ...
                              {
                                  _b_fail = YES ;
                                  _error_str = "Update aborted: missing or invalid param" ;
                              }
                              else
                              {
                ...
                {
                     _b_fail = YES ;
                     _error_str = "Input failure: "+_required_n_params+" params are required for '"+_action+"' action" ;
                }
                break ;
                case "merge":
                var _plane = NO_PLANE ;
                ...
                if ( _params_assoc_array['plane'].strcmp( "zplane" ) ) _plane = Z_PLANE ;
                else if ( _params_assoc_array['plane'].strcmp( "wplane" ) ) _plane = W_PLANE ;
                ...
                {
                     _b_fail = YES ;
                     _error_str = "Can't display thumbnail: incorrect canvas reference" ;
                }
                else
                {
                ...
                {
                     var _layers_list = circles_lib_canvas_layer_pile_get_per_plane( _plane_type );
                     if ( safe_size( _layers_list, 0 ) > 0 )
                     {
                          var _layer = null, _canvas = null ;
                          ...
                          _output_row += ( new String( "LEVEL" ) ).rpad( " ", 6 );
                          _output_row += ( new String( "ROLE" ) ).rpad( " ", 15 );
                          ...
                          for( var _i = 0 ; _i < _layers_list.length ; _i++ )
                          {
                              ...
                              if ( _layer != null )
                              {
                                   ...
                                   _output_row  = "<yellow>"+( ""+_id ).rpad( " ", 15 )+"</yellow>" ;
                                   _output_row += _layer.is_defaultcanvas() ? "<green>"+("yes").rpad( " ", 8 )+"</green>" : "<red>"+("no").rpad( " ", 8 )+"</red>" ;
                                   _output_row += ( "" + _z_index ).rpad( " ", 6 );
                                   _output_row += ( "" + _role_def ).rpad( " ", 15 );
                                   _output_row += ( ( _opacity * 100.0 ) + "%").rpad( " ", 8 );
                     ...
                     {
                         _b_fail = YES ;
                         _error_str = "No list returned: incoherent input plane type" ;
                     }
                }
                else
                {
                     _b_fail = YES ;
                     _error_str = "No list returned: missing input plane type" ;
                }
                break ;
                case "redirect":
                ...
                if ( is_html_canvas( _candidate_to_canvas ) )
                {
                     ...
                     switch( _params_assoc_array['fromplane'] )
                     {
                         case Z_PLANE:
                         if ( _from_activity.stricmp( "grid" ) ) _glob_zplane_grid_canvas_placeholder = _candidate_to_canvas ;
                         else if ( _from_activity.stricmp( "rendering" ) ) _glob_zplane_rendering_canvas_placeholder = _candidate_to_canvas ;
                         else if ( _from_activity.stricmp( "freedraw" ) ) _glob_zplane_freedraw_canvas_placeholder = _candidate_to_canvas ;
                         else if ( _from_activity.stricmp( "work" ) ) _glob_zplane_work_canvas_placeholder = _candidate_to_canvas ;
                         break ;
                         case W_PLANE:
                         if ( _from_activity.stricmp( "grid" ) ) _glob_wplane_grid_canvas_placeholder = _candidate_to_canvas ;
                         else if ( _from_activity.stricmp( "rendering" ) ) _glob_wplane_rendering_canvas_placeholder = _candidate_to_canvas ;
                         else if ( _from_activity.stricmp( "freedraw" ) ) _glob_wplane_freedraw_canvas_placeholder = _candidate_to_canvas ;
                         else if ( _from_activity.stricmp( "work" ) ) _glob_wplane_work_canvas_placeholder = _candidate_to_canvas ;
                         break ;
                         default:
                         _b_fail = YES ;
                         _error_str = "Redirection assignment failure: inconsistent plane reference" ;
                         break ;
                     }
                ...
                {
                     _b_fail = YES ;
                     _error_str = "Inconsistent canvas references:" ;
                     _error_str += "\nmissing " + circles_lib_plane_get_def( _params_assoc_array['toplane'] ) + " " + _params_assoc_array['tolayer'] ;
                }
                break ;
                case "default":
                ...
                break ;
                case "info":
                var _zplane_grid_label = ( _glob_zplane_grid_canvas_placeholder != null ) ? _glob_zplane_grid_canvas_placeholder.getContext( _glob_canvas_ctx_2D_mode ).label : "unknown" ;
                var _zplane_rendering_label = _glob_zplane_rendering_canvas_placeholder != null ? _glob_zplane_rendering_canvas_placeholder.getContext( _glob_canvas_ctx_2D_mode ).label : "unknown" ;
                var _zplane_freedraw_label = ( _glob_zplane_freedraw_canvas_placeholder != null ) ? _glob_zplane_freedraw_canvas_placeholder.getContext( _glob_canvas_ctx_2D_mode ).label : "unknown" ;
                var _zplane_work_label = ( _glob_zplane_work_canvas_placeholder != null ) ? _glob_zplane_work_canvas_placeholder.getContext( _glob_canvas_ctx_2D_mode ).label : "unknown" ;

                var _wplane_grid_label = ( _glob_wplane_grid_canvas_placeholder != null ) ? _glob_wplane_grid_canvas_placeholder.getContext( _glob_canvas_ctx_2D_mode ).label : "unknown" ;
                var _wplane_rendering_label = _glob_wplane_rendering_canvas_placeholder != null ? _glob_wplane_rendering_canvas_placeholder.getContext( _glob_canvas_ctx_2D_mode ).label : "unknown" ;
                var _wplane_freedraw_label = ( _glob_wplane_freedraw_canvas_placeholder != null ) ? _glob_wplane_freedraw_canvas_placeholder.getContext( _glob_canvas_ctx_2D_mode ).label : "unknown" ;
                var _wplane_work_label = ( _glob_wplane_work_canvas_placeholder != null ) ? _glob_wplane_work_canvas_placeholder.getContext( _glob_canvas_ctx_2D_mode ).label : "unknown" ;
                ...
                {
                    _b_fail = YES ;
                    _error_str = "Missing action param" ;
                }
                break ;
           }
     }
