function circles_terminal_cmd_alphabet()
{
     ...
     else if ( !_b_fail )
     {
        ...
        if ( safe_size( _action, 0 ) == 0 )
        {
           ...
           circles_lib_output( _output_channel, DISPATCH_INFO, "Missing input action: alphabet will be shown below", _par_1, _cmd_tag );
           _action = "show" ;
        }
        ...
        switch( _action )
        {
              ...
              if ( _params_assoc_array['settings'].includes( "inv" ) )
                 ...
                 if ( _symbol.length > 0 )
		           ...
		           circles_lib_output( _output_channel, DISPATCH_INFO, "Alphabet includes "+_size+" element"+(_size==1?"":"s"), _par_1, _cmd_tag );
					 }
									...
									if ( _symbol.length > 0 )
									{
										 if( _glob_alphabet.includes( _symbol ) )
										 {
												 ...
												 else
												 {
														 ...
														 circles_lib_output( _output_channel, _out_id ? DISPATCH_SUCCESS : DISPATCH_ERROR,
           ...
           if ( _n_sd > 0 )
           {
              ...
              circles_lib_output( _output_channel, DISPATCH_INFO, "Erasing current alphabet", _par_1, _cmd_tag );
              _glob_alphabet = [];
              circles_lib_output( _output_channel, DISPATCH_INFO, "Now checking "+_n_sd+" registered seed"+(_n_sd==1?"":"s")+" for alphabet regeneration", _par_1, _cmd_tag );
           ...
           if ( safe_size( _glob_alphabet, 0 ) > 0 )
           circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, "Alphabet is <lightblue>" + _glob_alphabet.join( ", " ) + "</lightblue>", _par_1, _cmd_tag );
           else circles_lib_output( _output_channel, DISPATCH_INFO, "Alphabet is empty", _par_1, _cmd_tag );
           _fn_ret_val = _glob_alphabet.clone() ;
           break ;
	         default: break ;
				}
