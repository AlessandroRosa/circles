function circles_terminal_cmd_silent()
{
     ...
     if ( _params.length > 0 )
     {
         ...
         else if ( !_b_fail )
         {
             ...
             var _already = ( _glob_terminal_silent == _params_assoc_array['mode'] ) ? YES : NO ;
             switch( _action )
             {
                 case "release":
                 ...
                 break ;
                 default:
                 _glob_terminal_silent = _params_assoc_array['mode'] ;
                 var _silent_label = "" ;
                 switch( _glob_terminal_silent )
                 {
                    case YES:
                    _silent_label = "silent mode is "+( _already ? "already " : "" )+"on" + ( _already ? "" : ": questions will be skipped" );
                    break ;
                    default:
                    _silent_label = "silent mode is "+( _already ? "already " : "" )+"off" + ( _already ? "" : ": questions won't be skipped" );
                    break ;
                 }
                     
                 if ( _params.length == 0 ) _silent_label = "Current " + _silent_label.toLowerCase();
                 if ( _params.length > 0 )
                 circles_lib_output( _output_channel, _already ? DISPATCH_INFO : DISPATCH_SUCCESS, _silent_label, _par_1, _cmd_tag );
                 else 
                 circles_lib_output( _output_channel, DISPATCH_INFO, _silent_label, _par_1, _cmd_tag );
