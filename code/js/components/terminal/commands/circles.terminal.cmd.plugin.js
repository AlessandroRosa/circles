             ...
             switch( _action )
             {
                 ...
                 else
                 {
                         ...
                         if ( _result.includes( "@@" ) ) _result = _result.split( "@@" );
							       ...
							       {
												 circles_lib_output( _output_channel, DISPATCH_WARNING, "No available plug-ins for subset '"+_subset+"'", _par_1, _cmd_tag );
										 		 break ;
										 }
                 ...
                 if ( _plugin_tmp_vars_array['plugin_sel'] != null )
                 {
                      ...
                      if ( _fam.length > 0 && _def.length > 0 )
                      {
                          ...
                          circles_lib_output( _output_channel, _return_open ? DISPATCH_SUCCESS : DISPATCH_ERROR, ( _return_open ? "Success" : "Failure" )+" in opening "+_fam+" "+_def, _par_1, _cmd_tag );

                          if ( !_return_open )
                          circles_lib_output( _output_channel, DISPATCH_WARNING, "Check whether input '"+_def+"' could have been mispelled or '"+_fam+"' is not the correct category, or mandatory params are missing", _par_1, _cmd_tag );
                          else
                          {
                            if ( _json != null )
                            {
                      ...
                      else circles_lib_output( _output_channel, DISPATCH_ERROR, "Cannot open the mask: please, set a Plug-in first", _par_1, _cmd_tag );
                 }
                 else circles_lib_output( _output_channel, DISPATCH_ERROR, "Cannot open current Plug-in mask: please, set it first", _par_1, _cmd_tag );
                 break ;
                 case "release":
                 ...
                 if ( _plugin_tmp_vars_array['plugin_sel'] != null )
                 {
                    ...
                    var jqxhr = $.get( _path, function() {}).done(function( _data ) {
                        ...
                        if ( _data.length > 0 )
                        {
                          circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, "Remote control messages for Plug-in <white>" + _json.fam + " / " + _json.def + "</white>", _par_1, _cmd_tag );
                          for( var _i = 0 ; _i < _data.length ; _i++ )
                          {
                            if ( _data[_i].includes( "=" ) )
                            {
                              ...
                              if ( _data[_i].length == 2 )
                              {
                                 ...
                                 circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, "<white>"+_data[_i][0]+"</white> <lightblue>"+_data[_i][1]+"</lightblue>", _par_1, _cmd_tag );
                              }
                            }
                          }
                        }
                        else circles_lib_output( _output_channel, DISPATCH_ERROR, "Cannot get current Plug-in remote control lists: missing file remotecmds.info", _par_1, _cmd_tag );
                      })
                      .fail(function() {
                        circles_lib_output( _output_channel, DISPATCH_ERROR, "Cannot get current Plug-in remote control lists: missing file remotecmds.info", _par_1, _cmd_tag );
                      })
                 }
                 else circles_lib_output( _output_channel, DISPATCH_ERROR, "Cannot get current Plug-in remote control lists: please, set it first", _par_1, _cmd_tag );
                 break ;
                 case "send":
                 if ( _plugin_tmp_vars_array['plugin_sel'] != null )
                 {
                    ...
                    else circles_lib_output( _output_channel, DISPATCH_ERROR, "Missing params specification after 'send' action", _par_1, _cmd_tag );
                 }
                 ...
                 if ( _plugin_tmp_vars_array['plugin_sel'] != null )
                 {
                      ...
                      if ( _src != null )
                      {
                          ...
                          {
                             circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, _output, _par_1, _cmd_tag );
                          }
                      }
                      else circles_lib_output( _output_channel, DISPATCH_ERROR, "Please, use 'set' action to fix the working Plug-in first or cmds wouldn't be accepted", _par_1, _cmd_tag );
                 }
                 ...
                 if ( _plugin_tmp_vars_array['plugin_sel'] != null )
                 {
                      ...
                      if ( _var_set.includes( "=" ) )
                      {
                        ...
                        if ( _var_set[0].length > 0 && _var_set[1].length > 0 )
                        {
                          ...
                          circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, "Plugin <white>" + _plugin_sel.fam + " " + _plugin_sel.def + "</white> : var <white>" + _var_set[0] + "</white> "+(_b_ok?"correctly":"not")+" set to <white>" + _var_set[1] + "</white>", _par_1, _cmd_tag );
                        }
                        else
                        {
                          if ( _var_set[0].length == 0 )
                          circles_lib_output( _output_channel, DISPATCH_ERROR, "Missing <var-id> specification", _par_1, _cmd_tag );
                          if ( _var_set[1].length == 0 )
                          circles_lib_output( _output_channel, DISPATCH_ERROR, "Missing <var-value> specification", _par_1, _cmd_tag );
                        }
                      }
                      else circles_lib_output( _output_channel, DISPATCH_ERROR, "var setting shall be in the form <var-id>=<var-value>", _par_1, _cmd_tag );
                 }
                 else circles_lib_output( _output_channel, DISPATCH_ERROR, "Please, use 'set' action to fix the working Plug-in first or cmds wouldn't be accepted", _par_1, _cmd_tag );
                 break ;
                 default: break ;
