...
function getViewportExtents() { return new Array( $(window).width(), $(window).height() ); }
function getHTMLpageExtents() { return new Array( $(document).width(), $(document).height() ); }
...
function move_div( _div_id, POS_X_STR, POS_Y_STR, DIVw, DIVh, _animate )
{
    ...
    if ( DIV != null )
    {
        ...
        else if ( POS_Y_STR == "middle" ) Y = safe_int( ( PAGEheight - divHEIGHT ) / 2, 0 ) - divHEIGHT / 2 ;
...
function setOpacity( _obj_id, val ) // val input is a float 0 <= val <= 1
{
    val = parseFloat( val ) ;
...
function COVERdiv( bSHOW, zINDEX )
{
    ...
    if ( $( "#DIALOGcoverDIV" ).get(0) != null )
    {
        ...
        setOpacity( "DIALOGcoverDIV", 0.74 ) ;
...
function SPLASH( HTMLcode, bSHOW, SPLASHwidth, SPLASHheight, zINDEX, SPLASHtop )
{
    ...
    var DIALOGMODALdiv = getOBJECTbyID( "DIALOGMODALdiv" ) ;
    var DIALOGMODALdivCONTAINER = getOBJECTbyID( "DIALOGMODALdivCONTENTS" ) ;
...
function SPLASHanimated( HTMLcode, bSHOW, SPLASHwidth, SPLASHheight, zINDEX, SPLASHtop )
{
      ...
      ( bSHOW ) ? $("#DIALOGMODALdiv").fadeIn("slow") : $("#DIALOGMODALdiv").fadeOut("slow") ;
