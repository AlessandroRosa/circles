...
function CIRCLESembeddingsGENERALPURPOSE_VAR_DECLARE( _question, _silent )
{
		_question = safe_int( _question, YES ), _silent = safe_int( _silent, NO );
    var _var_id = $("#PLUGINvaridEDIT").val(), _varvalue = $("#PLUGINvarvalueEDIT").val();
    if ( _var_id.length == 0 || _varvalue.length == 0 )
    {
        var _msg = "Can't set on the var params:" + _glob_crlf ;
        if ( _var_id.length == 0 ) _msg += _glob_crlf + "- missing var name;" ;
        if ( _varvalue.length == 0 ) _msg += _glob_crlf + "- missing var value;" ;
        if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, _msg, _glob_app + " - " + _plugin_definitions_array[_plugin_last_ref] );
        return NO ;
    }
    else
    {
        ...
        else if ( _varvalue.includes( _var_id ) )
        {
             var _msg = "Detected cyclic reference of var"+_glob_crlf+_glob_crlf+" '"+_var_id+"'"+_glob_crlf+_glob_crlf+"inside input value"+_glob_crlf+_glob_crlf+"'"+_varvalue+"'" ;
             if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, _msg, _glob_app + " - " + _plugin_definitions_array[_plugin_last_ref] );
             return NO ;
        }
        ...
        var _v_complex = circles_lib_math_parse_formula( _varvalue );
        _v_complex = parse_complex_from_string( _v_complex + "" );

        if ( !is_complex( _v_complex ) )
        {
            if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_ERROR, "The input var is not a complex formula.", _glob_app + " - " + _plugin_definitions_array[_plugin_last_ref] );
            return NO ;
        }

        _plugin_user_vars[ ""+_var_id ] = _varvalue ;
...
function CIRCLESembeddingsGENERALPURPOSE_VAR_LIST()
{
    var _keys = _plugin_user_vars.keys_associative();
    var _n_keys = safe_size( _keys, 0 ), _html_code = "" ;
    if ( _n_keys > 0 )
    {
        _html_code += "<table>" ;
        ...
        _html_code += "</table>" ;
        circles_lib_output( OUTPUT_SCREEN, DISPATCH_INFO, _html_code, _glob_app + " - Vars list" );
    }
    else
    {
        _html_code += "<table><tr><td ALIGN=\"center\">The vars list is empty</td></tr></table>" ;
        circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, _html_code, _glob_app + " - " + _plugin_definitions_array[_plugin_last_ref] );
    }
}

function CIRCLESembeddingsGENERALPURPOSE_VAR_CHANGE() { CIRCLESembeddingsGENERALPURPOSE_VAR_DECLARE(); }

function CIRCLESembeddingsGENERALPURPOSE_VAR_VAL_REGISTER()
{
    var _var_id = $("#PLUGINvaridEDIT").val(), _varvalue = $("#PLUGINvarvalueEDIT").val();
    if ( safe_string( _var_id, "" ).length > 0 && safe_string( _varvalue, "" ).length > 0 )
    {
        var _v_complex = circles_lib_math_parse_formula( _varvalue );
        _v_complex = parse_complex_from_string( _v_complex + "" );
        ...
        else
        {
            ...
            if ( !_plugin_rec_var_vals[''+_var_id].includes( _varvalue ) )
            _plugin_rec_var_vals[''+_var_id].push( _varvalue );
            else
            {
    ...
    else circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, "Missing or incomplete input vars data.", _glob_app );
    
}

function CIRCLESembeddingsGENERALPURPOSE_VAR_VAL_DELETE( _question, _silent )
{
    _question = safe_int( _question, YES ), _silent = safe_int( _silent, NO );
    ...
    if ( safe_size( _ENTRY, 0 ) > 0 )
    {
       if ( _ENTRY.includes( ":" ) && _ENTRY.count( ":" ) == 1 )
       {
          ...
          if ( _b_go )
          {
             ...
             {
                if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, "The value '"+_val+"' of var '"+_var+"' has been removed with success.", _glob_app );
                var _combo_code = CIRCLESembeddingsGENERALPURPOSE_VAR_VALS_RECORD_COMBO_BUILD();
                $("#PLUGINrecordedvaluesCOMBOcontainer").html( _combo_code );
                $("#PLUGINvaridEDIT").val( "" );
                $("#PLUGINvarvalueEDIT").val( "" );
             }
             else if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, "Problems while removing the value '"+_val+"' of var '"+_var+"'.", _glob_app );
          }
       }
    }
    else if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, "Please, select one entry to be removed.", _glob_app );
}

function CIRCLESembeddingsGENERALPURPOSE_VAR_BOMB( _question, _silent )
{
    if ( _question ? confirm( "Confirm to delete all vars permanently ?" ) : YES )
    {
			 ...
			 if ( safe_size( _keys, 0 ) > 0 )
			 {
			   ...
			   if ( !_silent )
			   {
						circles_lib_output( OUTPUT_SCREEN, safe_size( _keys, 0 ) == 0 ? DISPATCH_SUCCESS : DISPATCH_ERROR,
											 safe_size( _keys, 0 ) == 0 ? "All user-defined vars have been deleted" : "Problems while deleting user-defined vars",
											 _glob_app );				 
				 }
			}
			else if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, "No used-defined vars declared yet", _glob_app + " - " + _plugin_definitions_array[_plugin_last_ref] );
    }
}
...
function CIRCLESembeddingsGENERALPURPOSE_VAR_DELETE( _question, _silent )
{
    ...
    else
    {
        if ( _question ? confirm( "Confirm to delete var '"+_var_id+"' ?" ) : YES )
        {
            ...
            var _msg = _deleted ? "Var '"+_var_id+"' deleted with success" : "Fail to delete var '"+_var_id+"'" ;
            if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, _deleted ? DISPATCH_SUCCESS : DISPATCH_ERROR, _msg, _glob_app + " - " + _plugin_definitions_array[_plugin_last_ref] );
            if ( _deleted ) CIRCLESembeddingsGENERALPURPOSE_VAR_REFRESH_PANEL();
            return [ _deleted ? RET_OK : RET_ERROR, _msg ] ;
        }
        else return [ RET_MSG_ID_IRRELEVANT, "Deletion aborted by user" ] ;
    }
}
...
function CIRCLESembeddingsGENERALPURPOSE_SET_USERVARS_IN_PARAMS( _edit_acquisition, _gen_chunk )
{
    ...
    if ( _gen_chunk != null )
    {
         if ( _gen_chunk.length != 4 ) return -1 ;
    }
    ...
    if ( CIRCLESembeddingsGENERALPURPOSEresolved_mm_params_array.length != 4 ) return -1 ;
    else
    {
...
function CIRCLESembeddingsGENERALPURPOSE_VAR_COMPUTE( _param_id )
{
    ...
    else
    {
        ...
        circles_lib_output( OUTPUT_SCREEN, DISPATCH_INFO, _html_code, _glob_app + " - Compute param '"+_param_id+"'" );
    }
}

function CIRCLESembeddingsGENERALPURPOSE_VAR_WATCH( _param_id )
{
    ...
    else if ( CIRCLESembeddingsGENERALPURPOSE_VAR_IS_INCLUDED( _param_formula ) )
    {
       ...
       _html_code += "</table>" ;
       circles_lib_output( OUTPUT_SCREEN, _n_extracted == 0 ? DISPATCH_WARNING : DISPATCH_INFO, _html_code, _glob_app + " - Extracted vars from param '"+_param_id+"'" );
    }
    else circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, "Param '"+_param_id+"' does not include any user-defined var.", _glob_app + " - " + _plugin_definitions_array[_plugin_last_ref] );
}
...
function CIRCLESembeddingsGENERALPURPOSE_VAR_VALS_RECORD_COMBO_BUILD()
{
    if ( _plugin_rec_var_vals.is_associative() )
    {
        ...
        if ( _n_vals > 0 )
        {
            ...
            {
               $.each( _plugin_rec_var_vals[ ""+_keys[_k] ],
                       function( _i, _item ) { _html_code += "<OPTION VALUE=\""+_keys[_k]+":"+_item+"\">" + _keys[_k] + ":" + _item ; }
                     );                
            }
...
function CIRCLESembeddingsGENERALPURPOSE_VAR_HELP( _section )
{
    _section = safe_int( _section, 1 );
    var _msg = "" ;
    switch( _section )
    {
        case 1: // vars info
    ...
    circles_lib_output( OUTPUT_SCREEN, DISPATCH_INFO, _msg, _glob_app + " - " + _plugin_definitions_array[_plugin_last_ref] );
}
...
function CIRCLESembeddingsGENERALPURPOSE_VAR_EXPORT()
{
    ...
    if ( _plugin_user_vars.size_associative > 0 || CIRCLESembeddingsGENERALPURPOSE_gens_container.length > 0 )
    {
       ...
       $.each( CIRCLESembeddingsGENERALPURPOSE_gens_container,
               function( _i, _chunk )
               {
                 _exported_code.push( "plugin var g" + ( _i + 1 ) + "_1=" + _chunk[0] );
                 _exported_code.push( "plugin var g" + ( _i + 1 ) + "_2=" + _chunk[1] );
                 _exported_code.push( "plugin var g" + ( _i + 1 ) + "_3=" + _chunk[2] );
                 _exported_code.push( "plugin var g" + ( _i + 1 ) + "_4=" + _chunk[3] );
               } );
       ...
       if ( _plugin_user_vars.is_associative() )
       {
           ...
           if ( _keys.length > 0 && _values.length > 0 )
           {
               ...
               _exported_code.push( "plugin var v" + ( _i + 1 ) + "=["+_keys[_i]+"@"+_values[_i]+"]" );
           }
       }
...
function CIRCLESembeddingsGENERALPURPOSE_VAR_IMPORT( _one_row_of_code )
{
    ...
    var _i, _cmd_array, _cmd_str, _params_str, _pieces, _vartype, _varvalue, _var_array ;
    for( var _i = 0 ; _i < _tokens.length ; _i++ )
    {
         ...
         else if ( _action.stricmp( "var" ) )
         {
            ...
            {
               _pieces = _tok.split( "=" ), _vartype = _pieces[0].charAt(0), _varvalue = _pieces[1] ;
               if ( _vartype.stricmp( "v" ) ) // it's a user var
               {
                  _var_array = _varvalue.replaceAll( [ "[", "]" ], "").split( "@" );
               ...
               {
                  _plugin_import_chunk.push( _varvalue );
