function _canvas_draw_circle( _canvas, _center_x, _center_y, _radius, _draw, _drawcolor, _fill, _fillcolor, _line_thickness, _opacity )
{
     ...
     if ( !object_exists( _drawcolor ) ) _drawcolor = "" ;
     ...
     _context.strokeStyle = ( _drawcolor.length > 0 ) ? _drawcolor : "black" ;
     if ( _draw )
     {
...
function _canvas_polyline( _canvas, POINTSarray, _drawcolor, _line_thickness, _close, _opacity )
{
     ...
     if ( _p_length > 0 )
     {
         ...
         for( var i = 1 ; i < _p_length ; i++ )
         {
             ...
             _canvas_draw_line( _canvas, FROM_PT.x, FROM_PT.y, TO_PT.x, TO_PT.y, _drawcolor, _line_thickness, _opacity ) ;
         }
...
function _canvas_draw_point( _canvas, _x, _y, _draw, _drawcolor, _fill, _fillcolor, _line_thickness, radius, _opacity )
{
     ...
     if ( _drawcolor.length == 0 ) _drawcolor = "black" ;
     if ( _drawcolor.length > 0 || _fillcolor.length > 0 )
     {
          _context.globalAlpha = _opacity ;
          _context.beginPath();
          _context.lineWidth = _line_thickness ;
          ...
          if ( _drawcolor.length > 0 ) _context.strokeStyle = _drawcolor ;
