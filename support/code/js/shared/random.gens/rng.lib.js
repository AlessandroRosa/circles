...
function RNG(seed)
{
    if (seed == null) seed = (Math.random() + Date.now()).toString();
    else if (typeof seed === "function")
    {
        // Use it as a uniform number generator
        this.uniform = seed;
        this.nextByte = function() { return ~~(this.uniform() * 256); };
        seed = null;
    }
    else if (Object.prototype.toString.call(seed) !== "[object String]") seed = JSON.stringify(seed);
    this._normal = null;
    if (seed) this._state = new RC4(seed);
 ...
 * return 0 <= x <= 1
 * Remark: The 'best' multiplier with m = 231 − 1 is a = 950,706,376
   (Fishman and Moore 1986).
*/

RNG.prototype.linear_congruential = function( _seed )
{
   /** Rules:
     * a - 1 should be divisible by m's prime factors
     * c and m should be co-prime
   */
   var _m = 69, _a = 13, _b = 29;
   for( var _i = 0 ; _i < 8 ; _i++ ) _seed = ( _a * _seed + _b ) % _m ;
   return _seed / _m ; // so it ranges in [0,1]
}
