...
<?php require_once( "../fns/fns.php" ) ;
    ...
    $filter = $_POST['filter'] ;
    if ( array_key_exists( "path_to_root", $_POST ) ) $PATH_TO_ROOT = $_POST["path_to_root"] ;
	...
	$showfolders = count( $search_params ) > 3 ? intval( $search_params[3] ) : 0 ; if ( is_nan( $showfolders ) ) $showfolders = 1 ;
      ...
      if ( file_exists( $fullpath ) )
      {
        ...
        {
          $contents = file_exists( $CHUNK['fullpath'] ) && is_file( $CHUNK['fullpath'] ) ? file_get_contents( $CHUNK['fullpath'] ) : "" ;
