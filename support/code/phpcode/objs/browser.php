...
class browser{
    ...
    function browser()
    {
        ...
        {
              $bd['browser'] = "opera" ;
              $tmp = strtolower( $agent );
              $tmp = str_replace( "opera/", "", $tmp );
              $t = explode( "(", $tmp );
              $bd['version'] = trim( $t[0] ) ;  
        }
        else if ( preg_match( "/navigator+/i", $agent ) )
        {
            $bd['browser'] = "netscape" ;
            $tmp = strtolower( $agent );
            $tmp = stristr( $agent, "navigator" ) ;
            $t = explode( "/", $tmp );
            $bd['version'] = trim( $t[1] ) ;  
        }
        ...
        else if ( preg_match( "/msie+/i", $agent ) )
        {
            $bd['browser'] = "msie" ;
            $tmp = strtolower( $agent );
            $tmp = stristr( $agent, "msie" ) ;
            $t = explode( ";", $tmp );
            $bd['version'] = trim( $t[0] ) ;
            $bd['version'] = str_replace( "MSIE", "", $bd['version'] );
            $bd['version'] = trim( $bd['version'] ) ;
        }
        else if ( preg_match( "/chrome+/i", $agent ) ) $bd['browser'] = "chrome" ;
        else if ( preg_match( "/safari+/i", $agent ) ) $bd['browser'] = "safari" ;
...
{ 
      ...
      return false || (preg_match('/ipad/i',$user_agent))             
...
{
    $br = new browser() ;
    return strtolower( $br->Version );
}

function isIE()
{
    $br = new browser() ;
    $name = strtolower( $br->Name );
    return ( strpos( $name, "msie", 0 ) !== false ) ? 1 : 0 ;
}
...
function isFIREFOX()
{
    $br = new browser() ;
    $name = strtolower( $br->Name );
    return ( strpos( $name, "firefox" , 0 ) !== false ) ? 1 : 0 ;
}
...
function isOPERA()
{
    $br = new browser() ;
    $name = strtolower( $br->Name );
    return ( strpos( $name, "opera" , 0 ) !== false ) ? 1 : 0 ;
}
...
function isNETSCAPE()
{
    $br = new browser() ;
    $name = strtolower( $br->Name );
    return ( strpos( $name, "netscape" , 0 ) !== false ) ? 1 : 0 ;
}
...
function isSAFARI()
{
    $br = new browser() ;
    $name = strtolower( $br->Name );
    return ( strpos( $name, "safari" , 0 ) !== false ) ? 1 : 0 ;
}
...
function isKONQUEROR()
{
    $br = new browser() ;
    $name = strtolower( $br->Name );
    return ( strpos( $name, "konqueror" , 0 ) !== false ) ? 1 : 0 ;
}
...
function isCHROME()
{
    $br = new browser() ;
    $name = strtolower( $br->Name );
    return ( strpos( $name, "chrome" , 0 ) !== false ) ? 1 : 0 ;
}
