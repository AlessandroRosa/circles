...
function CIRCLESembeddingsRILEYremotectrl( _options, _return_fn, _ret_array, _output_channel )
{
	...
	switch( _options[0].toLowerCase() )
	{
		...
		break ;
		case "info.params":
        _ret_array.push( YES, "<green>Param identifiers for the Earle plugin: 'rho'</green>" ) ;
		return YES ;
		break ;
		case "init":
        ...
        CIRCLESembeddingsRILEY_CONFIG(); GLOB_PLUGIN_WIZARD_STEP(1.1,YES); GLOB_PLUGIN_GENS_SHOW(YES);
		CIRCLESembeddingsRILEY_RECORD_PARAMS();
		...
		break ;
		case "update.params":
		var _switch_to = 0, _param_id = "", _param_ids = [ "rho" ], _p_sum = 0 ;
