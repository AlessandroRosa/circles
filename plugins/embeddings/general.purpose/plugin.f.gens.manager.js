...
function CIRCLESembeddingsGENERALPURPOSE_GEN_DELETE( _index, _question, _silent )
{
		...
		_index = Math.max( safe_int( _index, UNFOUND ), UNFOUND );
		if ( _index != UNFOUND )
		{
            ...
            var _b_go = !_question ? YES : confirm( "Confirm to delete generator #"+( _index + 1 )+" ?" );
               ...
               var _index_ref = _plugin_last_ref;
				 	 		 var _old_length = safe_size( CIRCLESembeddingsGENERALPURPOSE_gens_container, 0 );
                 ...
                 $("[id$=initBTN]").css( "color", COLOR_DISABLED );
                 $("[id$=renderBTN]").css( "color", COLOR_DISABLED );
								 circles_lib_output( OUTPUT_SPECIAL_FX, DISPATCH_SUCCESS, "Deletion of entry '"+( _index + 1 )+"' has been successful.", 'PLUGIN_OUTMSG' ) ;
							 }
							 else circles_lib_output( OUTPUT_SPECIAL_FX, DISPATCH_WARNING, "Deletion of entry '"+( _index + 1 )+"' has failed.", 'PLUGIN_OUTMSG' ) ;
						}
				}
				else circles_lib_output( OUTPUT_SPECIAL_FX, DISPATCH_ERROR, "Can't delete entry '"+( _index + 1 )+"': invalid referenced data.", 'PLUGIN_OUTMSG' ) ;
		}
		else circles_lib_output( OUTPUT_SPECIAL_FX, DISPATCH_ERROR, "Can't delete entry '"+( _index + 1 )+"': missing reference data.", 'PLUGIN_OUTMSG' ) ;
}

function CIRCLESembeddingsGENERALPURPOSE_GENS_FEATURES( _index )
{
    var _features_html = "" ;
	  GEN_CHUNK = CIRCLESembeddingsGENERALPURPOSE_gens_container[_index] ;
	  if ( GEN_CHUNK != null )
		{
        if ( _GEN_CHECK_MASK == 15 )
        {
            ...
            var _mm = new mobius_map( _A_FORMULA, _B_FORMULA, _C_FORMULA, _D_FORMULA );
        }
        else _features_html = "<SPAN STYLE=\"color:"+DEFAULT_COLOR_ERROR+";\">Input error</SPAN>" ;
		}

    return _features_html ;
}

function CIRCLESembeddingsGENERALPURPOSE_GEN_SELECT( _index, _N_GENS, _src_mask )
{
    ...
    var _input_maps_array = _src_mask == 1 ? CIRCLESembeddingsGENERALPURPOSE_gens_container : _glob_seeds_array ;
  	...
  	if ( GEN_CHUNK != null )
		...
		else circles_lib_output( OUTPUT_SPECIAL_FX, DISPATCH_ERROR, "Selection aborted: missing reference index.", 'PLUGIN_OUTMSG' ) ;
}
...
{
    _resolve_formula = safe_int( _resolve_formula, NO );
    _restore = safe_int( _restore, NO ), _show = safe_int( _show, NO );
...
function CIRCLESembeddingsGENERALPURPOSE_GEN_NORMALIZE( _index )
{
		_index = Math.max( safe_int( _index, UNFOUND ), UNFOUND );
		if ( _index != UNFOUND )
		{
             ...
             else circles_lib_output( OUTPUT_SPECIAL_FX, DISPATCH_ERROR, "Can't find generator at index '"+( _index+1 )+"'", 'PLUGIN_OUTMSG' ) ;
          }
       }
    }
}