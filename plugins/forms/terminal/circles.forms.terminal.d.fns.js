...
function CIRCLESformsTERMINALdebugSAVEFILE( _suffix )
{
    ...
    if ( _div != null )
    {
         ...
         if ( _code.length > 0 )
         {
             var _filename = "listing.debug.txt" ;
             var _basename = basename( _filename );
   			     var _extension = _filename.includes( "." ) ? _filename.split( ".").get_last() : "" ;
								 _filename = _glob_title.length > 0 ? ( _glob_title + "." + _basename + "." +  _extension ) : "circles." + _filename ;
             var blob = new Blob( [ _code ], { type: 'plain/text', endings: 'native' });
...
function CIRCLESformsTERMINALbatchcompilerCLEANall( _question, _silent, _suffix )
{
    _question = safe_int( _question, YES ), _silent = safe_int( _silent, YES );
    var _question = "Confirm to clean everything (code and debug) ?" ;
...
function CIRCLESformsTERMINALbatchcompilerCLEANcode( _question, _silent, _suffix )
{
...
function CIRCLESformsTERMINALbatchcompilerCLEANdebug( _question, _silent, _suffix )
{
    _question = safe_int( _question, YES );
    _silent = safe_int( _silent, NO );
...
function CIRCLESformsTERMINALhistoryDISPLAYtoggle( _suffix, _silent, _out_channel )
{
    ...
    {
         var _h = _glob_terminal.history().data();
             _h.reverse();
