...
function CIRCLESmethodMANAGERrandomtableCOMBOsave()
{
    ...
    if ( _pack.length > 0 && !_glob_rnd_probability_array.includes( _pack ) )
    {
        ...
        var _ret = CIRCLESmethodMANAGERrandomtableCOMBOcreate();
				circles_lib_output( OUTPUT_SPECIAL_FX, _ret ? DISPATCH_SUCCESS : DISPATCH_WARNING, _ret ? "Random IFS table saved with success" : "Fail to save: current random table is empty", 'PROCESSrandomPROBABILITYoutput' ) ;
    }
    else if ( _glob_rnd_probability_array.length == 0 )
		circles_lib_output( OUTPUT_SPECIAL_FX, DISPATCH_WARNING, "Fail to save: current random table is empty", 'PROCESSrandomPROBABILITYoutput' ) ;
}
