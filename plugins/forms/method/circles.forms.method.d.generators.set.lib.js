...
function CIRCLESgenssetMANAGERgensSORT() { _glob_gens_model_array.sort(); CIRCLESgenssetMANAGERreload(); }
       ...
       if ( _probs_n > 0 )
       {
           ...
           for( var _i = 0 ; _i < _probs_n ; _i++ )
           {
              ...
              if ( _HIDDEN_EDIT != null )
              {
                 ...
                 {
                    _glob_gens_model_array.push( _gen_word );
                    _n_restored++ ;
                 }
              }
           }
           ...
           else if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_ERROR, "Fail to restore generators from random table.", _glob_app_title );
       }
       else if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, "Fail to restore: the random table is empty.", _glob_app_title );
   }
}
...
function CIRCLESgenssetMANAGERgensMODELScombo( _question )
{
   ...
   if ( _chunk_index > 0 )
   {
       ...
       if ( _chunk_length > 0 )
       {
           ...
           {
               circles_lib_gens_bomb();
               for( var _i = 1 ; _i < _chunk_length ; _i++ ) _glob_gens_model_array.push( _chunk[_i] );
               CIRCLESgenssetMANAGERreload();
...
function CIRCLESgenssetMANAGERshowGENStable( _silent )
{
   ...
   if ( _set_exists && _glob_method == METHOD_ALGEBRAIC )
   {
      ...
      var _sch_n = circles_lib_gens_model_count(), _is_pqword = NO, _is_pqword_inv = NO, _is_repetend = NO, _type = "" ;
      for( var _i = 0 ; _i < _sch_n ; _i++ )
      {
         ...
         if ( !_glob_gens_model_array.includes( _unsolved_gen_word ) )
...
function CIRCLESgenssetMANAGERgeneratorNEW( _new_word )
{
   ...
   circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, "Fail to add a new entry: the generators set init is still pending.", _glob_app_title );
	 else if ( table != null )
	 {
       if ( circles_lib_gens_model_count() == 0 ) while( table.hasChildNodes() ) table.removeChild(table.firstChild);
	 ...
	 else circles_lib_output( OUTPUT_SCREEN, DISPATCH_CRITICAL, "Fail to add gen: memory failure", _glob_app_title );
}
...
{
   var _pack = circles_lib_gens_model_count() > 0 ? _glob_gens_model_array.join( "@" ) : "" ;
   if ( _pack.length > 0 && !_glob_gens_set_store.includes( _pack ) )
   {
       _glob_gens_set_store.push( _pack );
       CIRCLESgenssetMANAGERgensCOMBOcreate();
   }
   else if ( _glob_gens_set_to_init ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, "Fail to save: generators set shall be init first", _glob_app_title );
   else if ( circles_lib_gens_model_count() == 0 ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, "Fail to save: current generators set is empty", _glob_app_title );
}
...
function CIRCLESgenssetMANAGERgensCOMBOselect()
{
   ...
   if ( _sel_entry.length > 0 && _sel_entry.includes( "@" ) )
   {
      ...
      if ( _unpacked.length > 0 )
      {
         ...
         _glob_gens_model_array = _unpacked.clone();
         CIRCLESgenssetMANAGERreload();
...
function CIRCLESgenssetMANAGERgensSETUP( _question, _silent )
{
   ...
   if ( _entries_n > 0 )
   {
       ...
       {
           _glob_text = "" ;
           var _ret_chunk = circles_lib_gens_build( OUTPUT_TEXT, YES, YES, _question, _silent );
           ...
           if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, _ret_id ? DISPATCH_INFO : DISPATCH_ERROR, _ret_id ? _glob_gens_set_report : _ret_msg, _glob_app_title + " - Construct gens from current set", 520, "auto", null, 230 );
     			 ...
     			 if ( circles_lib_gens_model_count() != circles_lib_count_rnd_probabilities() ) _glob_rnd_probability_array.flush();
   ...
   {
      var _msg = "The generators set is empty."+_glob_crlf+"Fail to build entries" ;
...
function CIRCLESgenssetMANAGERgensEXACT( _question, _silent, _out_channel )
{
   ...
   var _items_n = circles_lib_count_items( _items_array ), _sch_n = circles_lib_gens_model_count();
   if ( _items_n == 0 )
   {
       var _msg = "Fail to build the exact generators set."+_glob_crlf+"The "+( _glob_items_switch == ITEMS_SWITCH_SEEDS ? "Seeds" : "Generators" )+" list is empty" ;
      ...
      if ( _inverse ) _gen = _inverse_generator ;
          ...
          var _b_go = !_question ? YES : confirm( "Confirm to add the "+( _inverse ? "inverse" : "new" )+" gen '"+_gen+"' to the current set ?" );
              ...
              // just a raw string check: no matter if word, pq-word or repetend
							var _b_found = _glob_gens_model_array.includes( _gen );
							...
							else if ( _b_found && !_silent )
              {
                  ...
                  circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, _msg, _glob_app_title );
					   	...
					   	{
							    _glob_gens_model_array.push( _gen );
									if ( !_gen.testME( _glob_pqword_regex_pattern ) )
                  _glob_gens_model_array.push( circles_lib_word_inverse_get( _gen ) );
                  ...
                  $( "#ALGEBRAICnewGENERATORStable" ).html( CIRCLESgenssetMANAGERshowGENStable(YES) );
					...
					break ;
					case -1:
					if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, "Fail to add: the gen symbol is empty.", _glob_app_title );
					break ;
					case -2:
					if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, "Fail to add: the current alphabet is empty and the candidate new gen can't be added to the generators set.", _glob_app_title );
					break ;
			}
	 }
	 else if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_CRITICAL, "Fail to add gen to the generators set: memory failure", _glob_app_title );
}

function CIRCLESgenssetMANAGERgeneratorUPDATEinverse( _gen, _row_index )
{
   // try to recover old value before replacement
   var _old_generator = _glob_gens_model_array[_row_index], _old_inverse_generator = "" ;
   if ( _old_generator != null )
   {
       ...
       var _old_inverse_gen_index = _glob_gens_model_array.indexOf( _old_inverse_generator );
       ...
       if ( _is_pqword || _is_pqword_inv ) _inverse_generator = _gen.end_with( "inv" ) ? _gen.replaceAll( "inv", "" ) : _gen + "inv" ;
	     ...
	     switch( _ret_check )
          ...
          var _b_go = !_question ? YES : confirm( "Confirm to update the new gen '"+_gen+"' ?" );
              ...
              _glob_gens_model_array[_row_index] = _gen ;
              // updates the inverse gen entry
              if ( _old_inverse_gen_index != UNDET )
              _glob_gens_model_array[_old_inverse_gen_index] = _inverse_generator ;
              else if ( !_glob_gens_model_array.includes( _inverse_generator ) )
              _glob_gens_model_array.push( _inverse_generator );
              ...
              CIRCLESgenssetMANAGERiconSETUP(YES);
					...
					break ;
					case -1:
					if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, "Fail to update: the gen symbol is empty.", _glob_app_title );
					break ;
					case -2:
					if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING, "Fail to update: the current alphabet is empty and the candidate new gen can't be added.", _glob_app_title );
					break ;
	        default: break ;
			 }
	 }
...
function CIRCLESgenssetMANAGERgensDELETE( _row_index, _force, _b_switch, _question, _silent )
{
   ...
   if ( _glob_gens_set_to_init && !_force )
   {
       if ( !_silent )
       {
           ...
           circles_lib_output( OUTPUT_SCREEN, DISPATCH_WARNING | DISPATCH_OKCANCEL, "Fail to delete this entry: the generators set init is still pending.", _glob_app_title );
       }
   }
	 else if ( _edit != null )
	 {
  		 ...
  		 var _b_go = _gen_symbol.length == 0 ? YES : ( !_question ? YES : confirm( "Confirm to delete the gen '"+_gen_symbol+"' from the generators set ?" ) );
			 if ( _b_go )
			 {
               ...
               var _sch_n = circles_lib_gens_model_count();
		           for( var _i = 0 ; _i < _sch_n ; _i++ )
		           {
									 _candidate_gen = _glob_gens_model_array[_i] ;
									 if ( _gen_symbol.stricmp( _candidate_gen ) ||
                        _inv_gen_symbol.stricmp( _candidate_gen ) )
                   {
							       _glob_gens_model_array.remove( _i, _i );
		                 _glob_gens_set_rowcount-- ;
                     _n_removed++ ;
                     _i = -1 ;
               ...
               if ( _glob_gens_set_rowcount == 0 && _b_switch ) circles_lib_items_switch_to(ITEMS_SWITCH_SEEDS,YES);
					 ...
					 else if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_CRITICAL, "Fail to delete gen '"+_gen_symbol+"' from the current set: code failure", _glob_app_title );
			 }
	 }
	 else if ( !_silent ) circles_lib_output( OUTPUT_SCREEN, DISPATCH_CRITICAL, "Fail to delete gen '"+_gen_symbol+"' from the current set: memory failure", _glob_app_title );
}
