function circles_terminal_cmd_point()
{
     ...
     if ( _params.length > 0 )
     {
        ...
        _params_assoc_array['pt'] = [] ;
        _params_assoc_array['storagequeue'] = [] ;
        _params_assoc_array['storagesubset'] = "circles" ;
         ...
         for( _i = 0 ; _i < _up_to_index ; _i++ )
         {
			...
			{
				_params_assoc_array['radius'] = safe_string( _p.replace( /radius:/gi, "" ), "" ) ;
				if ( _params_assoc_array['radius'].testME( _glob_positive_float_regex_pattern ) )
				{
					_msg = "<lightblue>Radius has been set to</lightblue> <snow>"+_params_assoc_array['radius']+"</snow>" ;
					circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, _msg, _par_1, _cmd_tag );
				}
            ...
            else if ( _p.testME( _glob_cartesian_coords_regex_pattern ) )
            {
                ...
                _params_assoc_array['pt'].push( new point( safe_float( _pt_array[0], 0 ), safe_float( _pt_array[1], 0 ) ) );
         ...
         else if ( !_b_fail )
         {
             ...
             switch( _action )
             {
                 ...
                 else if ( _params_assoc_array['plane'] == NO_PLANE ) { _b_fail = YES, _error_str = "Can't plot point: missing plane reference" ; }
        
                // beware of some missing color param, so let's check'em deeper
                if ( _params_assoc_array['bordercolor'] == null && _params_assoc_array['fillcolor'] == null )
                {
                    _b_fail = YES, _error_str = "Missing draw and filling colors: this circle won't be visible" ;
                }
                else if ( _params_assoc_array['pt'].length == 0 ) { _b_fail = YES, _error_str = "Missing point(s) coordinate(s)" ; }
                else
                {
                ...
                var _fillcolor = _params_assoc_array['fillcolor'] ;
                var _fill = _fillcolor != null ? ( ( _fillcolor.length > 0 && !_fillcolor.is_one_of_i( "noclr", "transparent" ) ) ? YES : NO ) : NO ;
                var _opacity = safe_float( _params_assoc_array['opacity'], DEFAULT_MAX_OPACITY ) ;
                var _bordersize = safe_int( _params_assoc_array['bordersize'], _glob_pt_border );
                if ( _bordersize == 0 ) { _border = NO ; _bordercolor = "" ; }
                var _radius = safe_float( _params_assoc_array['radius'], _glob_pt_radius ) ;
        
                var _pt_obj = null ;
                // switch to different constructors, according to input data
                  for( var _i = 0 ; _i < _params_assoc_array['pt'].length ; _i++ )
                  {
                      if ( _params_assoc_array['pt'][_i] != null ) _pt_obj = _params_assoc_array['pt'][_i] ;
                      if ( is_point( _pt_obj ) )
                      {
                          circles_lib_draw_point( _canvas_context, _mapper, _pt_obj.x, _pt_obj.y,
                                            _border, _bordercolor, _fill, _fillcolor, _bordersize, _radius, _opacity, _params_assoc_array['propertiesmask'] );
                          circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, "<snow>(" + circles_lib_plane_def_get( _params_assoc_array['plane'] ) + ")</snow> <green>Point "+_pt_obj.output("cartesian")+" processed with success</green>", _par_1, _cmd_tag );
                          ...
                          if ( _params_assoc_array['rec'] == YES || _storage_queue_request )
                          {
                               ...
                               _rec_chunk['label'] = _params_assoc_array['label'].length > 0 ? _params_assoc_array['label'] : "" ;
