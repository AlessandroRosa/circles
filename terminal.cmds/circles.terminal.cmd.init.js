...
function circles_terminal_cmd_init()
{
    ...
    for( var _i = 0 ; _i < _params_array.length ; _i++ )
    {
        ...
        else if ( _p.is_one_of( "lock", "force" ) ) _params_assoc_array[""+_p] = YES ;
    ...
    else if ( !_b_fail )
    {
        ...
        switch( _action )
        {
            ...
            if ( ( _glob_init_mask & INIT_LOCK ) != 0 &&
                 ( _params_assoc_array['initoptions'] != INIT_NONE || _params_assoc_array['settings']['symbols'] ) )
            {
            ...
            else if ( _items_n > 0 && !_b_fail )
            {
                ...
                if ( _params_assoc_array['auto'] )
                {
                    ...
                    _ret_chunk = circles_lib_items_init( UNDET, NO, YES, _init_mask | INIT_SYMBOLS, YES, _params_assoc_array['force'], _output_channel );
                    _ret_id = is_array( _ret_chunk ) ? _ret_chunk[0] : RET_ERROR ;
                    _ret_msg = is_array( _ret_chunk ) ? _ret_chunk[1] : "Unknown error" ;
            		if ( _ret_id.is_one_of( RET_OK, RET_IRRELEVANT ) ) circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, _ret_msg, _par_1, _cmd_tag );
                ...
                else
                {
                    ...
                    {
                        _init_mask = safe_int( _params_assoc_array['initoptions'], INIT_NONE ) ;
                        if ( _init_mask == INIT_NONE ) circles_lib_output( _output_channel, DISPATCH_WARNING, "Missing init options", _par_1, _cmd_tag );
                        else
                        {
