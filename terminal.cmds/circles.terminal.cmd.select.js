     ...
     if ( _params.length > 0 )
     {
         ...
         else
         {
             ...
             switch( _action )
             {
                ...
                break ;
                default:
                var symbol = _params_assoc_array['symbol'].trim();
                if ( symbol.length > 0 )
                {
                    _params = "select " + _params_assoc_array['symbol'].join( " " );
                    circles_terminal_cmd_disk();
                }
                else
                {
                    _b_fail = YES, _error_str = "Missing input symbol" ;
                }
                break ;
             }
         }
     }
     else { _b_fail = YES, _error_str = "Missing input symbol" ; }
