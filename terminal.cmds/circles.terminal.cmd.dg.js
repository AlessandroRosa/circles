...
function circles_terminal_cmd_dg()
{
    ...
    if ( _params.length > 0 )
    {
        ...
        for( var _i = 0 ; _i < _params_array.length ; _i++ )
        {
            ...
            {
                _params_assoc_array['tag'].push( _p.replaceAll( "tag:", "" ) );
                _begin_flag = 1 ;
            }
            ...
            else if ( _params_assoc_array['action'] == "renametag" )
			{
				if ( _params_assoc_array['tag'].length == 0 ) _params_assoc_array['tag'] = _p ;
				else if ( _params_assoc_array['renametag'].length == 0 ) _params_assoc_array['renametag'] = _p ;
			}
        ...
        else if ( !_b_fail )
        {
           ...
           switch( _action )
           {
                ...
                var _tag = safe_string( _params_assoc_array['tag'][0], "" ) ;
                ...
                else
                {
                    function _call_group()
                    {
                        ...
                        if ( circles_lib_count_seeds() > 0 && safe_size( _error_append, 0 ) == 0 )
                        {
                            ...
                            if ( _ret_id == RET_OK ) circles_lib_output( _output_channel, DISPATCH_SUCCESS, "All group members have been init with success", _par_1, _cmd_tag );
							else circles_lib_output( _output_channel, DISPATCH_WARNING, "All group members have not been init with success", _par_1, _cmd_tag );
                        }
                ...
                else
                {
                    ...
                    $.each( _maps_ref, function( _i, _map_tag )
                            {
                                circles_lib_output( _output_channel, DISPATCH_INFO, "Candidate map #"+(_i+1)+" tag : '"+_map_tag+"'", _par_1, _cmd_tag );
				...
				if ( _index != null )
				{
					...
					if ( _len == 0 ) { _b_go = 0 ; circles_lib_output( _output_channel, DISPATCH_WARNING, "The groups list is empty", _par_1, _cmd_tag ); }
					else if ( _index < 0 ) { _b_go = 0 ; circles_lib_output( _output_channel, DISPATCH_WARNING, "Negative indexes are not allowed", _par_1, _cmd_tag ); }
					else if ( _index > _len )
					{
                ...
                break ;
				case "renametag":
				if ( _params_assoc_array['tag'] == null )
				circles_lib_output( _output_channel, DISPATCH_ERROR, "Missing source tag", _par_1, _cmd_tag );
				else if ( _params_assoc_array['renametag'] == null )
				circles_lib_output( _output_channel, DISPATCH_ERROR, "Missing new tag", _par_1, _cmd_tag );
				else
				{
					var _idx = _dg_cmd_find( _params_assoc_array['tag'] );
					if ( _idx != UNDET )
					{
						_glob_groups_table[_idx][2] = _params_assoc_array['renametag'] ;
						circles_lib_output( _output_channel, DISPATCH_SUCCESS, "Old tag '"+_params_assoc_array['tag']+"' has been renamed into '"+_params_assoc_array['renametag']+"' with success", _par_1, _cmd_tag );
					}
				}
				break ;
                case "release":
                ...
                else if ( safe_size( _ref_mm_array, 0 ) > 0 )
                {
                    ...
                    else
                    {
                        ...
                        var _new_n = safe_size( _glob_groups_table, 0 );
                        if ( _new_n == _old_n + 1 ) circles_lib_output( _output_channel, DISPATCH_SUCCESS, "Group in use has been tagged as '"+_tag+"' and saved with success", _par_1, _cmd_tag );
                        else
                        {
                ...
                else if ( _ws_len > 0 )
                {
                    ...
                    if ( _consistence_check )
                    {
                        ...
                        if ( _g_n > 0 && _g_n % 2 == 0 )
                        {
                            circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, "<lime>Subgroup has been created with success and tagged as '"+_tag+"'</lime>", _par_1, _cmd_tag );
        ...
        else
        {
               ...
               is_complex( _c_complex ) && is_complex( _d_complex ) && !_b_fail )
          {
               ...
               if ( _b_ret != RET_ERROR && _service_array.includes( "render" ) )
               {
                   ...
                   else circles_lib_output( _output_channel, DISPATCH_SUCCESS,  "All group members have been init with success", _par_1, _cmd_tag );
                   if ( _ret_id != RET_ERROR )
                   circles_lib_terminal_interpreter( "refresh wplane silent clean", _glob_terminal, _output_channel );
               }
