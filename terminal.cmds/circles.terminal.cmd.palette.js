function circles_terminal_cmd_palette()
{
     ...
     if ( _params.length > 0 )
     {
         ...
         // pre-scan for levenshtein correction
    		 var _cmd_terms_dict = [];
    				 _cmd_terms_dict.push( "on", "off", "adapt", "append", "create", "invert", "destroy",
         ...
         for( var _i = 0 ; _i < _up_to_index ; _i++ )
         {
              ...
              else if ( _p.stricmp( "plain" ) ) _cmd_params['plain'] = YES ;
              else
              {
               ...
               switch( _action )
               {
                  ...
                  if ( circles_lib_colors_is_def( _rgb_start ) && circles_lib_colors_is_def( _rgb_end ) )
                  {
                     ...
                     var _ret_id = _ret_chunk[0], _ret_palette = _ret_chunk[1], _ret_msg = _ret_chunk[2] ;
		                 ...
		                 _rgb_start = _cmd_params['action'].strcmp( "resize" ) ? _glob_palette_array[0] : ( circles_lib_colors_get_formats( _rgb_start ) )[COLOR_RGB_INT] ;
		                 _rgb_end = _cmd_params['action'].strcmp( "resize" ) ? _glob_palette_array.get_last() : ( circles_lib_colors_get_formats( _rgb_end ) )[COLOR_RGB_INT] ;
		                 ...
		                 else if ( _glob_palette_array.length > 0 )
		                 {
		                    ...
		                    else if ( _cmd_params['action'] == "resize" )
