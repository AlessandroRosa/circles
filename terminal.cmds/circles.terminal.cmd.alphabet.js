function circles_terminal_cmd_alphabet()
{
    ...
    else if ( _params.length > 0 )
    {
        ...
        for( var _i = 0 ; _i < _params_array.length ; _i++ )
        {
            ...
            {
                if ( _cmd_params['action'].is_one_of( "add", "delete" ) )
                {
                  _p = _p.replaceAll( _glob_punctuation, "" );
                  if ( _p.length == 1 )
                  {
    				_cmd_params['symbol'].push( _p );
    				_cnt++ ;
                  }
                  else if ( _p.includes( "," ) )
                  {
                    ...
                    {
                         if ( _symbol.length == 1 ) _cmd_params['symbol'].push( _symbol );
    ...
    else if ( !_b_fail )
    {
        ...
        if ( safe_size( _action, 0 ) == 0 )
        {
           ...
           circles_lib_output( _out_channel, DISPATCH_INFO, "Missing input action: current alphabet is shown below", _par_1, _cmd_tag );
           _action = "show" ;
        }

     	var _symbols_array = _cmd_params['symbol'] ;
        ...
        switch( _action )
        {
			...
			else
			{
		        ...
		        circles_lib_output( _out_channel, DISPATCH_INFO, "Current alphabet includes "+_size+" element"+(_size==1?"":"s"), _par_1, _cmd_tag );
			}
			...
			else
			{
				...
				if ( _n_report == 0 )
				{
					...
					circles_lib_output( _out_channel, DISPATCH_SUCCESS, "It correctly matches with the group in use", _par_1, _cmd_tag );
					...
					if ( _symbol.length > 0 )
					{
						if( _glob_alphabet.includes( _symbol ) )
						{
							...
							else
							{
								...
								circles_lib_output( _out_channel, _out_id ? DISPATCH_SUCCESS : DISPATCH_ERROR, _out_msg, _par_1, _cmd_tag );
							}
						}
            ...
            if ( _n_sd > 0 )
            {
              ...
              circles_lib_output( _out_channel, DISPATCH_INFO, "Deleting current alphabet", _par_1, _cmd_tag );
              _glob_alphabet = [];
              circles_lib_output( _out_channel, DISPATCH_INFO, "Checking now "+_n_sd+" registered seed"+(_n_sd==1?"":"s")+" for alphabet regeneration", _par_1, _cmd_tag );
			...
			{
				circles_lib_output( _out_channel, DISPATCH_INFO, "Current alphabet is empty", _par_1, _cmd_tag );
				if ( safe_size( _items_array, 0 ) == 0 )
				circles_lib_output( _out_channel, DISPATCH_INFO, "Probably no group is in use", _par_1, _cmd_tag );
				else
				circles_lib_output( _out_channel, DISPATCH_INFO, "You can regenerate it by typing 'alphabet regen'", _par_1, _cmd_tag );
			}
            _fn_ret_val = _glob_alphabet.clone() ;
            break ;
	        default: break ;
