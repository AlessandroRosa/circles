function circles_terminal_cmd_alphabet()
{
    ...
    else if ( !_b_fail )
    {
        ...
        if ( safe_size( _action, 0 ) == 0 )
        {
           ...
           circles_lib_output( _output_channel, DISPATCH_INFO, "Missing input action: current alphabet is shown below", _par_1, _cmd_tag );
           _action = "show" ;
        }
        ...
        switch( _action )
        {
				...
				for( var _i = 0 ; _i < _symbols_array.length ; _i++ )
				{
					...
					if ( _symbol.length > 0 )
					{
						...
						if ( _mask == 3 )
						{
							...
							circles_lib_output( _output_channel, DISPATCH_SUCCESS, _error_str_array['err2'].replaceAll( [ "%symbol%", "%not%", "%success%" ], [ _symbol, " ", "with success" ] ), _par_1, _cmd_tag );
							if ( _params_assoc_array['settings'].includes( "inv" ) )
							{
								_glob_alphabet.push( _symbol.flipCase() );
								circles_lib_output( _output_channel, DISPATCH_INFO, _error_str_array['err2'].replaceAll( [ "%symbol%", "%not%", "%success%" ], [ _symbol.flipCase(), " ", "with success" ] ), _par_1, _cmd_tag );
							}
						}
						else circles_lib_output( _output_channel, DISPATCH_WARNING, _error_str_array['err2'].replaceAll( [ "%symbol%", "%not%", "%success%" ], [ _symbol, " not ", "" ] ), _par_1, _cmd_tag );
																 
						if ( ( _mask & 1 ) == 0 )
						circles_lib_output( _output_channel, DISPATCH_ERROR, _error_str_array['err3'], _par_1, _cmd_tag );
						if ( ( _mask & 2 ) == 0 )
						{
						   _error_str = _error_str_array['err7'].replaceAll( [ "%inclusor%", "%symbol%" ], [ "already includes", _symbol ] );
						   circles_lib_output( _output_channel, DISPATCH_ERROR, $.terminal.escape_brackets( _error_str ), _par_1, _cmd_tag );
						}
					}
				}
            ...
            case "bomb":
            if ( safe_size( _glob_alphabet, 0 ) == 0 )
            circles_lib_output( _output_channel, DISPATCH_INFO, "The current alphabet is still empty", _par_1, _cmd_tag );                   
            else
            {
                var _bomb_alphabet = function()
                {
                   _glob_alphabet = [];
                   if ( safe_size( _glob_alphabet, 0 ) == 0 ) circles_lib_output( _output_channel, DISPATCH_SUCCESS, "All entries in the current alphabet have been deleted with success", _par_1, _cmd_tag );
                   else circles_lib_output( _output_channel, DISPATCH_WARNING, "Problems while trying to delete all entries in the current alphabet." + _glob_crlf + "Please, retry", _par_1, _cmd_tag );
				   var _size = safe_size( _glob_alphabet, 0 ) ;
				   circles_lib_output( _output_channel, DISPATCH_INFO, "Current alphabet includes "+_size+" element"+(_size==1?"":"s"), _par_1, _cmd_tag );
                }
            ...
            break ;
            case "check":
            circles_lib_output( _output_channel, DISPATCH_INFO, "Starting alphabet check", _par_1, _cmd_tag );
            var _size = safe_size( _glob_alphabet, 0 ) ;
            if ( !is_array( _glob_alphabet ) )
            {
		        circles_lib_output( _output_channel, DISPATCH_WARNING, "Memory failure: alphabet archive needs to be regenerated again", _par_1, _cmd_tag );
		        circles_lib_output( _output_channel, DISPATCH_INFO, _error_str_array['info1'], _par_1, _cmd_tag );
			}
			else if ( _size == 0 )
           	circles_lib_output( _output_channel, DISPATCH_WARNING, "Check stopped: alphabet is empty", _par_1, _cmd_tag );
			else if ( _n_seeds != _size )
			{
				if ( _n_seeds == 0 )
           		circles_lib_output( _output_channel, DISPATCH_WARNING, "Check stopped: no seeds have been registered yet", _par_1, _cmd_tag );
           		else if ( _size == 0 )
           		{
					circles_lib_output( _output_channel, DISPATCH_WARNING, "Size failure: alphabet archive is empty", _par_1, _cmd_tag );
				    circles_lib_output( _output_channel, DISPATCH_INFO, _error_str_array['info1'], _par_1, _cmd_tag );
				}
		        else
		        {
					circles_lib_output( _output_channel, DISPATCH_WARNING, "Size failure: alphabet archive needs to be regenerated again", _par_1, _cmd_tag );
				    circles_lib_output( _output_channel, DISPATCH_INFO, _error_str_array['info1'], _par_1, _cmd_tag );
				}
			}
			else
			{
				var _report = [] ;
				circles_lib_output( _output_channel, DISPATCH_INFO, "Found "+_size+" entr"+(_size==1?"y":"ies")+" in the alphabet: "+_glob_alphabet.join( ", " ), _par_1, _cmd_tag );
				$.each( _glob_alphabet, function( _i, _sym )
						{
				...
				if ( _n_report == 0 )
				{
					circles_lib_output( _output_channel, DISPATCH_SUCCESS, "Alphabet check has been passed with success", _par_1, _cmd_tag );
					if ( safe_size( _items_array, 0 ) > 0 )
					circles_lib_output( _output_channel, DISPATCH_SUCCESS, "It correctly matches with the group in use", _par_1, _cmd_tag );
				...
				{
					circles_lib_output( _output_channel, DISPATCH_WARNING, "Found "+_n_report+" errors", _par_1, _cmd_tag );
					$.each( _report, function( _i, _rep ) { circles_lib_output( _output_channel, DISPATCH_WARNING, _rep, _par_1, _cmd_tag ); } );
				}
			}
            break ;
            case "colorize":
				...
				for( var _i = 0 ; _i < _symbols_array.length ; _i++ )
				{
					...
					if ( _symbol.length > 0 )
					{
						if( _glob_alphabet.includes( _symbol ) )
						{
							...
							_mobius_map_index = circles_lib_find_item_index_by_symbol( _glob_seeds_array, _symbol, _output_channel );
							...
							else
							{
								...
								circles_lib_output( _output_channel, _out_id ? DISPATCH_SUCCESS : DISPATCH_ERROR, _out_msg, _par_1, _cmd_tag );
							}
						}
						else { _b_fail = YES, _error_str = _error_str_array['err7'].replaceAll( [ "%inclusor%", "%symbol%" ], [ "does not include", _symbol ] ); }
					}
				}
            ...
            if ( _n_sd > 0 )
            {
			  _long_mode = _params_assoc_array['settings'].includes_i( "long" ) ? 1 : 0 ;
			  if ( _long_mode ) circles_lib_output( _output_channel, DISPATCH_INFO, "Long mode has been switched on", _par_1, _cmd_tag );

			  circles_lib_output( _output_channel, DISPATCH_INFO, "Attempting to regenerate the alphabet from current seeds configuration", _par_1, _cmd_tag );
			  var ITEM, _symbol, _inv_symbol, _check, _out_array = [] ;
              circles_lib_output( _output_channel, DISPATCH_INFO, "Deleting current alphabet", _par_1, _cmd_tag );
              _glob_alphabet = [];
              circles_lib_output( _output_channel, DISPATCH_INFO, "Checking now "+_n_sd+" registered seed"+(_n_sd==1?"":"s")+" for alphabet regeneration", _par_1, _cmd_tag );
              ...
              if ( _n_alpha > 0 )
              {
                  ...
                  if ( _long_mode )
                  {
                      _MSG += "Alphabet" + _glob_crlf ;
                      _MSG += ( " " ).repeat( 35 )+"<white>Clr Draw (disks)" + ( " " ).repeat( 2 )+"Clr Fill (disks)</white>" + _glob_crlf ;
                      _MSG += "<snow>Symbol</snow>    <lightblue>Inverse symbol</lightblue>  <white>Check</white>" ;
					  _MSG += "    <yellow>Complex" + "  Screen" + "   Complex" + "  Screen</yellow>" + _glob_crlf ;
                  ...
                  {
                     circles_lib_output( _output_channel, DISPATCH_ERROR, "Mismatch errors while regenerating the alphabet", _par_1, _cmd_tag );
                     circles_lib_output( _output_channel, DISPATCH_INFO, _fail_array.join( _glob_crlf ), _par_1, _cmd_tag );
                  }
                  else
				  {
					var _size = safe_size( _glob_alphabet, 0 ) ;
					circles_lib_output( _output_channel, DISPATCH_INFO, "Current alphabet includes "+_size+" element"+(_size==1?"":"s"), _par_1, _cmd_tag );
	 				circles_lib_output( _output_channel, DISPATCH_SUCCESS, "Alphabet has been completely regenerated with success", _par_1, _cmd_tag );
				  }

                  circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, "Regenerated alphabet includes these "+_n_alpha+" entries <lightblue>" + _glob_alphabet.join( ", " ) + "</lightblue>", _par_1, _cmd_tag );
              }
              else circles_lib_output( _output_channel, DISPATCH_ERROR, "Fail to regenerate alphabet", _par_1, _cmd_tag );
            }
            else circles_lib_output( _output_channel, DISPATCH_WARNING, "Fail to regenerate alphabet: missing registered seeds", _par_1, _cmd_tag );
            _fn_ret_val = _glob_alphabet.clone() ;
            break ;
            case "release":
            ...
            if ( safe_size( _glob_alphabet, 0 ) > 0 )
			{
				circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, "Current alphabet is <lightblue>" + _glob_alphabet.join( ", " ) + "</lightblue>", _par_1, _cmd_tag );
				circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, "Current alphabet size is <white>" + safe_size( _glob_alphabet, 0 )+"</white>", _par_1, _cmd_tag );
			}
            else
			{
				circles_lib_output( _output_channel, DISPATCH_INFO, "Current alphabet is empty", _par_1, _cmd_tag );
				if ( safe_size( _items_array, 0 ) == 0 )
				circles_lib_output( _output_channel, DISPATCH_INFO, "Probably no group is in use", _par_1, _cmd_tag );
				else
				circles_lib_output( _output_channel, DISPATCH_INFO, "You can regenerate it by typing 'alphabet regen'", _par_1, _cmd_tag );
			}
            _fn_ret_val = _glob_alphabet.clone() ;
            break ;
	        default: break ;
