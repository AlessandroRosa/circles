function circles_terminal_cmd_stop()
{
     ...
     else if ( !_b_fail )
     {
         ...
         switch( _action )
         {
             ...
             break ;
             default:
             if ( _glob_worker_lock || _glob_inline_worker != null )
             {
                  CIRCLESmultithreadingSTOPworker(YES);
                  if ( !_glob_process_running_flag )
                  circles_lib_output( _output_channel, DISPATCH_SUCCESS, "Current process has been stopped by user", _par_1, _cmd_tag );
             }
             else if ( _glob_worker_lock == 0 )
             circles_lib_output( _output_channel, DISPATCH_WARNING, "There's no current working process to stop at the moment", _par_1, _cmd_tag );
             break ;
         }
     }
