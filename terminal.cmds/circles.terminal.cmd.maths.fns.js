function circles_terminal_cmd_deg()
{
     ...
     if ( _params.length == 0 )
     circles_lib_output( _output_channel, DISPATCH_WARNING, "Please, input the value in radians for conversion into degrees", _par_1, _cmd_tag );
     else
     {
         ...
         else
         {
              ...
              circles_lib_output( _output_channel, DISPATCH_INFO, _rad + ( ( _rad == 1 ) ? " radiant" : " radians" ) + "</lightblue>\n<cadetblue>" + _deg + ( ( _deg == 1 ) ? " degree" : " degrees" ), _par_1, _cmd_tag );
         }
     }
...
function circles_terminal_cmd_rad()
{
     ...
     _params = safe_string( _params, "" ).trim();
     if ( _params.length == 0 )
     circles_lib_output( _output_channel, DISPATCH_WARNING, "Please, input the value in degrees for conversion into radians", _par_1, _cmd_tag );
     else
     {
         var _params_array = _params.includes( " " ) ? _params.split( " " ) : [ _params ] ;
         var _p ;
