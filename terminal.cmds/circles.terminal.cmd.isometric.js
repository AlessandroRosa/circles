...
function circles_terminal_cmd_isometric()
{
     ...
     if ( _params.length > 0 )
     {
        ...
        _cmd_params['all'] = 0 ;
        _cmd_params["copy"] = NO ;
        _cmd_params['border'] = NO ;
        _cmd_params['bordercolor'] = "" ;
        _cmd_params['dump'] = NO ;
        _cmd_params['dump_array'] = null ;
        _cmd_params['dump_operator_index'] = UNDET ;
        _cmd_params['fillcolor'] = "" ;
        _cmd_params['help'] = NO ;
        ...
        if ( _cmd_params['dump'] )
        {
         ...
         for( var _i = 0 ; _i < _up_to_index ; _i++ )
         {
              ...
              else if ( _p.stricmp( "table" ) ) _cmd_params['table'] = YES ;
              else if ( _p.is_one_of_i( "show", "compute" ) ) _cmd_params['action'] = _p.toLowerCase();
              ...
              {
                   if ( _cmd_params['bordercolor'].length == 0 ) _cmd_params['bordercolor'] = _p ;
                   else if ( _cmd_params['fillcolor'].length == 0 ) _cmd_params['fillcolor'] = _p ;
              }
              else if ( _p.testME( _glob_simple_string_regex_pattern ) )
              {
                   _cmd_params['action'] = "compute" ;
                   _cmd_params['word'] = _p ;
              }
         ...
         {
             if ( _symbols_array.length > 0 && _cmd_params['action'].length == 0 ) _cmd_params['action'] = "compute" ;
             ...
             switch( _action )
             {
                ...
                case "compute":
                  ...
                  else if ( safe_size( _glob_alphabet, 0 ) == 0 )
                  {
                       ...
                       if ( _cmd_params['word'].length > 0 ) _error_str += " : can't compute isometric circle from word '"+_cmd_params['word']+"'" ;
                  }
                  else if ( safe_size( _symbols_array, 0 ) == 0 && _cmd_params['word'].length == 0 ) circles_lib_output( _out_channel, DISPATCH_WARNING, "Can't "+_action+" the isometric circles: missing input symbols", _par_1, _cmd_tag );
                  else if ( _cmd_params['word'].length > 0 )
                  {
                       var _word = _cmd_params['word'] ;
                       var _border = ( _cmd_params['border'] || _cmd_params['bordercolor'].length > 0 || _cmd_params['fillcolor'].length > 0 ) ? YES : NO ;
                       var G, _mm ;
                       ...
                       if ( !_b_fail )
                       {
                           ...
                           if ( _border && _cmd_params['plane'] == NO_PLANE )
                           {
                           ...
                           if ( _border )
                           circles_terminal_cmd_isometric_draw( _cmd_params['plane'], _word, _isometric_cc,
                                                                 _cmd_params['bordercolor'], _cmd_params['fillcolor'],
                           ...
                           if ( _cmd_params['rec'] || _cmd_params["copy"] )
                           {
                                ...
                                _rec_chunk['bordercolor'] = _cmd_params['bordercolor'] ;
                                _rec_chunk['fill'] = _fill ;
                                _rec_chunk['fillcolor'] = _cmd_params['fillcolor'] ;
                  ...
                  else if ( _symbols_array.length > 0 )
                  {
                       ...
                       for( var _i = 0 ; _i < _symbols_array.length ; _i++ )
                       {
                            ...
                            if ( is_item_obj( ITEM ) && _index != UNFOUND && _inverse_index != UNFOUND )
                            {
                                 ...
                                 if ( _border )
                                 circles_terminal_cmd_isometric_draw( _cmd_params['plane'], ITEM.symbol, _isometric_cc,
                                                                      _cmd_params['bordercolor'], _cmd_params['fillcolor'],
                                                                      2, _out_channel, _par_1, _cmd_tag );
...
function circles_terminal_cmd_isometric_draw( _plane_type, _word, _isometric_cc, _bordercolor, _fillcolor, _bordersize, _out_channel, _par_1 )
{
     ...
     {
        circles_lib_output( _out_channel, DISPATCH_INFO, "Plot circle '"+_word+"' on the " + circles_lib_plane_def_get( _plane ), _par_1, _cmd_tag );
        var _mapper = null ;
