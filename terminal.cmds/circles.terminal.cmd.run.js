function circles_terminal_cmd_run()
{
	...
	var _cmd_terms_dict = [ "release", "html", "help" ];
    ...
    else if ( !_b_fail )
    {
        ...
        switch( _action )
        {
            ...
            break ;
            default:
            if ( _glob_terminal != null && _out_channel == OUTPUT_TERMINAL && _cmd_params['help'] != YES )
            {
				var _script_code = $("#CIRCLESbatchcompilerTEXT" + _glob_terminal_form_suffix ).val();
        		if ( _script_code.length > 0 )
        		{
        			circles_lib_output( _out_channel, DISPATCH_SUCCESS, "Batch script found and run", _par_1, _cmd_tag );
        		 	circles_lib_output( _out_channel, DISPATCH_INFO, "Check debug tab for report", _par_1, _cmd_tag );
        		 	circles_lib_output( _out_channel, DISPATCH_INFO, "Type 'stop' to halt the current process", _par_1, _cmd_tag );
        		...
        		else circles_lib_output( _out_channel, DISPATCH_WARNING, "No batch script code to run", _par_1, _cmd_tag );         
            }
            break ;
        }
    }
