    ...
    var _cmd_params = [];
     ...
     else if ( _params.length > 0 )
     {
        ...
        if ( _cmd_params['dump'] )
        {
         ...
         _cmd_params['plane'] = _glob_target_plane != NO_PLANE ? circles_lib_plane_def_get( _glob_target_plane ) : "zplane" ;
         _cmd_params['help'] = NO ;
         _cmd_params['action'] = "" ;
         _cmd_params['params'] = [];
         ...
         for( var _i = 0 ; _i < _up_to_index ; _i++ )
         {
            ...
            {
                if ( _cmd_params['w'] == null ) _cmd_params['w'] = safe_int( _p, 0 );
                else if ( _cmd_params['h'] == null ) _cmd_params['h'] = safe_int( _p, 0 );
            }
            ...
            {
				if ( _cmd_params['action'].strcmp( "redirect" ) && _p.stricmp( "list" ) ) _cmd_params['action'] += _p ;
				else
				{
            ...
            else if ( _p.is_one_of_i( "copy" ) )
            {
                ...
                if ( _glob_storage[_cmd_tag]['open'] )
                {
                    ...
                    if ( _out_channel == OUTPUT_TERMINAL )
                    circles_lib_output( _out_channel, DISPATCH_STANDARD, "Insert source layer for copy (e.g. 'zplane work')", _par_1, _cmd_tag );
                }
                ...
                if ( _glob_storage[_cmd_tag]['action'].strcmp( "copy" ) )
                {
                    ...
                    else
                    {
                        if ( _glob_storage[_cmd_tag]['from_layer'] == null )
                        {
                            ...
                            if ( _out_channel == OUTPUT_TERMINAL )
                            circles_lib_output( _out_channel, DISPATCH_STANDARD, "Insert destination layer for copy (e.g. 'wplane work')", _par_1, _cmd_tag );
                        }
                        else if ( _glob_storage[_cmd_tag]['to_plane'] == null )
                        {
                            _glob_storage[_cmd_tag]['to_plane'] = _p ;
            ...
            else if ( _cmd_params['action'].strcmp( "redirect" ) )
            {
                if ( _cmd_params['from_plane'] == null )
                {
                    if ( _p.stricmp( "zplane" ) ) _cmd_params['from_plane'] = Z_PLANE ;
                    else if ( _p.stricmp( "wplane" ) ) _cmd_params['from_plane'] = W_PLANE ;
                    _index_associations['from_plane'] = _i ;
                }
                else if ( _i == _index_associations['from_plane'] + 1 )
                {
                    _cmd_params['from_service'] = _p ;
                    _index_associations['from_service'] = _i ;
                }
                else if ( _i == _index_associations[ "to" ] + 1 )
                {
                    if ( _p.stricmp( "zplane" ) ) _cmd_params['to_plane'] = Z_PLANE ;
                    else if ( _p.stricmp( "wplane" ) ) _cmd_params['to_plane'] = W_PLANE ;
                    else if ( _p.stricmp( "bip" ) ) _cmd_params['to_plane'] = BIP_BOX ;
                    _index_associations['to_plane'] = _i ;
                }
                else if ( _i == _index_associations['to_plane'] + 1 )
                {
                    _cmd_params['to_service'] = _p ;
                    _index_associations['to_service'] = _i ;
                }
            }
            else if ( !_p.testME( _glob_illegalchars_regex_pattern ) &&
                      _cmd_params['action'].is_one_of( "create", "delete", "list", "thumbnail", "update" ) )
            {
                if ( _p.is_one_of_i( "zplane", "wplane", "bip" ) )
                {
                    _cmd_params['plane'] = _p ;
                    _index_associations['plane'] = _i ;
					if ( _cmd_params['action'].is_one_of( "create", "delete", "update" ) )
                    {
     ...
     {
           var _params = _cmd_params['params'] ;
           ...
           switch( _action )
           {
                ...
                var _from_canvas = ( _from_layer != null ) ? _from_layer.get_idcanvas() : "" ;
                    ...
                    _to_plane = circles_lib_canvas_layer_find( _to_plane, FIND_LAYER_BY_ROLE_DEF, _to_plane, _out_channel );
                    ...
                    else if ( _action.strcmp( "delete" ) )
                    {
                          ...
                          else
                          {
                            var _ret = UNDET, _delete_layer = function()
                            {
                                ...
                                else { var _msg = "Layer '"+_action_params_array['roledef']+"' has been deleted with success from the "+_action_params_array['plane']+" list" ; circles_lib_output( _out_channel, DISPATCH_SUCCESS, _msg, _par_1, _cmd_tag ); }
                            };
                ...
                {
                    circles_lib_output( _out_channel, DISPATCH_INFO, "Restoring layer properties", _par_1, _cmd_tag );
                    circles_lib_output( _out_channel, DISPATCH_INFO, "Restoring layer redirection assignments", _par_1, _cmd_tag );
                    circles_lib_reset_canvas();
                    circles_lib_output( _out_channel, DISPATCH_SUCCESS, "All default layer settings restored with success", _par_1, _cmd_tag );
                }
                ...
                break ;
                case "list":
                var _plane_type = _cmd_params['plane'] != null ? ( _cmd_params['plane'].stricmp( "zplane" ) ? Z_PLANE : W_PLANE ) : NO_PLANE ;
                if ( _plane_type != NO_PLANE )
                {
                    ...
                    if ( safe_size( _layers_list, 0 ) > 0 )
                    {
                        ...
                        for( var _i = 0 ; _i < _layers_list.length ; _i++ )
                        {
                            ...
                            else circles_lib_output( _out_channel, DISPATCH_WARNING, "Found memory leak while returning layers list", _par_1, _cmd_tag );
                        }
                    }
                ...
                case "merge":
                var _plane = NO_PLANE ;
                if ( _cmd_params['plane'].strcmp( "all" ) ) _plane = ALL_PLANES ;
                else if ( _cmd_params['plane'].strcmp( "zplane" ) ) _plane = Z_PLANE ;
                else if ( _cmd_params['plane'].strcmp( "wplane" ) ) _plane = W_PLANE ;
                ...
                {
                    circles_lib_output( _out_channel, _zplane_already ? DISPATCH_INFO : DISPATCH_SUCCESS, "Z-plane canvases " + ( _zplane_already ? "are already merged" : "have been merged with success" ), _par_1, _cmd_tag );
                    _glob_zplane_canvas_merge = YES ;
                }

                if ( _plane.is_one_of( W_PLANE, ALL_PLANES ) )
                {
                    circles_lib_output( _out_channel, _zplane_already ? DISPATCH_INFO : DISPATCH_SUCCESS, "W-plane canvases " + ( _wplane_already ? "are already merged" : "have been merged with success" ), _par_1, _cmd_tag );
                ...
                break ;
                case "redirect":
                var _candidate_from_layer = circles_lib_canvas_layer_find( _cmd_params['from_plane'], FIND_LAYER_BY_ROLE_DEF, _cmd_params['from_service'], _out_channel );
                if ( is_html_canvas( _candidate_from_layer ) )
                {
                    var _to_service = _cmd_params['to_service'] ;
                ...
                break ;
				case "redirectlist":
				switch( circles_lib_plane_get_value( _cmd_params['plane'] ) )
				{
					case Z_PLANE:
				...
				break ;
                case "resize":
                var _plane = NO_PLANE, _default = safe_int( _cmd_params['default'], NO );
                if ( _cmd_params['plane'].stricmp( "zplane" ) ) _plane = Z_PLANE ;
                else if ( _cmd_params['plane'].stricmp( "wplane" ) ) _plane = W_PLANE ;
                var _width_percentage = safe_int( _cmd_params['w'], 50 );
				...
				if ( _plane == NO_PLANE ) circles_lib_output( _out_channel, DISPATCH_ERROR, "Fail to reverse the layers pile: missing input plane specification", _par_1, _cmd_tag );
				else
				{
					var _layers_pile = circles_lib_canvas_layer_pile_get(_plane) ;
					if ( _layers_pile == null ) circles_lib_output( _out_channel, DISPATCH_ERROR, "Fail to reverse the layers pile: invalid data", _par_1, _cmd_tag );
					else
					{
						_layers_pile.reverse();
						var _plane_def = circles_lib_plane_def_get(_plane);
						circles_lib_output( _out_channel, DISPATCH_SUCCESS, "The layers pile, in the "+_plane_def+", has been reverted with success", _par_1, _cmd_tag );
					}
				}
				break ;
                case "split":
                ...
                var _plane_type = _cmd_params['plane'].strcmp( "zplane" ) ? Z_PLANE : W_PLANE ;
                var _role_def = _cmd_params['params'][0] ;
                var _w = safe_int( _cmd_params['w'], 0 );
                var _h = safe_int( _cmd_params['h'], 0 );
                var _side = 0 ;
