function circles_terminal_cmd_rgb()
{
     ...
     if ( _params.length > 0 )
     {
         ...
         else
         {
             ...
             switch( _action )
             {
                  ...
                  break ;
                  default:
                  var _ret_chunk = circles_lib_colors_get_formats( _params_assoc_array['color'] );
                  var startINDEX = 0 ;
                  _b_fail = _ret_chunk[startINDEX] ;
                      startINDEX++ ;
                  var _rgb_dec_fmt = _ret_chunk[startINDEX] ;
                      startINDEX++ ;
                  var _rgb_hex_fmt = _ret_chunk[startINDEX] ;
                      startINDEX++ ;
                  var _tag = _ret_chunk[startINDEX] ;
                  
                  var _MSG = "RGB decimal format : " + _rgb_dec_fmt + _glob_crlf ;
                  _MSG += "RGB hex format : " + _rgb_hex_fmt + _glob_crlf ;
                  _MSG += "Tag : " + _tag + _glob_crlf ;
                  circles_lib_output( _output_channel, DISPATCH_INFO, _MSG, _par_1, _cmd_tag );
                  break ;
             }
         }
     }
     else { _b_fail = YES, _error_str = "Missing color format" ; }
