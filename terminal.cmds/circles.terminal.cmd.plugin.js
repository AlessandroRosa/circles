function circles_terminal_cmd_plugin()
{
     ...
     if ( _params.length > 0 )
     {
         ...
         for( var _i = 0 ; _i < _params_array.length ; _i++ )
         {
            ...
            else if ( _p.is_one_of_i( "available", "close", "current", "list", "remotecmds", "open", "set", "send", "target", "var", "varslist" ) ) _cmd_params['action'] = _p.toLowerCase();
            else if ( _p.is_one_of_i( "silent" ) ) _cmd_params['settings'].push( _p ) ;
            else
            {
                if ( _cmd_params['action'] != null )
                {
                    ...
                    {
                      case "available":
               		  if ( !is_array( _cmd_params['settings'][ _cmd_params['action'] ] ) )
      								  _cmd_params['settings'][ _cmd_params['action'] ] = [] ;

      				  _cmd_params['settings'][ _cmd_params['action'] ].push( _p );
                      break ;
                      case "var": _cmd_params['settings']['var'] = _p ; break ;
                      case "open":
                      if ( _cmd_params['settings']['send.params'] == null ) _cmd_params['settings']['send.params'] = [] ;
                      _cmd_params['settings']['send.params'].push( _p );
                      break ;
                      case "set":
                      if ( _cmd_params['settings']['family'] == null ) _cmd_params['settings']['family'] = _p ;
                      else if ( _cmd_params['settings']['def'] == null ) _cmd_params['settings']['def'] = _p ;
                      break ;
                      case "send":
                      if ( _cmd_params['settings']['send.params'] == null ) _cmd_params['settings']['send.params'] = [] ;
                      _cmd_params['settings']['send.params'].push( _p );
         ...
         else if ( !_b_fail )
         {
             ...
             {
             	 case "available" :
                 var _settings = _cmd_params['settings'][ 'available' ] ;
                 ...
                 if ( _plugin_tmp_vars_array['plugin_sel'] != null )
                 {
                    if ( is_array( _cmd_params['settings']['send.params'] ) )
                    {
                       var _src = _plugin_tmp_vars_array['plugin_sel']['orig_family_def'] ;
                       if ( _src != null )
                       {
                          var _famLC = _src.fam.toLowerCase(), _defUC = _src.def.toUpperCase().replace( /[\.\_\-]/g, "" ) ;
                          var _options = _cmd_params['settings']['send.params'] ;
                          var _ret_array = [] ;
                          var _dispatcher_fn = "CIRCLES" + _famLC + _defUC + "remotectrl( _options, null, _ret_array, "+_out_channel+" )" ;
                          var _output = null ;
                          try{ eval( "_output = " + _dispatcher_fn + ";" ) }
                 ...
                 var _fam = _cmd_params['settings']['family'] != null ? _cmd_params['settings']['family'] : "" ;
                 ...
                 if ( is_consistent_string( _fam ) && is_consistent_string( _def ) )
                 {
				   ...
				   if ( _exists )
				   {
					   ...
					   circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<green>Plug-in target has been correctly set to</green> <white>"+_cmd_params['settings']['family']+" "+_cmd_params['settings']['def']+"</white>", _par_1, _cmd_tag );
                 ...
                 if ( _plugin_tmp_vars_array['plugin_sel'] != null )
                 {
                      ...
                      var _var_set = _cmd_params['settings']['var'] ;
