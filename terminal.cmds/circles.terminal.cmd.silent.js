function circles_terminal_cmd_silent()
{
	...
	// this cmd might not have input parameters
    	...
    	_local_cmds_params_array.push( "html", "help", "no", "on", "off", "release", "yes" );
        ...
        for( var _i = 0 ; _i < _params_array.length ; _i++ )
        {
            ...
            else if ( _p.is_one_of_i( "on", "yes" ) ) _params_assoc_array['mode'] = ON ;
            else if ( _p.is_one_of_i( "off", "no" ) ) _params_assoc_array['mode'] = OFF ;
        }
        ...
        {
            var _action = _params_assoc_array['action'], _already = !_glob_terminal_echo_flag == _params_assoc_array['mode'] ? YES : NO ;
            switch( _action )
            {
                case "release":
                ...
                default:
                var _silent_label = "" ;
                switch( _params_assoc_array['mode'] )
                {
                    case YES:
                ...
                if ( _params.length > 0 )
                circles_lib_output( _output_channel, _already ? DISPATCH_INFO : DISPATCH_SUCCESS, _silent_label, _par_1, _cmd_tag, 1 );
                else
                circles_lib_output( _output_channel, DISPATCH_INFO, _silent_label, _par_1, _cmd_tag, 1 );
        
                if ( circles_lib_terminal_batch_script_exists() && _output_channel == OUTPUT_TERMINAL )
          		{
        			_glob_terminal_change = YES ;
        	        circles_lib_output( _output_channel, DISPATCH_INFO, TERMINAL_LABEL_01, _par_1, _cmd_tag, 1 );
        		}
                if ( _params_assoc_array['mode'] != UNDET ) _glob_terminal_echo_flag = !_params_assoc_array['mode'] ;
                break ;
            }
        }
