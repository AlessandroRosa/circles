    ...
    else if ( _params.length > 0 )
    {
        ...
        for( var _i = 0 ; _i < _params_array.length ; _i++ )
        {
            ...
            else if ( _p.testME( _glob_fraction_regex_pattern ) || _p.testME( _glob_float_regex_pattern ) )
            {
                ...
                {
                    _b_fail = YES ; _error_str = "P/Q word for cusp computation shall range from 0 to 1" ;
                }
            }
            ...
            else if ( _p.testME( _glob_cartesian_coords_regex_pattern ) )
            {
                ...
                {
                    var _b_found = NO ;
		            $.each( _params_assoc_array['settings']['points'], function( _i, _pt )
			                {
								if ( _pt.is_equal_to( _new_pt ) ) { _b_found = YES ; return NO ; }
							} ) ;
		...
		else if ( !_b_fail )
		{
		    ...
		    switch( _action )
		    {
		        ...
		        if ( is_string( _frac_str ) )
		        {
				    ...
				    circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, "<white>"+_frac_str+"</white> <lightgray>has been turned to fraction</lightgray> <white>"+_out_frac+"</white>", _par_1, _cmd_tag );
				              
                    if ( _frac_str.testME( _glob_fraction_regex_pattern ) && !_out_frac.strcmp( _frac_str ) )
				    circles_lib_output( _output_channel, DISPATCH_INFO, _frac_str+" has been reduced to "+_out_frac, _par_1, _cmd_tag );
				    ...
				    if ( safe_size( _convergents, 0 ) > 0 )
				    {
				        ...
				        {
				            circles_lib_output( _output_channel, DISPATCH_INFO, "Displaying the convergents in the continued fraction format", _par_1, _cmd_tag );
						...
						else circles_lib_output( _output_channel, DISPATCH_INFO, "Displaying the convergents sequence: "+_convergents.join(","), _par_1, _cmd_tag );
				    }
					else else circles_lib_output( _output_channel, DISPATCH_WARNING, "No output convergents", _par_1, _cmd_tag );
				}
				else circles_lib_output( _output_channel, DISPATCH_WARNING, "Can't compute convergents: missing input fraction", _par_1, _cmd_tag );
		        break ;
		        case "crossratio":
