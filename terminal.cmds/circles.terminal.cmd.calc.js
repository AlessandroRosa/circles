function circles_terminal_cmd_calc()
{
    ...
    else if ( _params.length > 0 )
    {
    	...
    	var _cmd_terms_dict = [];
    		_cmd_terms_dict.push( "all", "bip", "crossratio", "convergents", "release",
        ...
        for( var _i = 0 ; _i < _params_array.length ; _i++ )
        {
            ...
            else if ( _p.is_one_of_i( "html", "fraction" ) ) _cmd_params['html'] = YES ;
            else if ( _p.is_one_of_i( "convergents", "crossratio", "release" ) ) _cmd_params['action'] = _p ;
            else if ( circles_lib_colors_is_def( _p ) ) _cmd_params['settings']['color'] = _p ;
            else if ( _p.testME( _glob_fraction_regex_pattern ) || _p.testME( _glob_float_regex_pattern ) )
            {
                _cmd_params['settings']['frac'] = _p ;
                if ( eval( _p ) < 0 || eval( _p ) > 1 && _cmd_params['action'] == "cusp" )
                {
            ...
            {
				if ( !is_array( _cmd_params['settings']['points'] ) ) _cmd_params['settings']['points'] = [] ;
                ...
                {
                    var _b_found = NO ;
		            $.each( _cmd_params['settings']['points'], function( _i, _pt )
			                {
					...
					else _cmd_params['settings']['points'].push( _new_pt );
				}
		...
		else if ( !_b_fail )
		{
		    ...
		    {
		        case "convergents":
		        var _frac_str = _cmd_params['settings']['frac'] ;
                var _color = safe_string( _cmd_params['settings']['color'], "yellow" );
		        if ( is_string( _frac_str ) )
		        {
				    ...
				    if ( safe_size( _convergents, 0 ) > 0 )
				    {
						...
						else circles_lib_output( _out_channel, DISPATCH_INFO, "Displaying the convergents sequence: "+_convergents.join(","), _par_1, _cmd_tag );
				    }
