    ...
    if ( _params.length > 0 )
    {
        ...
        for( var _i = 0 ; _i < _params_array.length ; _i++ )
        {
			...
			else if ( _p.includes( "," ) && _p.count( "," ) == 3 && _p.split( "," ).length == 4 )
			{
				...
				_params.forEach( function( _c ) {
					_coord = parseFloat( _c ) ;
					if ( isNaN( _coord ) )
					{
						circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, _msg, _par_1, _cmd_tag );
		    ...
		    switch( _action )
		    {
                ...
                else if ( safe_size( _coords_got_em, 0 ) > 0 ) // 4 floating numbers regrouped into left-top and right-bottom points
                {
					...
					if ( _params_assoc_array['left_top_flag'] )
    				{
						...
						_msg = "<lightblue>Rectangle left top corner</lightblue> <lime>has been filled with success</lime> <lightblue>"+_params_assoc_array['left_top'].output("cartesian",_round_to)+"</lightblue>" ;
    					circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, _msg, _par_1, _cmd_tag );
     				}
     				...
     				if ( _params_assoc_array['right_bottom_flag'] )
     				{
     					...
     					_msg = "<lightblue>Rectangle right bottom corner</lightblue> <lime>has been filled with success and set at</lime> <lightblue>"+_params_assoc_array['right_bottom'].output("cartesian",_round_to)+"</lightblue>" ;
     					circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, _msg, _par_1, _cmd_tag );
     				}
