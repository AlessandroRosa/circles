function circles_terminal_cmd_word()
{
     ...
     if ( _params.length > 0 )
     {
         ...
         _cmd_params['all'] = NO ;
         _cmd_params['connect'] = NO ;
         _cmd_params["item"] = ITEMS_SWITCH_SEEDS ;
         _cmd_params['objectref'] = "" ;
         _cmd_params['list'] = NO ;
         ...
         // pre-scan for levenshtein correction
    		 var _cmd_terms_dict = [];
    				 _cmd_terms_dict.push( "all", "calc", "connect", "copy", "displacedset", "draw", "draworbits", "fixedpoints",
         ...
         for( var _i = 0 ; _i < _up_to_index ; _i++ )
         {
              ...
              else if ( _p.stricmp( "copy" ) ) _cmd_params["copy"] = YES ;
              else if ( _p.testME( _glob_cartesian_coords_regex_pattern ) )
                   _cmd_params["pts"].push( read_2D_point( _p ) ) ;
              else if ( _p.is_one_of( "draworbits", "markpoints", "rle", "sink", "source" ) )
									 _cmd_params['extras'].push( _p.toLowerCase() );
              else if ( _p.stricmp( "trace" ) )
              {
                   _cmd_params['extras'].push( _p.toLowerCase() );
                   if ( _cmd_params['action'].length == 0 ) _cmd_params['action'] = _p.toLowerCase();
                   else if ( _cmd_params['action'].stricmp( "treescan" ) ) _cmd_params['service'] = _p.toLowerCase();
              }
              else if ( _p.includes( "@" ) && _p.count( "@" ) == 1 ) _cmd_params['objectref'] = _p ;
              ...
              else if ( _p.is_one_of_i( "fixedpoints", "orbit", "lastpt" ) ) _cmd_params['service'] = _p.toLowerCase();
              else if ( _p.length > 0 && _cmd_params['action'].is_one_of_i( "trace" ) ) _symbols_array.push( _p );
              else if ( _p.is_one_of_i( "all", "connect", "html", "list" ) ) _cmd_params[''+_p.toLowerCase()] = YES ;
              else if ( _p.toLowerCase().start_with( "w:" ) || _p.testME( _glob_pqword_regex_pattern ) ) _cmd_params['word'].push( _p.replaceAll( "w:", "" ) );
              else if ( _p.testME( _glob_region_regex_pattern ) )
              {
                 ...
                 {
                    _cmd_params['service'] = "region" ;
                    _cmd_params['region'] = new rect( safe_float( _p[0], 0 ), safe_float( _p[1], 0 ),
              ...
              {
                 if ( _cmd_params['bordercolor'] == null ) _cmd_params['bordercolor'] = _p ;
                 else if ( _cmd_params['fillcolor'] == null ) _cmd_params['fillcolor'] = _p ;
              }
              else if ( _p.toLowerCase().start_with( "startpt:" ) && _p.includes_i( "(", ",", ")" ) &&
                       ( _glob_method.is_one_of( METHOD_ALGEBRAIC ) &&
                       _cmd_params['action'].stricmp( "track" ) )
                      )
              {
                  ...
                  _cmd_params['start'] = new point( _x, _y );
              }
              else
              {
                  if ( _cmd_params['action'].strcmp( "treescan" ) )
                  {
                     if ( _p.start_with_i( "startword:" ) ) _cmd_params['treescan_startword'] = _p.replaceAll( "startword:", "" );
                     else if ( _p.start_with_i( "startpt:" ) )
                     {
                       _p = _p.replaceAll( "startpt:", "" );
                       if ( _p.testME( _glob_cartesian_coords_regex_pattern ) ) _cmd_params['treescan_startpt'] = _p ;
                     }
                     else if ( _p.start_with_i( "tolerance:" ) ) _cmd_params['treescan_tolerance'] = safe_float( _p.replaceAll( "tolerance:", "" ), 0 );
                     else if ( _p.start_with_i( "depth:" ) )
                     {
                       _p = _p.replaceAll( "depth:", "" );
                       _cmd_params['treescan_depth'] = _p.stricmp( "loop" ) ? UNDET : safe_int( _p, 1 );
                     }
                     else if ( _p.start_with_i( "trace:" ) )
                     {
                       _cmd_params['treescan_trace'] = safe_float( _p.replaceAll( "trace:", "" ), 0 );
                       _cmd_params['service'] = "trace" ;
                     }
                  }
     ...
     else if ( !_b_fail )
     {
        ...
        var _all = _cmd_params['all'] != null ? _cmd_params['all'] : NO ;
         ...
         else if ( _cmd_params['word'].length > 0 ) _symbols_array = _cmd_params['word'].clone();
         ...
         if ( _cmd_params['bordercolor'] == null ) _cmd_params['bordercolor'] = _glob_draw_seed_color ;
         if ( _cmd_params['fillcolor'] == null ) _cmd_params['fillcolor'] = _glob_fill_seed_color ;
         switch( _action )
         {
             case "release":
                     ...
                     $.each( _symbols_array, function( _i, _word )
                             {
                                ...
                                if ( _word.testME( _glob_pqword_regex_pattern ) )
                                {
                                   ...
                                   circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "input p/q word '<lightblue>"+_word+"</lightblue>' resolved into '<snow>"+_resolved_pq_word+"</snow>'", _par_1, _cmd_tag );
                                ...
                                else
                                {
                                   ...
                                   if ( _is_repetend )
                                   {
                                      circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "Found repetend '<lightblue>"+ $.terminal.escape_brackets(_word)+"</lightblue>' resolved into '<snow>"+_solved_repetend+"</snow>'", _par_1, _cmd_tag );
             ...
             {
                circles_lib_output( _out_channel, DISPATCH_INFO, "The displaced set is the union of non-trivial orbits from all permutations of '"+_word+"'", _par_1, _cmd_tag );
                circles_lib_output( _out_channel, DISPATCH_INFO, "Computing the displaced set of input word '"+_word+"'", _par_1, _cmd_tag );
                var _start_pt = null, _orbit ;
                if ( safe_size( _cmd_params['pts'] ) == 0 )
                {
                   circles_lib_output( _out_channel, DISPATCH_INFO, "Missing explicit input start point: fixed point of '"+_word+"' will be assumed", _par_1, _cmd_tag );
                   circles_lib_output( _out_channel, DISPATCH_INFO, "Attempting to their computation", _par_1, _cmd_tag );
                   var _fp_def = "" ;
                   if ( _cmd_params['extras'].includes( "sink" ) ) _fp_def = "sink" ;
                   else if ( _cmd_params['extras'].includes( "source" ) ) _fp_def = "source" ;
                   else
                   {
                      _fp_def = "sink" ;
                      circles_lib_output( _out_channel, DISPATCH_INFO, "Missing input fixed point type: '"+_fp_def+"' will be assumed as default type", _par_1, _cmd_tag );
                   }
                   
                   var _items_array = _glob_items_switch == ITEMS_SWITCH_GENS ? _glob_gens_array : _glob_seeds_array ;
                   var _ret_chunk = circles_lib_word_fixedpoints_get( _word, _items_array, NO, _out_channel ) ;
                   ...
                   switch( _ret_id )
                   {
                       ...
                       circles_lib_output( _out_channel, DISPATCH_WARNING, "Missing seeds", _par_1, _cmd_tag );
                       break ;
                       case CIRCLES_MISSING_INPUT:
                       circles_lib_output( _out_channel, DISPATCH_WARNING, "Missing input word", _par_1, _cmd_tag );
                       break ;
                       case CIRCLES_MISSING_SEEDS:
                       circles_lib_output( _out_channel, DISPATCH_WARNING, "Missing alphabet", _par_1, _cmd_tag );
                       break ;
                       default:
                       ...
                       {
                           circles_lib_output( _out_channel, DISPATCH_INFO, "Start point has been taken from the "+_fp_def+" fixed", _par_1, _cmd_tag );
                           circles_lib_output( _out_channel, DISPATCH_INFO, "point of the Mobius map of word '"+_word+"'", _par_1, _cmd_tag );
                           circles_lib_output( _out_channel, DISPATCH_INFO, "Now set to " + _start_pt.output( "cartesian" ), _par_1, _cmd_tag );
                           circles_lib_output( _out_channel, DISPATCH_INFO, _glob_crlf, _par_1, _cmd_tag );
                       }
                       else
                       {
                           circles_lib_output( _out_channel, DISPATCH_WARNING, "No "+_fp_def+" fixed point has been detected for the Mobius map of word '"+_word+"'", _par_1, _cmd_tag );
                           circles_lib_output( _out_channel, DISPATCH_WARNING, "The computation of the displaced set has been halted", _par_1, _cmd_tag );
                       }
                       break ;
                   }
                }
                else if ( is_point( _cmd_params['pts'][0] ) )
                {
                   _start_pt = _cmd_params['pts'][0] ;
                   circles_lib_output( _out_channel, DISPATCH_INFO, "Start point is " + _start_pt.output( "cartesian" ), _par_1, _cmd_tag );
                }
                ...
                if ( is_point( _start_pt ) )
                {
                   ...
                   var _context = _glob_wplane_work_layer_pointer.getContext( _glob_canvas_ctx_2D_mode ) ;
                   if ( _cmd_params["copy"] ) _glob_storage["points"]["displacedset"] = [] ;

                   if ( _cmd_params["markpoints"] )
                   circles_lib_output( _out_channel, DISPATCH_INFO, "'Markpoint' flag is active", _par_1, _cmd_tag );
                   ...
                   for( var _w = 0 ; _w < _word.length ; _w++ )
                   {
                      ...
                      if ( _cmd_params['extras'].includes( "draworbits" ) )
                      ...
                      _word = circles_lib_word_cyclic_permutation( _word ) ;
		               ...
		               if ( _cmd_params['extras'].includes( "markpoints" ) )
		               {
		               		 var _t = null ;
		               		 $.each( _displaced_set,
		               		 				 function( _i, _pt )
                   ...
                   if ( _cmd_params["copy"] )
                   {
                      ...
                      circles_lib_output( _out_channel, DISPATCH_INFO, "The displaced set has been copied into the storage space at 'points@displacedset'", _par_1, _cmd_tag );
									 }
                }
             }
             break ;
             ...
             else if ( safe_size( _cmd_params['objectref'], 0 ) == 0 )
             {
                _b_fail = 1 ;
             ...
             {
                var _unpacked_ref = _cmd_params['objectref'].split( "@" );
                var _index = Math.max( 0, safe_int( _unpacked_ref[0], 0 ) );
                var _object_entity = ( _unpacked_ref[1] + "" ).trim().toLowerCase();
                var _mm = circles_lib_word_mobiusmap_get( _word, _glob_seeds_array, _out_channel );
                      ...
                      $.each( _symbols_array, function( _i, _word )
                              {
                                  ...
                                  else
                                  {
                                      ...
                                      if ( _is_repetend )
                                      {
                                           circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "Found repetend '<lightblue>"+_word+"</lightblue>' resolved into '<snow>"+_solved_repetend+"</snow>'", _par_1, _cmd_tag );
                                           _word = _solved_repetend ;
                                      }
                                  }
                                  ...
                                  if ( is_mobius_map( _mm ) )
                                  {
                                      ...
                                      if ( !_glob_gens_model_array.includes( _word ) && !_glob_gens_model_array.includes( circles_lib_word_inverse_get( _word ) ) )
                                      {
                                          ...
                                          _glob_gens_array.push( new item_obj( _mm, _cc, _sc, circles_lib_word_inverse_get( _word ), 0,
                                           ...
                                           circles_lib_output( _out_channel, DISPATCH_SUCCESS, "Generator has been computed from word '"+_word+"' with success", _par_1, _cmd_tag );
                                           circles_lib_output( _out_channel, DISPATCH_INFO, "Inverse generator has been also computed with success", _par_1, _cmd_tag );
                  ...
                  $.each( _symbols_array, function( _i, _str )
                          {
                              ...
                              if ( _cmd_params['extras'].includes( "rle" ) && _case.is_one_of( 2, 3 ) )
                              {
                                   ...
                                   circles_lib_output( _out_channel, DISPATCH_WARNING, "Fail to compute trace: missing registered seeds", _par_1, _cmd_tag );
		                                   ...
		                                   {
		                                        _tr = _mm.trace();
		                                        if ( _cmd_params['extras'].includes( "rle" ) )
		                                        {
             ...
             {
                  circles_lib_output( _out_channel, DISPATCH_INFO, "Testing the input word '"+_word+"'", _par_1, _cmd_tag );
                  circles_lib_output( _out_channel, DISPATCH_INFO, "It is "+_word.length+" char"+( _word.length == 1 ? "" : "s" )+" long", _par_1, _cmd_tag );
                  var _is_cyclically_reduced = circles_lib_word_is_cyclically_reduced( _word );
                  circles_lib_output( _out_channel, DISPATCH_INFO, "It is "+( !_is_cyclically_reduced ? "not " : "" )+"cyclically reduced", _par_1, _cmd_tag );
                  var _is_reduced = circles_lib_word_is_reduced( _word, _glob_alphabet );
                  circles_lib_output( _out_channel, DISPATCH_INFO, "It is "+( !_is_reduced ? "not " : "" )+"reduced", _par_1, _cmd_tag );
                  
                  if ( !_is_reduced )
                  {
										  var _reduced_word = circles_lib_word_reduce( _word, _glob_alphabet );
		                  circles_lib_output( _out_channel, DISPATCH_INFO, "It can be reduced to '"+_reduced_word+"'", _par_1, _cmd_tag );
		                  circles_lib_output( _out_channel, DISPATCH_INFO, "The resulting reduced word '"+_reduced_word+"' is "+_reduced_word.length+" char"+( _reduced_word.length == 1 ? "" : "s" )+" long", _par_1, _cmd_tag );
		                  _word = _reduced_word ;
									}
									
									var _mm = circles_lib_word_mobiusmap_get( _word, _glob_seeds_array, _out_channel ) ;
                  var _items_array = _glob_items_switch == ITEMS_SWITCH_GENS ? _glob_gens_array : _glob_seeds_array ;
									var FF = circles_lib_word_fixedpoints_get( _word, _items_array, NO, _out_channel ) ;
									
                  circles_lib_output( _out_channel, DISPATCH_INFO, _glob_crlf.repeat(2), _par_1, _cmd_tag );
                  circles_lib_output( _out_channel, DISPATCH_INFO, "This is the resulting Mobius map from word '"+_word+"'", _par_1, _cmd_tag );
                  circles_lib_output( _out_channel, DISPATCH_INFO, _mm.output( _glob_crlf ), _par_1, _cmd_tag );
		                   ...
		                   circles_lib_output( _out_channel, DISPATCH_INFO, "Found " + _n_pts + " fixed point" + ( _n_pts == 1 ? "" : "s" ), _par_1, _cmd_tag );
											 $.each( _ret_pts,
											 				 function( _i, _chunk )
											 				 {
																		circles_lib_output( _out_channel, DISPATCH_INFO, "Fixed point #"+(_i+1) + " - " + _chunk[2], _par_1, _cmd_tag );
																		circles_lib_output( _out_channel, DISPATCH_INFO, _chunk[0].formula(YES,YES,_round_to), _par_1, _cmd_tag );
															 }
											 			 ) ;
									}
             }
                      ...
                      $.each( _symbols_array,
                              function( _i, _symbol )
                              {
                                  ...
                                  if ( _is_rep == YES )
                                  {
                                       ...
                                       circles_lib_output( _out_channel, DISPATCH_INFO, "Found repetend syntax in "+$.terminal.escape_brackets(_symbol)+": resolved to " + _rep_str, _par_1, _cmd_tag );
                                       _symbol = _rep_str ;
                                  }

                                  if ( circles_lib_word_check( _symbol, _alphabet ) )
                                  {
                                      _mm = circles_lib_word_mobiusmap_get( _symbol, _glob_seeds_array, _out_channel );
                                      if ( is_mobius_map( _mm ) )
                                      {
                                           _tr = _mm.trace();
             ...
             case "track":
             var _bFOUND = 1 ;
             var _input_word = _cmd_params['word'][0], _symbol ;
             if ( _cmd_params['word'].length > 1 )
             {
                 circles_lib_output( _out_channel, DISPATCH_WARNING, "Just one word can be tracked at once", _par_1, _cmd_tag );
                 circles_lib_output( _out_channel, DISPATCH_INFO, "Now processing word '"+_input_word+"'", _par_1, _cmd_tag );
             }
             ...
             if ( _is_repetend )
             {
                 ...
                 circles_lib_output( _out_channel, DISPATCH_INFO, "Found repetend syntax in '"+_encoded+"'", _par_1, _cmd_tag );
                 circles_lib_output( _out_channel, DISPATCH_INFO, "Resolved repetend '"+_encoded+"' into '"+_solved_repetend+"'", _par_1, _cmd_tag );
             }
             ...
             else if ( _cmd_params['service'].strcmp( "orbit" ) && _cmd_params['start'] == null )
             {
             ...
             else
             {
                 ...
                 switch( _cmd_params['service'] )
                 {
                       case "orbit":
                       if ( _glob_method == METHOD_INVERSION )
                       {
                           var _ret_chunk = circles_lib_draw_word_inversion( _freedraw_canvas.getContext( _glob_canvas_ctx_2D_mode ), wplane_sm, null, YES, _input_word, YES, _out_channel );
                           ...
                           if ( _cmd_params['list'] && _ret_orbit != null )
                           {
                                var _padding_len = 24 ;
                                ...
                                circles_lib_output( _out_channel, DISPATCH_INFO, _o + " point" + ( ( _o != 1 ) ? "s" : "" ) + " tracked", _par_1, _cmd_tag );
                                circles_lib_output( _out_channel, DISPATCH_INFO, _n_col + _real_col + _imag_col + _type_col, _par_1, _cmd_tag );
                                var _pt, _x, _y, _n ;
                                for( var _i = 0 ; _i < _o ; _i++ )
                                {
                                     ...
                                     circles_lib_output( _out_channel, DISPATCH_INFO, _n + _x + _y, _par_1, _cmd_tag );
                                }

				                        if ( _cmd_params['dump'] &&
															 			 _cmd_params['dump_array'].length > 0 )
				                        {
				                            ...
				                            var _basename = basename( _filename );
				                			      var _extension = ( _filename.includes( "." ) ) ? _filename.split( ".").get_last() : "" ;
									 								  ...
									 								  _cmd_params['dump_array'] = is_array( _cmd_params['dump_array'] ) ? _cmd_params['dump_array'][0] : "circles.track.word.txt" ;
                       ...
                       {
                           var _start_x = safe_float( _cmd_params['start'].x, 0 );
                           var _start_y = safe_float( _cmd_params['start'].y, 0 );
                           var _items_array = _glob_items_switch == ITEMS_SWITCH_GENS ? _glob_gens_array : _glob_seeds_array ;
                           var _ret_chunk = circles_lib_draw_orbit_from_word( _freedraw_canvas.getContext( _glob_canvas_ctx_2D_mode ), wplane_sm, _items_array,
                                                                      ...
                                                                      _input_word, YES, _cmd_params['connect'],
																																			YES, _cmd_params['bordercolor'], _cmd_params['fillcolor'],
                                                                      YES, _glob_terminal_echo_flag, _glob_drawentity, _out_channel );
                           var _ret_id = is_array( _ret_chunk ) ? safe_int( _ret_chunk[0], NO ) : NO ;
                           ...
                           if ( _cmd_params['list'] && _ret_orbit != null )
                           {
                                ...
                                circles_lib_output( _out_channel, DISPATCH_INFO, _o + " point" + ( ( _o != 1 ) ? "s" : "" ) + " tracked", _par_1, _cmd_tag );
                                circles_lib_output( _out_channel, DISPATCH_INFO, _n_col + _real_col + _imag_col, _par_1, _cmd_tag );
                                var _pt, _x, _y, _n ;
                                for( var _i = 0 ; _i < _o ; _i++ )
                                {
                                     _pt = _ret_orbit[_i] ;
                       ...
                       {
					                  var _items_array = _glob_items_switch == ITEMS_SWITCH_GENS ? _glob_gens_array : _glob_seeds_array ;
                            var _ret_chunk = circles_lib_word_fixedpoints_get( _input_word, _items_array, _cmd_params['all'], _out_channel );
                            var _ret_id = safe_int( _ret_chunk[0], RET_ERROR );
                            ...
                            else
                            {
                                ...
                                if ( S > 0 )
                                {
                                    ...
                                    if ( _cmd_params['list'] ) circles_lib_output( _out_channel, DISPATCH_INFO, _header, _par_1, _cmd_tag );
                                    ...
                                    for( var s = 0 ; s < S ; s++ )
                                    {
                                         ...
                                         if ( _cmd_params['list'] )
                                         circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, _list_entry, _par_1, _cmd_tag );
                                    }
             ...
             if ( _cmd_params['treescan_startword'] == null ) _err_array.push( "Missing 'startword' param for treescan" );
             if ( _cmd_params['treescan_depth'] == null ) _err_array.push( "Missing 'depth' param for treescan" );
             if ( _cmd_params['service'].is_one_of_i( "orbit", "lastpt" ) )
             {
                 if ( _cmd_params['treescan_startpt'] == null ) _err_array.push( "Missing 'startpt' param for "+_cmd_params['service']+" treescan" );
             }
             else if ( _cmd_params['service'].is_one_of_i( "trace" ) )
             {
                 if ( _cmd_params['treescan_tolerance'] == null ) _err_array.push( "Missing 'tolerance' value for treescan" );
                 if ( _cmd_params['treescan_trace'] == null ) _err_array.push( "Missing 'trace' value for treescan" );
             }

             if ( _out_channel != OUTPUT_TERMINAL )
                  circles_lib_output( _out_channel, DISPATCH_WARNING, "Word treescan runs in console mode exclusively", _par_1, _cmd_tag );
             else if ( _err_array.length > 0 )
             {
                  _b_fail = YES, _error_str = _err_array.join( _glob_crlf );
             }
             else if ( _cmd_params['treescan_startword'].length >= _cmd_params['treescan_depth']
                       && _cmd_params['treescan_depth'] != UNDET )
             {
                  _b_fail = YES, _error_str = "Incoherent 'startword' length and 'depth' params: the former can't be equal or larger than the latter" ;
             }
             else if ( safe_string( _cmd_params['service'] ).length == 0 )
             {
             ...
             else if ( safe_int( _cmd_params['treescan_depth'], UNDET ) == UNDET &&
                       safe_string( _cmd_params['service'] ).stricmp( "orbit" ) )
             {
             ...
             {
                  var _startword = _cmd_params['treescan_startword'] ;
                  var _startpt = _cmd_params['treescan_startpt'] ;
                  _startpt = _startpt == null ? "0,0" : _startpt.replaceAll( [ "(", ")" ], "" );
                  var _tolerance = _cmd_params['treescan_tolerance'] ;
                  var _depth = _cmd_params['treescan_depth'] ;
                  var _trace = _cmd_params['treescan_trace'] ;
                  var _list = safe_int( _cmd_params['list'], NO );
                  ...
                  circles_lib_output( _out_channel, DISPATCH_INFO, "Resuming input params", _par_1, _cmd_tag );
                  if ( _startpt != null )
                  circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<snow>Start point</snow> <lightblue>"+_startpt+"</lightblue>", _par_1, _cmd_tag );
                  circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<snow>Start word</snow> <lightblue>"+_startword+"</lightblue>", _par_1, _cmd_tag );
                  if ( _tolerance != null )
                  circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<snow>Tolerance</snow> <lightblue>"+_tolerance+"</lightblue>", _par_1, _cmd_tag );
                  circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<snow>Depth</snow> <lightblue>"+( _depth == UNDET ? "Loop" : _depth )+"</lightblue>", _par_1, _cmd_tag );
                  if ( _trace != null )
                  circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<snow>Trace</snow> <lightblue>"+_trace+"</lightblue>", _par_1, _cmd_tag );
                  if ( _depth == UNDET ) circles_lib_output( _out_channel, DISPATCH_WARNING, "Looping mode activated: type 'stop' to break the tree scan running", _par_1, _cmd_tag );
                  circles_lib_output( _out_channel, DISPATCH_INFO, "Treescan start ...", _par_1, _cmd_tag );
                  circles_lib_output( _out_channel, DISPATCH_WARNING, "Type 'stop' to terminate this process", _par_1, _cmd_tag );
                  if ( _cmd_params["copy"] && _cmd_params['service'].stricmp( "orbit" ) )
                  circles_lib_output( _out_channel, DISPATCH_INFO, "Copy option unavailable for 'orbit' service", _par_1, _cmd_tag );
                  ...
                  _code_array.push( $.getScript( MULTITHREAD_FOLDER + "multithread.treescan.worker.js", function( response, status, jqxhr ) { _load_failure = status.toLowerCase().stricmp( "success" ) ? NO : YES ; } ).responseText );
		                  ...
		                  _glob_inline_worker.init_vars( { out_channel : _out_channel,
		                                   ...
		                                   trace : _trace,
		                                   region : _cmd_params['region'],
		                                   service : _cmd_params['service'],
		                                   symbols : _symbols.join( "@" ),
		                                   inverse_symbols : _inverse_symbols.join( "@" ),
		                                   alphabet : _alphabet.join( "@" ),
		                                   list : _list,
		                                   bordercolor : _cmd_params['bordercolor'],
		                                   fillcolor : _cmd_params['fillcolor'],
		                                   crlf : _glob_crlf,
		                                   copy : _cmd_params["copy"],
