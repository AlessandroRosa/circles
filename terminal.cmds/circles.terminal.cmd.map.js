function circles_terminal_cmd_map()
{
     ...
     if ( _params.length > 0 )
     {
         ...
         for( var _i = 0 ; _i < _params_array.length ; _i++ )
         {
              ...
              else if ( _p.start_with_i( "tag:" ) ) _cmd_params['tag'] = _p.replaceAll( "tag:", "" );
              else if ( _p.start_with_i( "newtag:" ) )
              {
                  _cmd_params['newtag'] = _p.replaceAll( "newtag:", "" ).trim();
                  _cmd_params['action'] = "newtag" ;
              }
              else if ( _p.start_with_i( "direct:" ) ) _cmd_params['formula']['direct'] = _p.replaceAll( "direct:", "" ).trim();
              else if ( _p.start_with_i( "inverse:" ) ) _cmd_params['formula']['inverse'] = _p.replaceAll( "inverse:", "" ).trim();
              else if ( _p.start_with_i( "notes:" ) ) _cmd_params['notes'] = _p.replaceAll( "notes:", "" );
              else if ( _p.is_one_of_i( "add", "clean", "delete", "generator", "list", "modify", "release" ) )
              {
                  _cmd_params['action'] = _p ;
                  if ( _p.stricmp( "generator" ) ) _cmd_params['class'] = FN_DEF_MOBIUS ;
              }
              else if ( _p.start_with_i( "symbol:" ) ) _cmd_params['symbol'] = _p.replaceAll( "symbol:", "" );
              else if ( _p.start_with_i( "class:" ) )
              {
                  var _class = _p.replaceAll( "class:", "" );
                  if ( _class.stricmp( "mobius" ) ) _cmd_params['class'] = FN_DEF_MOBIUS ;
                  else if ( _class.stricmp( "formula" ) ) _cmd_params['class'] = FN_DEF_FORMULA ;
                  else
                  {
                      _cmd_params['class'] = FN_DEF_NONE ;
              ...
              else if ( _cmd_params['action'].is_one_of( "add", "generator" ) && _p.is_one_of_i( "init" )  )
                  _cmd_params['service'].push( _p );
              else if ( _cmd_params['action'].is_one_of( "add", "generator" ) &&
                        _cmd_params['class'] == FN_DEF_MOBIUS )
              {
                   var _n = ( _p.includes_i( "," ) ) ? _p.split( "," ).length : 1 ;
                   if ( _n == 1 ) _cmd_params['formula'].push( _p );
                   else if ( _n == 4 )
                   {
                      var _arr = _p.split( "," );
                      _cmd_params['mobius']['a'] = _arr[0].trim();
                      _cmd_params['mobius']['b'] = _arr[1].trim();
                      _cmd_params['mobius']['c'] = _arr[2].trim();
                      _cmd_params['mobius']['d'] = _arr[3].trim();
                   }
              }
              else if ( _p.start_with_i( "a:" ) ) _cmd_params['mobius']['a'] = _p.replaceAll( "a:", "" ).trim();
              else if ( _p.start_with_i( "b:" ) ) _cmd_params['mobius']['b'] = _p.replaceAll( "b:", "" ).trim();
              else if ( _p.start_with_i( "c:" ) ) _cmd_params['mobius']['c'] = _p.replaceAll( "c:", "" ).trim();
              else if ( _p.start_with_i( "d:" ) ) _cmd_params['mobius']['d'] = _p.replaceAll( "d:", "" ).trim();
         ...
         else if ( !_b_fail )
         {
           ...
           var _class = safe_int( _cmd_params['class'], 0 );
           switch( _action )
           {
                case "add":
                var _warns_array = [] ;
                ...
                var _mobius_map_str = _cmd_params['formula'].join( "" );
                ...
                {
                    if ( _op_code == 2 ) circles_lib_output( _out_channel, DISPATCH_INFO, "Found one map already recorded and with symbol '"+_tag+"'", _par_1, _cmd_tag );
                    var _msg = "Attempt to "+( _op_code == 1 ? "add" : "update" )+" a new Mobius map '"+_tag+"'" ;
                    circles_lib_output( _out_channel, DISPATCH_INFO, _msg, _par_1, _cmd_tag );

                    var _mob_n = _cmd_params['mobius'].size_associative();
                    if ( _mob_n == 4 )
                    {
                        if ( safe_size( _cmd_params['mobius']['a'], 0 ) == 0 ) _warns_array.push( "Missing param 'a' formula" );
                        if ( safe_size( _cmd_params['mobius']['b'], 0 ) == 0 ) _warns_array.push( "Missing param 'b' formula" );
                        if ( safe_size( _cmd_params['mobius']['c'], 0 ) == 0 ) _warns_array.push( "Missing param 'c' formula" );
                        if ( safe_size( _cmd_params['mobius']['d'], 0 ) == 0 ) _warns_array.push( "Missing param 'd' formula" );
                    }
                    else if ( safe_size( _mobius_map_str, 0 ) > 0 )
                    {
                        ...
                        {
                            circles_lib_output( _out_channel, DISPATCH_INFO, "Found input param 'a' as " + _mobius_params_array[0], _par_1, _cmd_tag );
                            circles_lib_output( _out_channel, DISPATCH_INFO, "Found input param 'b' as " + _mobius_params_array[1], _par_1, _cmd_tag );
                            circles_lib_output( _out_channel, DISPATCH_INFO, "Found input param 'c' as " + _mobius_params_array[2], _par_1, _cmd_tag );
                            circles_lib_output( _out_channel, DISPATCH_INFO, "Found input param 'd' as " + _mobius_params_array[3], _par_1, _cmd_tag );

                            _cmd_params['mobius']['a'] = _mobius_params_array[0] ;
                            _cmd_params['mobius']['b'] = _mobius_params_array[1] ;
                            _cmd_params['mobius']['c'] = _mobius_params_array[2] ;
                            _cmd_params['mobius']['d'] = _mobius_params_array[3] ;
                        }
                        else _warns_array.push( "Syntax error in the input Mobius map" );
                    }
                    else _warns_array.push( "Missing 'input' for Mobius map" );
                }
                else if ( _cmd_params['class'] == FN_DEF_FORMULA )
                {
                    if ( safe_size( _cmd_params['formula']['direct'], 0 ) == 0 ) _warns_array.push( "Missing param 'a' formula" );
                    if ( safe_size( _cmd_params['formula']['inverse'], 0 ) == 0 ) _warns_array.push( "Missing param 'b' formula" );
                }
                ...
                else
                {
                    ...
                    if ( _cmd_params['class'] == FN_DEF_MOBIUS )
                    _glob_maps[ _tag ] = [ NO, _cmd_params['class'],
                                               _cmd_params['mobius']['a'],
                                               _cmd_params['mobius']['b'],
                                               _cmd_params['mobius']['c'],
                                               _cmd_params['mobius']['d'],
                                               ( _cmd_params['notes'] != null ) ? _cmd_params['notes'] : ""
                                           ] ;
                    else if ( _cmd_params['class'] == FN_DEF_FORMULA )
                    _glob_maps[ _tag ] = [ NO, _cmd_params['class'],
                                               _cmd_params['formula']['direct'],
                                               _cmd_params['formula']['inverse'],
                                               ( _cmd_params['notes'] != null ) ? _cmd_params['notes'] : ""
                                           ] ;

                    circles_lib_output( _out_channel, DISPATCH_SUCCESS, "Map '"+_tag+"' has been "+_op_str+" with success", _par_1, _cmd_tag );
                }
                break ;
                case "clean":
                function _clean_list()
                {
                    ...
                    if ( _smaps_n == 0 ) circles_lib_output( _out_channel, DISPATCH_SUCCESS, "The maps list is already empty", _par_1, _cmd_tag );
                    else
                    {
                         ...
                         circles_lib_output( _out_channel, ( _n_cleaned == 0 ) ? DISPATCH_INFO : DISPATCH_SUCCESS, ( _n_cleaned == 0 ) ? "No maps cleaned" : ( _n_cleaned + " map" + ( _n_cleaned == 1 ? "" : "s" ) + " cleaned" ), _par_1, _cmd_tag );
                    }
                }
                ...
                break ;
                case "generator":
                var _formula = _cmd_params['formula'] ;
                var _symbol = _cmd_params['symbol'] ;
                ...
                if ( !_auto_symbol )
                {
                     ...
                     circles_lib_output( _out_channel, DISPATCH_INFO, "Missing input symbol: automatically assigned tag '"+_symbol+"'", _par_1, _cmd_tag );
                }
                ...
                {
                    if ( _auto_symbol ) circles_lib_output( _out_channel, DISPATCH_INFO, "Found more input formulas: tags will be automatically assigned", _par_1, _cmd_tag );
                    $.each( _formula,
                            function( _i, _mobius_map_str )
                            {
                                ...
                                if ( safe_size( _mobius_params_array, 0 ) > 0 )
                                {
                                     ...
                                     if ( _a != null && _b != null && _c != null && _d != null )
                                     {
                                         ...
                                         if ( _use_inverse )
                                         {
                                              circles_lib_output( _out_channel, DISPATCH_INFO, "Current method is " + circles_lib_method_get_def( _glob_method ), _par_1, _cmd_tag );
                                              circles_lib_output( _out_channel, DISPATCH_INFO, "Inverse maps will be attempted to be declared", _par_1, _cmd_tag );
                                         }
                                         ...
                                         else
                                         {
                                            ...
                                            {
                                                _symbol = circles_lib_alphabet_suggest_symbol( _glob_seeds_array, CAPS_LETTER );
                                                circles_lib_output( _out_channel, DISPATCH_INFO, "Automatically assigned tag '"+_symbol+"'", _par_1, _cmd_tag );
                                            }

                                            _inv_symbol = _use_inverse ? circles_lib_word_inverse_get( _symbol ) : "" ;
                                            circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<peacock>>> Added a new generator with symbol '"+_symbol+"'</peacock>", _par_1, _cmd_tag );
                                            _new_n++ ;
                                         ...
                                         else
                                         {
                                            if ( _use_inverse )
                                            {
                                                circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<peacock>>> Added a inverse generator with symbol '"+_inv_symbol+"'</peacock>", _par_1, _cmd_tag );
                                                _new_n++ ;
                      ...
                      if ( _cmd_params['service'].includes_i( "init" ) )
                      {
                          circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<peacock>>>Gens have been init</peacock>", _par_1, _cmd_tag );
                          _map_cmd_init( _par_1, _out_channel );
                      }
                }
                break ;
                case "list":
                ...
                {
                    if ( _b_fail ) circles_lib_output( _out_channel, DISPATCH_WARNING, "The maps list is empty", _par_1, _cmd_tag );
                }
                break ;
                case "modify":
                var _warns_array = [] ;
                ...
                else
                {
                    ...
                    {
                        if ( _cmd_params['mobius']['a'] != null )
                        _sp[2] = _cmd_params['mobius']['a'] ;
                        if ( _cmd_params['mobius']['b'] != null )
                        _sp[3] = _cmd_params['mobius']['b'] ;
                        if ( _cmd_params['mobius']['c'] != null )
                        _sp[4] = _cmd_params['mobius']['c'] ;
                        if ( _cmd_params['mobius']['d'] != null )
                        _sp[5] = _cmd_params['mobius']['d'] ;
                        if ( _cmd_params['notes'] != null )
                        _sp[6] = _cmd_params['notes'] ;
                        circles_lib_output( _out_channel, DISPATCH_SUCCESS, "Map '"+_tag+"' has been modified with success", _par_1, _cmd_tag );
                    }
                    else if ( _sp != null && _sp[1] == FN_DEF_FORMULA )
                    {
                        if ( _cmd_params['formula']['direct'] != null )
                        _sp[2] = _cmd_params['formula']['direct'] ;
                        if ( _cmd_params['formula']['inverse'] != null )
                        _sp[3] = _cmd_params['formula']['inverse'] ;
                        if ( _cmd_params['notes'] != null )
                        _sp[4] = _cmd_params['notes'] ;
                        circles_lib_output( _out_channel, DISPATCH_SUCCESS, "Map '"+_tag+"' has been modified with success", _par_1, _cmd_tag );
                    }
                    else
                    {
                ...
                {
                    var _sp = _glob_maps[ _tag ] ;
                    var _newtag = _cmd_params['newtag'] ;
                    _glob_maps[ _newtag ] = _sp.clone();
                    if ( _glob_maps[ _newtag ] != null )
                    {
                        _glob_maps.remove_key( _tag );
                        circles_lib_output( _out_channel, DISPATCH_SUCCESS, "Map tag '"+_tag+"' has been turned to '"+_newtag+"'", _par_1, _cmd_tag );
                    }
...
function _map_cmd_init( _par_1, _out_channel )
{
     ...
     else circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, _ret_msg, _par_1, _cmd_tag );
     return _ret_id ;
}