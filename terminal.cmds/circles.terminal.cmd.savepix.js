function circles_terminal_cmd_savepix()
{
    ...
    if ( _params.length > 0 )
    {
    	...
    	var _cmd_terms_dict = [];
    	_cmd_terms_dict.push( "bip", "wplane", "zplane", "bipbox", "w-plane", "z-plane", "release", "html", "help" );
        ...
        else if ( !_b_fail ) 
        {
             ...
             switch( _action )
             {
				...
				else if ( !is_html_canvas( _canvas ) ) { _b_fail = YES, _error_str = "Fail to save: invalid source canvas" ; }
                break ;
            }
        }
    }
