...
function circles_terminal_cmd_zoom()
{
	...
	if ( _params.length > 0 )
	{
		...
		for( var _i = 0 ; _i < _params_array.length ; _i++ )
		{
		  ...
		  {
			circles_lib_output( _output_channel, DISPATCH_INFO, "Coordinates will be restored to original values", _par_1, _cmd_tag );
			_params_assoc_array['reset'] = YES ;
		...
		else
		{
			...
			switch( _action )
			{
				...
				break ;
				default: // set coords
				if ( !_params_assoc_array['reset'] && _tofit == NO && _side == 0 && _zf == 0 ) { _b_fail = YES, _error_str = "Can't zoom: invalid 'side' param and zoom factor" ; }
				...
				else
				{
					...
					{
						var _reset = _params_assoc_array['reset'] ;
						if ( _reset )
						{
							...
							circles_lib_output( _output_channel, DISPATCH_SUCCESS, _plane_def + ": coords restored to default values with success", _par_1, _cmd_tag );
						}
						...
						if ( _zf != 0 )
						{
							...
							circles_lib_output( _output_channel, DISPATCH_INFO, _zoom_str, _par_1, _cmd_tag );
							switch( _plane_type )
							{
								case Z_PLANE:
						...
						else
						{
							...
							if ( _plane_type.is_one_of( Z_PLANE, ALL_PLANES ) )
							{
								...
								zplane_sm.set_coords_rect( new rect( _glob_zplaneLEFT, _glob_zplaneTOP, _glob_zplaneRIGHT, _glob_zplaneBOTTOM, _RECT_ORIENTATION_CARTESIAN ) ) ;
								_ret_chunk = circles_lib_canvas_render_zplane( null, zplane_sm, null, YES, YES, _glob_terminal_autorefresh, !_silent, _silent, NO, _output_channel );
							...
							if ( _plane_type.is_one_of( W_PLANE, ALL_PLANES ) )
							{
								...
								wplane_sm.set_coords_rect( new rect( _glob_wplaneLEFT, _glob_wplaneTOP, _glob_wplaneRIGHT, _glob_wplaneBOTTOM, _RECT_ORIENTATION_CARTESIAN ) ) ;
								_ret_chunk = circles_lib_canvas_render_wplane( null, wplane_sm, null, YES, YES, _glob_terminal_autorefresh, NO, !_silent, _silent, _output_channel );
								if ( _ret_chunk[0] ) _ret_chunk[1] = "coordinates have been "+(_reset?"reset":"set")+"\n" + _ret_chunk[1] ;
							}
	...
	if ( _b_fail && _glob_terminal_errors_switch && _output_channel != OUTPUT_FILE_INCLUSION ) circles_lib_output( _output_channel, DISPATCH_ERROR, $.terminal.escape_brackets( _error_str ) + _glob_crlf + "Type '"+_cmd_tag+" /h' for syntax help", _par_1, _cmd_tag );
