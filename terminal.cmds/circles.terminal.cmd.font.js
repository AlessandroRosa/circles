function circles_terminal_cmd_font()
{
     ...
     if ( _params.length > 0 )
     {
         ...
         _cmd_params['keywords'] = NO ;
             _cmd_params['family'] = null ;
             _cmd_params['size'] = null ;
             _cmd_params['action'] = "set" ;
         ...
         for( var _i = 0 ; _i < _params_array.length ; _i++ )
         {
              ...
              {
                   _cmd_params['family'] = DEFAULT_TERMINAL_FONT_FAMILY ; 
                   _cmd_params['size'] = DEFAULT_TERMINAL_FONT_SIZE ; 
              }
              else
              {
                   if ( _p.isAlpha() ) _cmd_params['family'] = _p ; 
                   else if ( _p.isNumber() )
                   {
                       var _fs = safe_float( _p, 6 );
                       if ( _fs <= 0 ) { _b_fail = YES, _error_str = "Font size must be strictly positive" ; break ; }
                       else _cmd_params['size'] = _fs ;
                   }
         ...
         else if ( !_b_fail )
         {
             ...
             switch( _action )
             {
                 ...
                 break ;
                 case "set":
                 if ( _cmd_params['family'] != null ) $("#terminal_div").css( "font-family", _cmd_params['family'] );
                 if ( _cmd_params['size'] != null ) $("#terminal_div").css( { fontSize : _cmd_params['size'] } );
