function circles_terminal_cmd_render()
{
     ...
     if ( _params.length > 0 )
     {
         ...
         // pre-scan for levenshtein correction
    		 var _local_cmds_params_array = [];
    				 _local_cmds_params_array.push( "zplane", "wplane", "clean", "silent", "release", "html", "help" );
         ...
         for( var _i = 0 ; _i < _params_array.length ; _i++ )
         {
              ...
              else if ( _p.is_one_of_i( "clean", "silent" ) ) _cmd_params['settings'].push( _p ) ;
         ...
         else if ( !_b_fail )
         {
             ...
             switch( _action )
             {
                  ...
                  break ;
                  default:
                  if ( _cmd_params['plane'].length == 0 ) { _b_fail = YES, _error_str = "Missing plane target for rendering" ; }
                  else
                  {
                       var _params_str = _cmd_params['plane'] ;
       								 if ( _cmd_params['settings'].includes( "silent" ) ) _params_str += " silent" ;
       								 if ( _cmd_params['settings'].includes( "clean" ) ) _params_str += " clean" ;
                       circles_lib_terminal_interpreter( "refresh " + _params_str, _glob_terminal, _out_channel );
                  }
                  break ;
             }
         }
