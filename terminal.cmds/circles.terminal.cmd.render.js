     ...
     if ( _params.length > 0 )
     {
         ...
         else if ( !_b_fail )
         {
             ...
             switch( _action )
             {
                  ...
                  break ;
                  default:
                  if ( _params_assoc_array['plane'].length == 0 ) { _b_fail = YES, _error_str = "Missing plane target for rendering" ; }
                  else
                  {
                       var _params_str = _params_assoc_array['plane'] ;
       								 if ( _params_assoc_array['settings'].includes( "silent" ) ) _params_str += " silent" ;
       								 if ( _params_assoc_array['settings'].includes( "clean" ) ) _params_str += " clean" ;
                       circles_lib_terminal_interpreter( "refresh " + _params_str, _glob_terminal, _output_channel );
                  }
                  break ;
             }
         }
