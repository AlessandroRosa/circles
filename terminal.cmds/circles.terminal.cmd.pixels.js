function circles_terminal_cmd_pixels()
{
    ...
    else if ( !_b_fail )
    {
		...
		switch( _action )
		{
            ...
            break ;
   			case "size":
			var _size = _glob_rec_canvas_entities_array.keys_associative().length ;
			if ( _size == 0 )
			circles_lib_output( _out_channel, DISPATCH_INFO, "No pixels are stored in memory", _par_1, _cmd_tag );
			else if ( _size > 0 )
			circles_lib_output( _out_channel, DISPATCH_INFO, "There " + ( _size == 1 ? "is " : "are " ) + _size + " pixel" + ( _size == 1 ? "" : "s" ) + " stored in memory", _par_1, _cmd_tag );
			break ;
            default: break ;
		}
     }
