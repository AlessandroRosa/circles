     ...
     if ( _params.length > 0 )
     {
         ...
         else if ( !_b_fail )
         {
             ...
             switch( _action )
             {
                  ...
                  if ( _cc_radius == null || isNaN( _cc_radius ) )
                  {
										 _b_fail = YES, _error_str = "Disk radius has not been input" ;
									}
									else if ( _cc_radius <= 0 )
                  {
										 _b_fail = YES, _error_str = "Disk radius must be strictly positive" ;
									}
                  else if ( _cc_center == null )
                  {
										 _b_fail = YES, _error_str = "Disk center coordinates are missing" ;
									}
                  else if ( _index_selection_array.length > 1 )
                  {
										 _b_fail = YES, _error_str = "Only one symbol must be input" ;
                  }                                        
                  ...
                  else
                  {
                     ...
                     if ( _ret_id == 1 )
                     {
                         ...
                         if ( _new_sd_n == UNDET )
                         {
                            _b_fail = YES, _error_str = "Memory failure while trying to add a new complex disk" ;
                         }
                         else
                         {
                        ...
                        if ( _sel_n > 0 || _all )
                        {
                            ...
                            for( var _i = 0 ; _i < _entries_n ; _i++ )
                            {
                                ...
                                if ( _obj_index != UNDET && is_item_obj( ITEM ) )
                                {
                                    ...
                                    {
                                         if ( _l_array.length == 1 && _l_array[0].isAlpha() &&
                                              _items_array[_obj_index].symbol.length == 0 )
                 ...
                 if ( _n_queue > 0 && _exists )
                 {
                    ...
                    $.each( _params_assoc_array['settings']['storagequeue'],
                            function( _i, _item_obj )
                            {
                               ...
                               {
                                   if ( _subset.start_with( "seeds" ) )
                                       circles_lib_storage_parse_dependencies_syntax( _subset, "add", _item_obj ) ;
