         ...
         else if ( !_b_fail )
         {
             ...
             switch( _action )
             {
                  ...
                  else
                  {
                      ...
                      if ( _sd_n > 0 )
                      {
                          var _seeds_count = 0, _i, _m ;
    				              ...
    				              for( _i = 0 ; _i < _vars_array.length ; _i++ )
    				              {
          						        ...
          						        circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, "scanning for '<white>"+_var_id+"</white>' among <lightblue>seeds</lightblue>", _par_1, _cmd_tag );
                              for( _m = 0 ; _m < _sd_n ; _m++  )
                              {
                                  if ( _glob_seeds_array[_m].symbol.strcmp( _var_id ) )
                                  {
                                      circles_lib_output( _output_channel, DISPATCH_SUCCESS, "Found seed '"+_var_id+"'", _par_1, _cmd_tag );
                                      _b_found = YES, _items_found++ ;
                                      _item_output = _glob_seeds_array[_m].map.output();
                                      circles_lib_output( _output_channel, DISPATCH_INFO, _item_output, _par_1, _cmd_tag );
                                  }
                              }
          						    }
                      }
                      ...
                      var _gg_n = circles_lib_count_gens();
          		        if ( _gg_n > 0 )
          		        {
          				        _b_found = NO ;
          						    ...
          						    {
          						        _var_id = _vars_array[_i] ;
          						        circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, "scanning for '<white>"+_var_id+"</white>' inside the <lightblue>generators set</lightblue>", _par_1, _cmd_tag );
          				            for( var _m = 0 ; _m < _gg_n ; _m++  )
          				            {
          				                if ( _glob_gens_array[_m].symbol.strcmp( _var_id ) )
          				                {
          				                     circles_lib_output( _output_channel, DISPATCH_SUCCESS, "Found generator '"+_var_id+"'", _par_1, _cmd_tag );
          				                     _b_found = YES, _items_found++ ;
                      ...
                      _gg_n = is_array( _glob_terminal_vars_catalogue ) ? _glob_terminal_vars_catalogue.size_associative() : 0 ;
          		        if ( _gg_n > 0 )
          		        {
                           var _custom_var_type, _custom_var_value, _custom_var_formula ;
          						     ...
          						     {
          						          _var_id = _vars_array[_i] ;
          				              circles_lib_output( _output_channel, DISPATCH_MULTICOLOR, "scanning for '<white>"+_var_id+"</white>' among <lightblue>custom vars</lightblue>", _par_1, _cmd_tag );
          				              _custom_var = _glob_terminal_vars_catalogue[ "var."+_var_id ] ;
          				              if ( _custom_var != null )
          				              {
        				                   _items_found++ ;
        				                   circles_lib_output( _output_channel, DISPATCH_SUCCESS, "Found custom var '"+_var_id+"'", _par_1, _cmd_tag );
        				                   _custom_var_type = _custom_var['type'] ;
        				                   _custom_var_value = _custom_var['value'] ;
        				                   _custom_var_formula = _custom_var['formula'] ;
    
         				                   circles_lib_output( _output_channel, DISPATCH_INFO, "type: " + _custom_var_type, _par_1, _cmd_tag );
         				                   switch( _custom_var_type )
         				                   {
          				                     case "integer":
          				                     case "float":
          				                     if ( _custom_var_formula.length > 0 ) circles_lib_output( _output_channel, DISPATCH_INFO, "formula: " + _custom_var_formula, _par_1, _cmd_tag );
          				                     circles_lib_output( _output_channel, DISPATCH_INFO, "value: " + _custom_var_value, _par_1, _cmd_tag );
          				                     break ;
          				                     case "complex":
          				                     var _custom_var_obj = _custom_var['obj'] ;
          				                     if ( _custom_var_formula.length > 0 ) circles_lib_output( _output_channel, DISPATCH_INFO, "formula: " + _custom_var_formula, _par_1, _cmd_tag );
          				                     circles_lib_output( _output_channel, DISPATCH_INFO, "value: " + _custom_var_obj.formula(), _par_1, _cmd_tag );
          				                     break ;
          				                     case "mobius":
          				                     var _custom_var_obj = _custom_var['obj'] ;
          				                     circles_lib_output( _output_channel, DISPATCH_INFO, _custom_var_obj.output(), _par_1, _cmd_tag );
          				                     break ;
          				                     default: break ;
          				                 }
          				              }
                      ...
                      if ( _items_found == 0 ) circles_lib_output( _output_channel, DISPATCH_WARNING, "no items found", _par_1, _cmd_tag );
                   }
                  break ;
             }
         }
