function circles_terminal_cmd_doctor()
{
    ...
    var _cmd_terms_dict = [];
    	_cmd_terms_dict.push( "release", "seeds", "generators", "html", "help" );
    ...
    {    
        ...
        else if ( !_b_fail )
        {
            ...
            switch( _action )
            {
                ...
                var _error_array = _doctor_scan_items_integrity( _items_array, _out_channel, _cmd_tag );
    ...
    else if ( _items_n == 0 ) circles_lib_output( _out_channel, DISPATCH_WARNING, _msg = "Doctor's scan can't start: " + _ERR_33_01, _par_1, _cmd_tag );
...
function _doctor_scan_init_options( _items_array, _out_channel, _par_1, _src )
{
     if ( !is_array( _items_array ) ) _items_array = _glob_seeds_array ;
     var _HALT = NO, _error_scan = 0, _error_counter = 0 ;
     circles_lib_output( _out_channel, DISPATCH_INFO, "", _par_1, _src );
     circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<greenyellow>Start to scan init options</greenyellow>", _par_1, _src );
     if ( _glob_method == METHOD_NONE )
     {
        _error_counter++ ;
        circles_lib_output( _out_channel, DISPATCH_WARNING, "No selected method. Init options scan quits here.", _par_1, _src );
     ...
     else
     {
        ...
        circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<lightblue>Current method</lightblue> <snow>"+_method_def+"</snow>", _par_1, _src );
        circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<lightblue>Current process</lightblue> <snow>"+_process_def+"</snow>", _par_1, _src );
        circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<lightblue>Cleaning all init options</lightblue>", _par_1, _src );
        circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<lightblue>Reset init options for</lightblue> <snow>"+_method_def+"</snow> <lightblue>method</lightblue>", _par_1, _src );
        ...
        circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<lightblue>Dictionary settings optimized for</lightblue> <snow>"+_method_def+"</snow> <lightblue>method</lightblue>", _par_1, _src );
        circles_lib_method_set( _glob_method );
        return [ _error_scan, _error_counter ] ;
     }
}

function _doctor_scan_items_integrity( _items_array, _out_channel, _par_1, _src )
{
     if ( !is_array( _items_array ) ) _items_array = _glob_seeds_array ;

     var _HALT = NO, _error_scan = 0, _error_counter = 0 ;
     circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<greenyellow>Start to scan items integrity</greenyellow>", _par_1, _src );
     ...
     if ( _items_n != _n_items_safe )
     {
         ...
         circles_lib_output( _out_channel, DISPATCH_WARNING, "Found possible memory leak in items archive", _par_1, _src );
         circles_lib_output( _out_channel, DISPATCH_INFO, "Cleaning incoherent entries", _par_1, _src );
         ...
         circles_lib_output( _out_channel, DISPATCH_INFO, _n_incoherent+" incoherent entr"+(_n_incoherent==1?"y has":"ies have")+" been found and removed", _par_1, _src );
         _items_n = circles_lib_count_items( _items_array ) ;
         circles_lib_output( _out_channel, DISPATCH_INFO, "The items archive now includes "+_items_n+" entr"+(_items_n==1?"y":"ies"), _par_1, _src );
     }
     ...
     for( _i = 0 ; _i < _items_n ; _i++ )
     {
        ...
        if ( !is_item_obj( ITEM ) )
        {
            circles_lib_output( _out_channel, DISPATCH_WARNING, "Severe memory error: doctor scan has been halted", _par_1, _src );
            _HALT = YES ;
        }
        else
        {
            ...
            */
            ...
            circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, ( _i > 0 ? _glob_crlf : "" ) + "<yellow>Item #"+(_i+1)+"</yellow>" + _symbol, _par_1, _src );
            circles_lib_output( _out_channel, DISPATCH_INFO, "* scan for alphabet errors", _par_1, _src );
            if ( ITEM.symbol.length == 0 || ITEM.symbol.length != 1 )
            {
               _error_counter++ ;
               circles_lib_output( _out_channel, DISPATCH_WARNING, "Incorrect symbol: attempting to set up a new one", _par_1, _src );
               ...
               {
                  circles_lib_output( _out_channel, DISPATCH_INFO, "New symbol '"+_items_array[_i].symbol+"' has been set", _par_1, _src );
                  if ( _glob_method.is_one_of( METHOD_ALGEBRAIC ) )
                  {
                     _items_array[_i].inverse_symbol = circles_lib_word_inverse_get( ITEM.symbol );
                     circles_lib_output( _out_channel, DISPATCH_INFO, "New inverse symbol '"+_items_array[_i].inverse_symbol+"' has been set", _par_1, _src );
                  }
               }
            }

            circles_lib_output( _out_channel, DISPATCH_INFO, "* scan for geometry errors", _par_1, _src );
            ...
            {
                _error_counter++ ;
                circles_lib_output( _out_channel, DISPATCH_WARNING, "Missing Mobius map for item '"+_current_symbol+"'", _par_1, _src );
                circles_lib_output( _out_channel, DISPATCH_INFO, "Attempting to recover map from associated disk", _par_1, _src );
                ...
                circles_lib_output( _out_channel, DISPATCH_INFO, "Mobius map has been recovered with success", _par_1, _src );
                else
                {
                    circles_lib_output( _out_channel, DISPATCH_WARNING, "Fail to recover Mobius map for item '"+_items_array[_i].symbol+"'", _par_1, _src );
                    _HALT = YES ;
                }
            }
            else if ( _mask & 1 )
            {
                _error_counter++ ;
                circles_lib_output( _out_channel, DISPATCH_WARNING, "Missing disk for item '"+_current_symbol+"'", _par_1, _src );
                circles_lib_output( _out_channel, DISPATCH_INFO, "Attempt to recover it from related Mobius map in the default isometric type", _par_1, _src );
                ...
                circles_lib_output( _out_channel, DISPATCH_INFO, "Disk has been recovered with success", _par_1, _src );
                else
                {
                    circles_lib_output( _out_channel, DISPATCH_WARNING, "Fail to recover disk map for item '"+_items_array[_i].symbol+"'", _par_1, _src );
                    _HALT = YES ;
                }
            }
            else if ( _mask & (1|2) )
            {
                circles_lib_output( _out_channel, DISPATCH_WARNING, "Severe data error: no disk and Mobius map for item '"+_items_array[_i].symbol+"'", _par_1, _src );
                _HALT = YES ;
            }
            ...
            if ( ITEM.complex_circle.radius == 0 )
            {
                _error_scan |= 2 ;
                _error_counter++ ;
                circles_lib_output( _out_channel, DISPATCH_WARNING, "Warning: disk '"+_current_symbol+"' has zero radius", _par_1, _src );
                circles_lib_output( _out_channel, DISPATCH_INFO, "Manual setting needed", _par_1, _src );
            }
            else if ( isNaN( ITEM.complex_circle.center.x ) || isNaN( ITEM.complex_circle.center.y ) )
            {
                _error_scan |= 4 ;
                _error_counter++ ;
                circles_lib_output( _out_channel, DISPATCH_WARNING, "Warning: disk '"+_current_symbol+"' center coords are invalid", _par_1, _src );
                circles_lib_output( _out_channel, DISPATCH_INFO, "Manual setting needed", _par_1, _src );
            }
                      ...
                      ITEM.complex_circle.fillcolor.length == 0 && !_glob_palette_use )
            {
                _error_scan |= 4 ;
                _error_counter++ ;
                circles_lib_output( _out_channel, DISPATCH_WARNING, "Warning: disk '"+_current_symbol+"' colors are empty and palette is not flagged for use", _par_1, _src );
                circles_lib_output( _out_channel, DISPATCH_WARNING, "Border color reset to 'black'. Fill color, if required, could be manually set", _par_1, _src );
                _items_array[_i].complex_circle.bordercolor = DEFAULT_COLOR_STD ;
            }

            circles_lib_output( _out_channel, DISPATCH_INFO, "* scan for draw errors", _par_1, _src );
            ...
            {
                _error_counter++ ;
                circles_lib_output( _out_channel, DISPATCH_WARNING, "Found incongruent draw settings for item '"+_current_symbol+"'", _par_1, _src );
                ...
                circles_lib_output( _out_channel, DISPATCH_INFO, "Fixed", _par_1, _src );
            }
        }
      ...
      {
          circles_lib_output( _out_channel, DISPATCH_WARNING, "", _par_1, _src );
          circles_lib_output( _out_channel, DISPATCH_WARNING, "Warning! All maps have been found to be zero!", _par_1, _src );
          circles_lib_output( _out_channel, DISPATCH_WARNING, "Attempting to recover coefficients from circles", _par_1, _src );
          ...
          _ret_chunk = circles_lib_items_init_group_from_disks( YES, _glob_init_mask, YES, YES, _out_channel ) ;

          if ( _ret_chunk[0] == RET_OK )
          circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, _ret_chunk[1], _par_1, _src );
          else
          circles_lib_output( _out_channel, DISPATCH_ERROR, _ret_chunk[1], _par_1, _src );
      }
