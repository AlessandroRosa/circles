function circles_terminal_cmd_line()
{
    ...
    if ( _params.length > 0 )
    {
        ...
        // pre-scan for levenshtein correction
  		var _local_cmds_params_array = [ "close", "draw", "mark", "unmark", "zplane", "wplane", "bip", "layer",
        ...
        _cmd_params['storagesubset'] = "lines" ;
        ...
        for( _i = 0 ; _i < _up_to_index ; _i++ )
        {
              ...
              else if ( _p.is_one_of_i( "action" ) ) _cmd_params['action'] = _p ;
              ...
              else if ( _p.stricmp( "close" ) ) _cmd_params['close'] = YES ;
              else if ( _p.stricmp( "mark" ) ) _cmd_params['propertiesmask'] |= 1 ;
              else if ( _p.stricmp( "unmark" ) ) _cmd_params['propertiesmask'] &= ~1 ;
        ...
        else if ( !_b_fail )
        {
            ...
            switch( _action )
            {
                ...
                if ( _cmd_params['bordercolor'] == null ) { _b_fail = YES, _error_str = "Missing 'bordercolor' attribute: this line won't be visible" ; }
                else
                {
				...
				if ( !_b_fail )
				{
					circles_lib_draw_polyline( _canvas_context, _mapper, _cmd_params['points'], _bordercolor, _fillcolor, _bordersize, _cmd_params['close'], _opacity, UNDET, _cmd_params['propertiesmask'], YES );
					circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, "<snow>(" + circles_lib_plane_def_get( _cmd_params['plane'] ) + ")</snow> <green>Line processed with success</green>", _par_1, _cmd_tag );

					if ( _cmd_params['rec'] == YES )
					{
						var _rec_chunk = [];
						_rec_chunk['class'] = FIGURE_CLASS_LINE ;
						_rec_chunk['close'] = _cmd_params['close'] ;
						...
						_rec_chunk['obj'] = _cmd_params['points'].clone();
