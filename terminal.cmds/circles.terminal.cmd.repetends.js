function circles_terminal_cmd_repetends()
{
     ...
     if ( _params.length > 0 )
     {
    	...
    	var _cmd_terms_dict = [ "copy", "html", "set", "remove", "flush", "list", "suggest", "release", "help" ];
        ...
        for( var _i = 0 ; _i < _params_array.length ; _i++ )
        {
            ...
            else if ( _p.is_one_of_i( "copy", "html", "silent" ) ) _cmd_params[_p] = YES ;
            else if ( _p.is_one_of_i( "set", "remove", "flush", "list", "suggest", "release" ) ) _cmd_params['action'] = _p ;
            else
            {
              ...
              else if ( _sd_n > 0 )
              {
                   ...
                   switch( _action )
                   {
                        ...
                        if ( _entries_n > 0 )
                        {
							             	...
							             	_params_array['ifquestiondisabled_fn'] = function() { circles_lib_terminal_close( _out_channel ); }
                         ...
                         else circles_lib_output( _out_channel, DISPATCH_WARNING, "The repetends list is already empty", _par_1, _cmd_tag );
                         break ;
                         case "list":
                         var _out_text = circles_terminal_cmd_repetends_list( _out_channel );
                         if ( _cmd_params['dump'] )
                         {
                         ...
                         var removal_fn = function( _termination )
                         {
                             ...
                             circles_lib_output( _out_channel, _msg_type, _msg, _par_1, _cmd_tag );
                             if ( _new_counter == 0 ) circles_lib_output( _out_channel, DISPATCH_WARNING, "The repetends list is empty", _par_1, _cmd_tag );
                         }
                         ...
                         if ( _entries_n > 0 )
                         {
									           ...
									           _params_array['yes_fn'] = function()
													                                 ...
													                                 else
													                                 {
													                                    ...
													                                    {
													                                        case -1:
													                                        circles_lib_output( _out_channel, DISPATCH_WARNING, _remove_word_err + "the repetends list is empty", _par_1, _cmd_tag );
													                                        break ;
													                                        case -2:
													                                        circles_lib_output( _out_channel, DISPATCH_WARNING, _remove_word_err + "the input symbol is empty", _par_1, _cmd_tag );
													                                        break ;
													                                        case -3:
													                                        circles_lib_output( _out_channel, DISPATCH_WARNING, _remove_word_err + "parsing error", _par_1, _cmd_tag );
													                                        break ;
														          				                    default: break ;
													                                    }
													                                 }
                            ...
                            else circles_lib_output( _out_channel, DISPATCH_ERROR, "The input repetend is an empty string", _par_1, _cmd_tag );
                            break ;
                            case "suggest":
                            _repetend_length = safe_int( _repetend_length, 0 );
                            if ( _repetend_length == 0 )
                                 circles_lib_output( _out_channel, DISPATCH_ERROR, "Length shall be a positive integer number", _par_1, _cmd_tag );
                            else if ( _alphabet.length == 0 )
                                 circles_lib_output( _out_channel, DISPATCH_ERROR, _ERR_33_01, _par_1, _cmd_tag );
                            else
                            {
                                ...
                                circles_lib_output( _out_channel, DISPATCH_SUCCESS, "Repetends list has been filled with suggestions", _par_1, _cmd_tag );
                            }
                            break ;
			                default: break ;
                      }
...
function circles_terminal_cmd_repetends_list( _out_channel, _par_1 )
{
     ...
     else if ( _counter == 0 )
     {
         ...
         circles_lib_output( _out_channel, DISPATCH_WARNING, _out_text, _par_1, _cmd_tag );
     }
