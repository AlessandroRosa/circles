...
function circles_terminal_cmd_bip()
{
    ...
    else if ( _params.length > 0 )
    {
        ...
        if ( _cmd_params['dump'] )
        {
            _cmd_params['coordsdiagram'] = "bipbox" ;
            circles_lib_output( _out_channel, DISPATCH_INFO, "BIP params are being set up according to user-defined input", _par_1, _cmd_tag );
        ...
        {
            _p = _params_array[_i];
            if ( _p.toLowerCase().is_one_of_i( "/h", "/help", "--help", "/?" ) ) _cmd_params['help'] = YES ;
            else if ( _p.stricmp( "html" ) ) _cmd_params['html'] = YES ;
            else if ( _p.is_one_of_i( "/k" ) ) _cmd_params['keywords'] = YES ;
			else if ( _cmd_params['action'] == "save" ) _cmd_params['filename'] = safe_string( _p, "" );
            else if ( _p.is_one_of_i( "on", "open" ) )
            {
                _cmd_params['activation'] = YES ;
                _cmd_params['toggle'] = _p ;
            }
            else if ( _p.is_one_of_i( "close", "off" ) )
            {
                _cmd_params['activation'] = NO ;
                _cmd_params['toggle'] = _p.toLowerCase() ;
            }
                 ...
                 _cmd_params['inputparamlabel'] = _p.toLowerCase() ;
            else if ( _p.is_one_of_i( "zplane", "wplane", "bipbox" ) )
            {
				switch( _cmd_params['action'] )
				{
					...
					break ;
					default:
					if ( _cmd_params['inputparamlabel'] != null )
					{
						switch( _cmd_params['inputparamlabel'] )
						{
							case "coords":
							if ( _p.is_one_of_i( "zplane", "wplane", "bipbox" ) ) _cmd_params['coordsdiagram'] = _p.toLowerCase() ;
							break ;
							case "objects":
							if ( _p.is_one_of_i( "zplane", "wplane" ) ) _cmd_params['datadiagram'] = _p.toLowerCase() ;
							break ;
							default:
							if ( _p.is_one_of_i( "zplane", "wplane", "bipbox" ) ) _cmd_params['coordsdiagram'] = _p.toLowerCase() ;
							break ;
					...
					if ( _cmd_params['datadiagram'] != null )
					{
						switch( _cmd_params['datadiagram'].toLowerCase() )
						{
					...
					if ( _cmd_params['coordsdiagram'] != null )
					{
						switch( _cmd_params['coordsdiagram'].toLowerCase() )
						{
            ...
            else if ( _p.stricmp( "svg" ) ) _cmd_params['svg'] = YES ;
            else if ( _p.stricmp( "ps" ) ) _cmd_params['ps'] = YES ;
            else if ( _p.stricmp( "eps" ) ) _cmd_params['eps'] = YES ;
            else if ( _p.testME( _glob_positive_float_regex_pattern ) )
            {
                if ( _cmd_params['inputparamlabel'] == "xextent" ) _cmd_params['xextent'] = safe_float( _p, 0 );
                else if ( _cmd_params['inputparamlabel'] == "yextent" ) _cmd_params['yextent'] = safe_float( _p, 0 );
                else if ( _cmd_params['inputparamlabel'] == "shorterside" ) _cmd_params['shorterside'] = safe_float( _p, 0 );
                _cmd_params['action'] = "apply" ;
            }
            else if ( _p.testME( _glob_cartesian_coords_regex_pattern ) )
            {
                _cmd_params['center'] = _p ;
                var _str = _cmd_params['center'].replaceAll( [ "(", ")" ], "" );
                ...
                if ( _separator.length == 1 )
                {
                    ...
                    if ( _coords_array != null )
                    {
                       ...
                       _cmd_params['center'] = new point( _coords_array[0], _coords_array[1] );
                    }
                    else _cmd_params['center'] = null ;
                }
                _cmd_params['action'] = "apply" ;
            }
            else if ( circles_lib_colors_is_def( _p ) )
			{
				_cmd_params['background'] = _p ;
                _cmd_params['action'] = "apply" ;
			}
            else if ( _p.is_one_of_i( "acquire", "apply", "clean", "settings", "preview", "render", "release", "save" ) ) _cmd_params['action'] = _p ;
            else { _b_fail = YES, _error_str = "Unknown input param '"+_p+"' at token #"+(_i+1); break ; }
        }

        if ( _cmd_params['svg'] != null ) _glob_export_format = EXPORT_SVG ;
        else if ( _cmd_params['png'] != null ) _glob_export_format = EXPORT_PNG ;
        else if ( _cmd_params['ps'] != null ) _glob_export_format = EXPORT_PS ;
        else if ( _cmd_params['eps'] != null ) _glob_export_format = EXPORT_EPS ;
        else if ( _cmd_params['latex'] != null ) _glob_export_format = EXPORT_LATEX ;
        else _glob_export_format = EXPORT_NONE ;

        if ( _cmd_params['help'] )
		{
			circles_lib_terminal_help_cmd( _cmd_params['html'], _cmd_tag, _par_1, _out_channel );
			_out_channel = OUTPUT_TERMINAL ;
		}
        ...
        else if ( _cmd_params['action'] != null )
        {
            ...
            {
                _b_fail = YES, _error_str = "BIP action '"+_cmd_params['action']+"' requires the flag 'on' to take effect" ;
            }
            else
            {
                switch( _action )
                {
                   ...
                   switch( _glob_bip_original_plane_coords )
                   {
					  ...
					  _cmd_params['coordsdiagram'] = _glob_bip_original_plane_coords ;
                      break ;
                      case W_PLANE:
                   ...
                   if ( _coords_rect != null && _display_rect != null )
                   {
					  ...
					  if ( _cmd_params['coordsdiagram'] != null ) _tmp_applied_settings_mask |= 1 ;
                      ...
                      circles_lib_output( _out_channel, DISPATCH_INFO, "Bip box coordinates has been acquired from " + circles_lib_plane_def_get( _glob_bip_original_plane_coords ) + " with success", _par_1, _cmd_tag );
                      circles_lib_output( _out_channel, DISPATCH_INFO, "Bip Box - cartesian X extent is "+_glob_bip_x_extent, _par_1, _cmd_tag );
                      circles_lib_output( _out_channel, DISPATCH_INFO, "Bip Box - cartesian Y extent is "+_glob_bip_y_extent, _par_1, _cmd_tag );
                      circles_lib_output( _out_channel, DISPATCH_INFO, "Bip Box - center at "+_glob_bip_box_center_pt.output(), _par_1, _cmd_tag );
                      circles_lib_output( _out_channel, DISPATCH_INFO, "Bip Box shorter side in pixels : "+_glob_bip_shorterside_pixels, _par_1, _cmd_tag );
                      circles_lib_output( _out_channel, DISPATCH_INFO, "Bip Box - left up corner : "+( _corners['lu'] != null ? ( ( _corners['lu'].x + ", " + _corners['lu'].y ) ) : "undefined" ), _par_1, _cmd_tag );
                      circles_lib_output( _out_channel, DISPATCH_INFO, "Bip Box - right down corner : "+( _corners['rd'] != null ? ( ( _corners['rd'].x + ", " + _corners['rd'].y ) ) : "undefined" ), _par_1, _cmd_tag );

					  _ret_msg = "<white>Type 'bip settings' to resume all settings</white>" ;
					  circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, _ret_msg, _par_1, _cmd_tag );
                   ...
                   var _tmp_applied_settings_mask = 0 ;
                   if ( _cmd_params['coordsdiagram'] != null ) _tmp_applied_settings_mask |= 1 ;
                   if ( _cmd_params['datadiagram'] != null ) _tmp_applied_settings_mask |= 2 ;
                   if ( _cmd_params['center'] != null ) _tmp_applied_settings_mask |= 4 ;
                   if ( _cmd_params['background'] != null ) _tmp_applied_settings_mask |= 8 ;
                   if ( _cmd_params['xextent'] != null ) _tmp_applied_settings_mask |= 16 ;
                   if ( _cmd_params['yextent'] != null ) _tmp_applied_settings_mask |= 32 ;
                   if ( _cmd_params['shorterside'] != null ) _tmp_applied_settings_mask |= 64 ;
                   ...
                   _glob_bip_box_center_pt = ( _cmd_params['center'] != null ) ? _cmd_params['center'] : _glob_bip_box_center_pt ;
                   _glob_bip_bk = ( _cmd_params['background'] != null ) ? _cmd_params['background'] : _glob_bip_bk ;
                   _glob_bip_x_extent = ( _cmd_params['xextent'] != null ) ? _cmd_params['xextent'] : _glob_bip_x_extent ;
                   _glob_bip_y_extent = ( _cmd_params['yextent'] != null ) ? _cmd_params['yextent'] : _glob_bip_y_extent ;
                   _glob_bip_shorterside_pixels = ( _cmd_params['shorterside'] != null ) ? _cmd_params['shorterside'] : _glob_bip_shorterside_pixels ;
                   ...
                   if ( is_html_canvas( _glob_bipbox_canvas ) )
                   {
                      ...
                      var _ret_chunk = circles_lib_canvas_clean( _glob_bipbox_canvas, _glob_bip_bk, _out_channel );
                      ...
                      circles_lib_output( _out_channel, _ret_id == 1 ? DISPATCH_SUCCESS : DISPATCH_ERROR, _ret_msg, _par_1, _cmd_tag );
                   }
                   ...
                   switch( _glob_bip_original_plane_coords )
                   {
                      ...
                      case NO_PLANE: default: _coords_diagram_label = "Undetermined" ; break ;
                        ...
                        else circles_lib_output( _out_channel, DISPATCH_MULTICOLOR, _MSG, _par_1, _cmd_tag );

                    if ( _cmd_params['dump'] )
                    {
						_cmd_params['dump_array'] = is_array( _cmd_params['dump_array'] ) ? _cmd_params['dump_array'][0] : "circles.bip.list.txt" ;
                   ...
                   var _ret_chunk = circles_lib_bip_render( _silent, _out_channel );
                   var _ret_id = is_array( _ret_chunk ) ? safe_int( _ret_chunk[0], NO ) : NO ;
                   ...
                   {
                       var _filename = ""  ;
					   if ( _cmd_params['filename'] != null ) _filename = _cmd_params['filename'] ;
					   else
					   {
                       ...
                       var _ret_chunk = circles_lib_files_pix_save_ask( BIP_BOX, _glob_bipbox_canvas.id, _filename, YES, YES, _out_channel );
                       var _ret_id = is_array( _ret_chunk ) ? safe_int( _ret_chunk[0], NO ) : NO ;
         ...
         else if ( _cmd_params['toggle'] != null )
         {
            var _toggle = _cmd_params['toggle'] ;
            switch( _toggle )
            {
                case "on":
                case "off":
                ...
                else
                {
                    ...
                    var _activation = ( _cmd_params['activation'] != null ) ? ( _cmd_params['activation'] ? YES : NO ) : NO ;
                    ...
                    circles_lib_bip_activate( _glob_bip_use, _out_channel );
    ...
    if ( _b_fail && _glob_terminal_errors_switch && _out_channel != OUTPUT_FILE_INCLUSION )
		circles_lib_output( _out_channel, DISPATCH_ERROR, $.terminal.escape_brackets( _error_str ) + _glob_crlf + "Type '"+_cmd_tag+" /h' for syntax help", _par_1, _cmd_tag );
...
function circles_lib_terminal_bip_apply( _out_channel = OUTPUT_TERMINAL, _par_1 )
{
    var _tmp_applied_settings_mask = 0 ;
    var _ret_chunk = circles_lib_bip_apply_settings( _out_channel, !_glob_terminal_echo_flag, _glob_terminal_echo_flag, NO,
