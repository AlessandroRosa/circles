     ...
     else if ( _params.length > 0 )
     {
       ...
       var _p ;
         for( var _i = 0 ; _i < _params_array.length ; _i++ )
         {
              ...
              else if ( /\d+/g.test( _p ) || _p.is_one_of( "left", "top", "right", "bottom" ) ) _params_assoc_array['coords'].push( _p ) ;
     ...
     else if ( !_b_fail )
     {
         ...
         switch( _action )
         {
            ...
            if ( _params_assoc_array['coords'].length == 2 && _params_assoc_array['wnd'].length == 2 )
            {
                ...
                circles_lib_output( _output_channel, DISPATCH_ERROR, "Can't move: the input popup does not exist", _par_1, _cmd_tag );
            }
            else
            {
